...
| `GET /cases/:id/import/undo-stack` · `POST …/import/undo` · `POST …/import/redo` | **Import undo/redo** (#76). An import can flood the dashboard with hundreds of events; before each import the **pre-import** forensic timeline + IOCs are snapshotted onto a per-case stack (`state/import-undo-stack.json`, `ImportUndoStore`; depth `DFIR_IMPORT_UNDO_DEPTH`, default 10). `undo` restores the latest pre-import snapshot (and pushes the current state to a redo stack); `redo` re-applies it. Only the timeline + IOCs are snapshotted — findings / MITRE / attacker-path are re-derived by the background synthesis that runs after a restore (exactly as on import). `GET …/undo-stack` returns a lightweight `{ canUndo, canRedo, maxDepth, nextUndo, nextRedo, undo:[], redo:[] }` summary (labels/times/counts, not the raw snapshots) for the Undo/Redo buttons above the Forensic Timeline; both POSTs return the updated summary and push `import_undo_changed` over the WS. A no-op re-import adds no checkpoint; an undo also clears the "📥 last import" banner. Machine-local — **not** part of the portable case snapshot. 501 when no state store is wired. |
