...
| `GET /diagnostics/preflight` | **Startup pre-flight check** (#179) — returns `{ report, text }` with one item per configured integration (AI provider = critical, enrichment providers + Velociraptor = non-critical). Results are cached 30 s; `anyCriticalFailed` drives the red dashboard banner. |
