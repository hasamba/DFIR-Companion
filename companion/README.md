...
| `GET /cases/:id/import-meta` | Last-import record for the dashboard's **📥 Last import N ago — +N new events** banner above the Forensic Timeline: `{ lastImportedAt, lastImportKind, lastImportFile, addedCount, removedCount, lastDiff: { added, removed } }`. Written by the unified `/import` route after each import completes (diffs the timeline before/after by normalized time+description); drives the per-row `NEW` highlight. Stored in `state/import-meta.json`; pushed live over the WS (`import_meta_changed`). |
