...
| `POST /cases/:id/remediation-plan` | **AI remediation plan (#178)** — one text-only AI call → `{ plan }` (markdown): a concrete, prioritized, INCIDENT-SPECIFIC remediation plan (Contain now / Eradicate / Harden / Recover / Verify) grounded in the case's findings + the deterministic ATT&CK mitigations, referencing the actual hosts/accounts/CVEs/IOCs. Ephemeral (no state change); 501 without an AI provider. Prompt overridable via `DFIR_AI_REMEDIATION_PROMPT`/`_FILE`. The dashboard's *✨ Generate remediation plan* button renders it atop the panel. |
