...
| `GET /velociraptor/artifacts` · `GET/POST/DELETE /bundles` | **Triage bundles.** `GET /velociraptor/artifacts` lists the configured server's collectable **CLIENT** artifacts (`artifact_definitions()` filtered to `type = CLIENT`) for the bundle builder. `/bundles` is global CRUD over named artifact bundles (built-in *Fast/Full Triage* + custom; built-ins are read-only). Bundles persist next to `cases/` in a `bundles/` dir. |
