...
| `npm run ocr-index -- <caseId>` | Backfill the screenshot OCR full-text search index (#176): OCR every screenshot not already in `metadata/ocr.json`. No AI. Use for a case whose screenshots were captured **before** the feature existed or **before** OCR search was enabled (live captures are indexed automatically). |
