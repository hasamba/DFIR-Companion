...
| `npm run bench:storage` | Repeatable indexed-storage benchmark at 1M and 10M events: import throughput, cold-open latency, filtered-query p50/p95, cursor export and peak RSS. Override with `-- --sizes=100000 --chunk=10000 --query-runs=20`; `--temp-root=<dir>` selects a filesystem with enough free space. |
...
| `GET /diagnostics` | **Health / Diagnostics** (#118, #389) — operator system state plus bounded local importer/query/job/AI/export/WebSocket/capacity metrics and likely remediation. Metric labels are allowlisted and never carry evidence strings. Returns the detailed local `report` plus a separately redacted `supportPreview` that excludes evidence, identifiers, secrets and absolute paths; the dashboard requires preview before download. Nothing is sent externally. Set `DFIR_LOCAL_TELEMETRY=off` to disable collection without affecting core behavior. |
...
| `GET /cases/:id/state` | Current investigation state (JSON). The forensic timeline is cursor-paged (default/max 10,000 rows); use `timelineCursor` / `timelineLimit` and the returned `forensicTimelineNextCursor` / `forensicTimelineTotal` for large cases. |
...
| `GET /cases/:id/activity-log?category=&limit=` | **Per-case activity log** (#238, supersedes #224): a chronological record of every security-relevant action taken on the case — imports, mark/unmark false-positive, AI runs (synthesis/2nd opinion/ask/…), enrichment/anonymization toggles, settings changes, playbook edits, comments/tags, hunt runs, exports. Append-only JSONL (`metadata/activity.jsonl`, mirrors `captures.jsonl`/`imports.jsonl`) — **not** part of `InvestigationState`, so synthesis never wipes it. Optional `category` filters to one of `import`/`triage`/`ai`/`enrichment`/`anonymization`/`settings`/`playbook`/`collaboration`/`hunt`/`export`; `limit` caps the count (default 200, max 1000). Returns entries newest-first; a malformed line is skipped, never fatal. `501` when the store isn't wired. New entries ping dashboard clients over the WS (`activity_changed`). The dashboard's **Activity Log** panel calls this. |
...
| `POST /cases/:id/export/encrypted` · `POST /cases/import/encrypted` | **Encrypted case archive** (issue #56, replaces the old JSON-only snapshot) — the ENTIRE case directory (screenshots, raw evidence, every `state/*.json` file, no allowlist) is zipped, then **AES-256-GCM** encrypted under an analyst-chosen password. `POST /cases/:id/export/encrypted` body `{ password }` (min `MIN_PASSWORD_LENGTH` — 8 — characters, checked server-side, 400 otherwise) → binary `.dfircase` attachment download (`Content-Disposition: attachment; filename="<id>.dfircase"`); 404 if the case doesn't exist. `POST /cases/import/encrypted` body `{ data: base64, password, targetCaseId? }` (base64-in-JSON, matching this codebase's existing convention for binary uploads elsewhere — no multipart/multer) decrypts + unzips into a **new** case; `201 { ...meta, counts }`; `409` when `targetCaseId` already exists (re-import under a new id), `400` on a wrong password, corrupt archive, or malformed payload. Password travels in the POST body, never the URL/query string. Pure crypto in `analysis/caseEncryption.ts`, archive build/restore in `analysis/caseExportArchive.ts`. The dashboard's **Export → Export encrypted case archive (.dfircase)** + **Import case → Encrypted case archive (.dfircase)** call these. |
...
| `GET /jira/status` · `POST /cases/:id/push/jira` · `POST /cases/:id/push/jira/bulk` | Whether a Jira target is configured (plus the configured defaults), and file a **finding** as a Jira issue. Single push body `{ findingId, projectKey?, issueType? }` (404 for an unknown finding); bulk body `{ findingIds, projectKey?, issueType? }` → `{ created, updated, skipped, issues[], issueUrl?, warnings[] }`, aggregated the way the ClickUp playbook push is. A finding Jira refuses — or one the case doesn't have — is **skipped with the reason kept**, so one bad ticket never aborts the batch. Severity maps onto the priority name (critical→Highest…low→Low); every issue is labelled `dfir-companion` + the case id, and `url` is the **browse page**, not the REST resource. **Re-push UPDATES** the issue it created (key remembered in `state/jira-export.json`, `JiraExportStore`) instead of filing a duplicate. Enabled by `DFIR_JIRA_URL` + `_USER` + `_TOKEN` (501 otherwise), with `DFIR_JIRA_PROJECT_KEY` / `DFIR_JIRA_ISSUE_TYPE` as the defaults and `DFIR_JIRA_CA`/`_INSECURE` for a private CA. The dashboard's per-finding **Jira** chip and the finding bulk bar's **Push to Jira** call these; both stay hidden until `/jira/status` reports `configured`. Hand-rolled `JiraClient` over an injectable `fetchFn`; the `pushFindingToJira` / `pushFindingsToJira` orchestrators are unit-tested with no network. |
| `GET /servicenow/status` · `POST /cases/:id/push/servicenow` · `POST /cases/:id/push/servicenow/bulk` | Whether a ServiceNow target is configured (plus the configured defaults), and open a **finding** as an incident. Single push body `{ findingId, caller?, category?, subcategory? }` (404 for an unknown finding); bulk body `{ findingIds, caller?, category?, subcategory? }` → `{ created, updated, skipped, incidents[], incidentUrl?, warnings[] }`, same batch contract as the Jira bulk route above — a refused or unknown finding is skipped with the reason kept, never aborting the rest. Severity maps onto **urgency and impact** (critical/high→1, medium→2, low→3); each field falls back to its `DFIR_SERVICENOW_*` default. **Re-push UPDATES** the incident it opened (sys_id remembered in `state/servicenow-export.json`, `ServiceNowExportStore`) instead of opening a duplicate. Enabled by `DFIR_SERVICENOW_URL` + `_USER` + `_PASSWORD` (501 otherwise), with `DFIR_SERVICENOW_CALLER` / `_CATEGORY` / `_SUBCATEGORY` as the defaults and `DFIR_SERVICENOW_CA`/`_INSECURE` for a private CA. The dashboard's per-finding **SNow** chip and the finding bulk bar's **Push to ServiceNow** call these; both stay hidden until `/servicenow/status` reports `configured`. Hand-rolled `ServiceNowClient` over an injectable `fetchFn`; the `pushFindingToServiceNow` / `pushFindingsToServiceNow` orchestrators are unit-tested with no network. |
...
| `GET /velociraptor/artifacts` · `GET/POST/DELETE /bundles` | **Triage bundles** (dashboard *Settings → Integrations*). `GET /velociraptor/artifacts` lists the configured server's collectable **CLIENT** artifacts (`artifact_definitions()` filtered to `type = CLIENT`) for the bundle builder. The catalog is **cached in-process for 45 s** (each read otherwise spawns the `velociraptor` binary, and the interactive pickers/previews call it repeatedly); `?refresh=1` bypasses it — the picker's *Browse server artifacts* button sends it, so an artifact just added on the server shows up at once, and `POST /velociraptor/reconnect` starts a fresh client with a cold cache. Same for `GET /velociraptor/event-artifacts`. `/bundles` is global CRUD over named artifact bundles (built-in *Best Practice* + custom; bodies accept `{ name, artifacts, description?, defaultWaitMinutes?, timeoutSeconds?, params?, filters? }`). `params` is a per-artifact override map (`{ "Windows.Hayabusa.Rules": { "RuleLevel": "Critical, High, and Medium" } }`) passed to the hunt's `spec` so a heavy artifact emits less at the source; `filters` is a per-artifact VQL `WHERE` map (`{ "DetectRaptor.Generic.Detection.YaraFile": "NOT OSPath =~ 'pagefile'" }`) applied to `hunt_results` **before** the row cap to drop noise at the source. Bundles persist next to `cases/` in a `bundles/` dir. **Built-ins are editable in place**: `POST /bundles` with a built-in id writes an override; `DELETE /bundles/:id` removes a custom bundle **or resets an edited built-in** to its shipped default (404 only for an unknown non-built-in id). |
...
| `POST /cases/:id/velociraptor/import-external` | **Import an external hunt/flow** (launched in the Velociraptor GUI, not by the Companion). Body `{ ref, minSeverity?, superTimelineOnly? }` where `ref` is a **hunt id** (`H.…`), a **flow** (`C.…/F.…`), or the **GUI URL** of either (`parseVeloRef`). For a hunt it discovers the collected artifacts (`getHuntArtifacts`) and reads their rows; for a flow it looks up the collection (`getFlowInfo`) and **resolves the host automatically**, attributing every event to it. Either way the results go through the **same** deterministic Velociraptor import → diff → re-synthesize chain as every other Velociraptor path (reusing the artifact-map ingest), with the optional `minSeverity` floor and an optional **`superTimelineOnly`** route (raw host artifacts land in the super-timeline only, keeping the forensic timeline detections-focused). Returns `200 { kind, huntId?/clientId?/flowId?, artifacts, addedEvents, addedIocs, superTimelineOnly }`. v1 reads **collected rows**; upload-only artifacts (THOR / Hayabusa JSON reports) are a follow-up. 501 when `DFIR_VELOCIRAPTOR_API_CONFIG` is unset; 400 on an unrecognizable ref, 404 when the hunt/flow isn't found on the server. |
...
| `GET /cases/:id/playbook-match` | **Known-playbook sequence match (#230)**: `{ observed[], scopes[], matches[], minScore, caveat, source, generated }` — the case's ATT&CK techniques in **chronological order**, matched against the bundled catalog of canonical ransomware playbooks (`data/known-playbooks.json`: Conti, LockBit, BlackCat, Akira) as ordered technique chains. Where `adversary-hints` answers *which* techniques the case shares with a group, this answers *whether they happened in a known order*. Each match is `{ name, description, score, matchedCount, exactCount, outOfOrderCount, missingCount, scope, host?, steps[] }`, and each step `{ step, status: "matched"｜"missing"｜"out-of-order", matchedTechnique?, matchedEventId?, matchKind? }` — `matchedEventId` is the forensic event that evidences the step, so a UI can jump straight to it. **Fuzzy subsequence**: unrelated techniques between steps are allowed (real timelines are noisy and incomplete); the cursor advances on the first in-order hit, so each step matches at most once. **Sub-technique aware** like `adversaryHints.ts` — an exact id match counts full, a base-only match half (`BASE_MATCH_WEIGHT`) — and `score` is that weighted in-order fraction of the playbook's steps, 0–100. **Scoped**: every playbook is matched against both the whole-case sequence and each **named host's** slice of it, keeping its best (reported as `scope`/`host`) — a ransomware chain crosses hosts, so per-host-only matching would break it, while case-only matching lets one host's noise consume another's chain. Events with no recorded `asset` get no host scope (that bucket can hold several machines). Chronology uses the shared `sortByEventTime`, which parses timestamps — a string sort mis-orders mixed UTC offsets and floats undated events to the front. A match must clear both `DFIR_PLAYBOOK_MIN_SCORE` (default 40, i.e. two of a five-step chain; override per call with `?minScore=`) and a structural two-step minimum — a single shared technique is an overlap, not a sequence, and T1486 alone would otherwise "match" every ransomware playbook. Top-N via `DFIR_PLAYBOOK_TOP_N` (default 3) or `?topN=`. **Branching**: a step may list `alternatives` — the documented variants of that stage (LockBit enters by exploit OR stolen credentials), matched as any-of. Each playbook carries the CISA advisory it was distilled from (`reference`), and each step its ATT&CK `tactic`. Offline and pure (`buildPlaybookMatchResult`) — no AI, no network. Powers the dashboard's *Playbook Match* panel and report §4.6.2; a step the chain implies but nothing evidenced becomes an `playbook_step` item in **Evidence Gaps** / `GET /cases/:id/known-unknowns`, carrying the same deployable collection directive as an uncovered phase. **Sequence similarity to a published playbook, not attribution** (`caveat`; render it wherever matches are shown). |
...
## Case folder layout
    ...
    cases/<caseId>/
      ...
      state/
        investigation.sqlite              indexed findings/IOCs/forensic events (primary state)
        investigation.json                legacy pre-SQLite state (retained unchanged after migration)
...
On first open, a legacy `state/investigation.json` is transactionally imported into
`state/investigation.sqlite`. The JSON file is left untouched as the rollback source; SQLite is
authoritative after the migration commits. An interrupted migration is discarded and retried from
the original JSON on the next open. Backups use SQLite's consistent snapshot operation and store the
database beside the small JSON backup manifest, so the database is never base64-loaded into one
JavaScript string.
...
**Super-Timeline (two-tier timeline).** The companion keeps two timelines. The **forensic
timeline** is unchanged — normal imports populate it and the AI synthesizes it into findings,
so it stays detections-focused. Alongside it, the **super-timeline** is a Timesketch-style
**complete record of every imported event**, held in a separate, capped, per-case store the AI
never synthesizes. A dedicated dashboard section lets an analyst **filter** it by time / origin
(an origin checklist hides noisy detection sources — Sigma / YARA / Hayabusa — to see only raw
host artifacts) / **label**, save named **timeframes**, and label individual events. When an
event in the super-timeline matters to the analysis, **promote** it: the companion copies it into
the forensic timeline so the AI synthesizes it too. A built-in **Super-Timeline Triage**
...
of our deterministic rules graded it); **Info** telemetry goes to the **super-timeline only**, where
it's still browsable and **promotable**. This keeps the forensic timeline detections-focused and stops
synthesis being swamped by thousands of raw Info rows. The cut is configurable: global env
`DFIR_FORENSIC_MIN_SEVERITY` (default `Low`; set `Info` to restore the old "every event into forensic"
behavior, or `Medium`/`High`/`Critical` to cut harder) with a **per-case override**
(`state/forensic-gate.json`, `GET`/`PUT /cases/:id/forensic-gate`). The super-timeline still receives
**everything**, **promotion always bypasses the gate**, and **IOCs are still extracted from every event
(Info included)** — only forensic *timeline events* are gated. Applies to new imports only; existing
cases are untouched.

**IOC provenance (detection-linked vs telemetry-only).** Each IOC is classed by where it was seen:
**detection-linked** (it appears in at least one Low+ event) or **telemetry-only** (it only ever
appears in Info events). This is derived on read over the forensic ∪ super events and is **distinct
from the threat-intel verdict** — a telemetry-only IOC can still be malicious, and a detection-linked
...
the indexed investigation state and is never wiped by synthesis). Synthesis and the hunt-suggestion prompts
