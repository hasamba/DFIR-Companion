...
| `POST /cases/:id/velociraptor/run-bundle` · `GET …/hunt-job` · `POST …/collect` | Run a bundle as a **hunt** (`{ bundleId, waitMinutes?, includeLabels?, excludeLabels?, os? }` → 202 `{ huntId, guiUrl, collectAt }`), then **auto-collect** after `waitMinutes` (default `DFIR_VELO_HUNT_WAIT_MIN` / per-bundle default; clamped 1–1440): the per-artifact hunt results form the `{ "Artifact.Name": [rows] }` artifact-map fed to the deterministic **Velociraptor importer**, which records the import-meta diff and triggers synthesis. `hunt-job` returns the persisted per-case job (`state/velo-hunt.json`, survives restart) for the dashboard's status card + countdown; `collect` pulls results **now** (early, or to re-pull stragglers). All 501 when `DFIR_VELOCIRAPTOR_API_CONFIG` is unset. |
