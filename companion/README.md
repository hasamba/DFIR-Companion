...
| `npm run ocr-index -- <caseId>` | Backfill the screenshot OCR full-text search index (#176): OCR every screenshot not already in `metadata/ocr.json`. No AI. Use after enabling OCR search on a case captured earlier, or to recover entries skipped under load. |
