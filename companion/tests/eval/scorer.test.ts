...
describe("eventMatches (#64 fuzzy predicate)", () => {
  it("matches when every specified constraint holds; ignores omitted ones", () => {
    ...
    const produced = ev({
      ...
      description: "Mimikatz sekurlsa::logonpasswords on DC01",
      severity: "Critical",
      mitreTechniques: ["T1003.001"],
      asset: "dc01",
    });
  ...
  it("fails when the timestamp is outside tolerance", () => {
    expect(
      eventMatches(
        { timestamp: "2026-06-01T10:00:00Z" },
        ev({ id: "e1", timestamp: "2026-06-01T10:30:00Z" }),
      ),
    ).toBe(false);
    expect(
      eventMatches(
        { timestamp: "2026-06-01T10:00:00Z" },
        ev({ id: "e1", timestamp: "2026-06-01T10:04:00Z" }),
      ),
    ).toBe(true);
  });
  ...
  it("fails (does not throw) on an unparseable required timestamp", () => {
    expect(
      eventMatches({ timestamp: "2026-06-01T10:00:00Z" }, ev({ id: "e1", timestamp: "not-a-date" })),
    ).toBe(false);
  });

  it("requires ALL keywords and at least ONE technique overlap", () => {
    expect(
      eventMatches({ keywords: ["psexec", "smb"] }, ev({ id: "e1", description: "psexec over smb" })),
    ).toBe(true);
    expect(eventMatches({ keywords: ["psexec", "smb"] }, ev({ id: "e1", description: "psexec only" }))).toBe(
      false,
    );
    expect(
      eventMatches({ mitreTechniques: ["T1021.002", "T1570"] }, ev({ id: "e1", mitreTechniques: ["T1570"] })),
    ).toBe(true);
    expect(
      eventMatches({ mitreTechniques: ["T1021.002"] }, ev({ id: "e1", mitreTechniques: ["T1059"] })),
    ).toBe(false);
  });
  ...
  it("respects a custom tolerance window", () => {
    expect(
      eventMatches(
        { timestamp: "2026-06-01T10:00:00Z" },
        ev({ id: "e1", timestamp: "2026-06-01T10:20:00Z" }),
        { toleranceMinutes: 30 },
      ),
    ).toBe(true);
  });
  ...
  it("does not count finer-grained duplicates of a golden as false positives", () => {
    ...
    const perRow = [1, 2, 3].map((n) =>
      ev({
        id: `e${n}`,
        timestamp: `2026-06-01T10:00:0${n}Z`,
    ...
    const withNoise = [
      ...perRow,
      ev({ id: "n1", timestamp: "2026-06-01T10:14:00Z", description: "asmith logged off", asset: "WS03" }),
    ];
  ...
  it("accepts a produced SUB-technique for a golden parent, but not the reverse", () => {
    ...
    expect(
      eventMatches({ mitreTechniques: ["T1110"] }, ev({ id: "e1", mitreTechniques: ["T1110.001"] })),
    ).toBe(true);
    expect(
      eventMatches({ mitreTechniques: ["T1110.001"] }, ev({ id: "e1", mitreTechniques: ["T1110"] })),
    ).toBe(false);
  });
});
...
describe("scoreExtraction (#64 precision/recall)", () => {
  it("computes perfect precision/recall when golden and produced align 1:1", () => {
    ...
    const produced = [
      ev({ id: "e1", description: "successful logon" }),
      ev({ id: "e2", description: "mimikatz run" }),
    ];
  ...
  it("counts an unmatched produced event as a false positive (noise)", () => {
    const s = scoreExtraction(
      [{ keywords: ["logon"] }],
      [ev({ id: "e1", description: "logon" }), ev({ id: "e2", description: "screensaver started" })],
    );
  ...
  it("counts an unmatched golden as a false negative (missed detection)", () => {
    const s = scoreExtraction(
      [{ keywords: ["logon"] }, { keywords: ["exfil"] }],
      [ev({ id: "e1", description: "logon" })],
    );
  ...
  it("does not let duplicate produced events inflate recall past the golden count", () => {
    const s = scoreExtraction(
      [{ keywords: ["logon"] }],
      [ev({ id: "e1", description: "logon" }), ev({ id: "e2", description: "logon" })],
    );
...
describe("checkSynthesis (#64 coverage + hallucination + rubric)", () => {
  ...
  it("treats an IOC-only finding as grounded", () => {
    const findings: ProducedFinding[] = [
      { id: "f1", severity: "Medium", relatedEventIds: [], relatedIocs: ["i1"] },
    ];
    const r = checkSynthesis(events, findings);
