...
test("US-238: a toolbar that cannot fit its labels collapses to reachable icons", async ({ page }) => {
  ...
  //
  // The labeled baseline is a width with room to spare, and it drifts as buttons are added. The
  // full-label row wrapped at 1900 first (settled scrollHeight 77 with the class held off, vs
  // threshold ~49.6 — an earlier "settled 38" reading was taken with icons-only applied, i.e. it
  // measured the collapsed row). Then #1569's "Second look" button made it wrap at 2560 too
  // (toolbar 2404px wide, scrollHeight 68); it fits from 2800 (#1581). A collapse at 2560 is
  // therefore correct, not a race. If this baseline fails again, hold the class off and read
  // scrollHeight at this width before suspecting dashboard-toolbar-responsive.js.
  const LABELED_WIDTH = 3840;
  await page.setViewportSize({ width: LABELED_WIDTH, height: 800 });
  ...
  await expect(toolbar, `at ${LABELED_WIDTH}px the labels must fit uncollapsed`).not.toHaveClass(
    /\bicons-only\b/,
  );
  ...
  // A visible button can still be blank, so read the icon itself off the ::before.
  for (const id of ["#importBtn", "#newCaseBtn"]) {
    const btn = page.locator(id);
    await expect(btn).toBeVisible();
    const icon = await btn.evaluate((el) => {
      const cs = getComputedStyle(el, "::before");
      return { image: cs.backgroundImage, width: parseFloat(cs.width) || 0 };
    });
  ...
  await page.setViewportSize({ width: LABELED_WIDTH, height: 800 });
  await expect(toolbar, `growing back to ${LABELED_WIDTH}px must restore the labels`).not.toHaveClass(
    /\bicons-only\b/,
    { timeout: 10_000 },
  );
});
