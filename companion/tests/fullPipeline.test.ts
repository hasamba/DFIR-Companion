        ...
        iocs: [
          {
            id: "i1",
            type: "hash",
            value: "4813e753f6f9bfa5c5de0edbb8dd3cc7f1fa51714097d3144d44e5e89dbd33ef",
        ...
        forensicEvents: [
          {
            ...
            description: "suspicious process observed in screenshot",
            severity: "High",
        ...
        findings: [
          {
            ...
            description: "Execution observed on WIN-01",
            relatedIocs: ["i1"],
            mitreTechniques: ["T1059"],
            status: "open",
            relatedEventIds: ["e-from-capture"],
          },
        ],
        iocs: [
          {
...
describe("full-pipeline integration (capture → import → synthesis → report → encrypted-archive restore)", () => {
  it(
    "runs the complete lifecycle end-to-end with mocked AI and enrichment",
    async () => {
      ...
      let state = await pollState(
        stateStore,
        "pipeline-1",
        "extraction + auto-synthesis to produce a finding",
      ...
      state = await pollState(
        stateStore,
        "pipeline-1",
        "the THOR import to bring the forensic timeline to 2 events",
        (s) => s.forensicTimeline.length >= 2,
      );
      ...
      await request(app)
        .post("/cases/pipeline-1/enrich-control")
        .send({ providers: ["MockTI"] });
      const enrich = await request(app).post("/cases/pipeline-1/enrich").send({});
      expect(enrich.status).toBe(202);

      state = await pollState(stateStore, "pipeline-1", "the MockTI enrichment to annotate an IOC", (s) =>
        s.iocs.some((i) => i.enrichments),
      );
      ...
      expect(exportRes.headers["content-disposition"]).toContain(
        'attachment; filename="pipeline-1 - Full Pipeline Test.dfircase"',
      );
      ...
      const restore = await request(app)
        ...
        .send({
          ...
          targetCaseId: "pipeline-1-restored",
        });
      ...
      expect(restored.body.findings.some((f: { title: string }) => f.title === "Suspicious execution")).toBe(
        true,
      );
      ...
      const restoredEvidence = await request(app).get(
        `/cases/pipeline-1-restored/evidence/${screenshotFile}`,
      );
  ...
  it("does not leave AI or enrichment provider artifacts behind when AI is off", async () => {
    ...
    await request(app)
      .post("/cases")
      .send({ caseId: "pipeline-ai-off", name: "AI Off", investigator: "test", aiProvider: "mock" });
    // AI stays off
