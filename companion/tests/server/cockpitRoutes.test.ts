...
async function makeApp() {
  ...
  await request(app).post("/cases").send({ caseId: "c1", name: "Case", investigator: "Case Owner", aiProvider: null });
...
describe("cockpit routes", () => {
  ...
  it("returns 404 for an unknown case without creating cockpit state", async () => {
    ...
    expect((await request(app).post("/cases/missing/cockpit/review").send({ investigator: "Alice" })).status).toBe(404);
  });
  ...
  it("pins and assigns a finding through its owning stores while preserving cockpit audit history", async () => {
    ...
    expect((await request(app)
      ...
      .send({ action: "pin", actor: "Alice" })).status).toBe(200);
    expect((await request(app)
      .patch("/cases/c1/cockpit/cards/lead%3Afinding%3Af1")
      .send({ action: "assign", actor: "Alice", value: "Bob" })).status).toBe(200);

    expect(await pinnedFindingsStore.load("c1")).toEqual([expect.objectContaining({ findingId: "f1", pinnedBy: "Alice" })]);
    expect(await findingWorkflowStore.load("c1")).toEqual([expect.objectContaining({ findingId: "f1", assignee: "Bob" })]);
  ...
  it("dismisses, defers, restores, and marks review per investigator", async () => {
    ...
    expect(cockpit.body.parked[0]).toMatchObject({ id: "lead:finding:f1", deferredUntil: "2099-01-01T00:00:00.000Z" });
  ...
  it("rejects unknown cards and invalid actions", async () => {
    const { app } = await makeApp();
    expect((await request(app)
      .patch("/cases/c1/cockpit/cards/not-real")
      .send({ action: "pin", actor: "Alice" })).status).toBe(404);
    expect((await request(app)
      .patch("/cases/c1/cockpit/cards/lead%3Afinding%3Af1")
      .send({ action: "explode", actor: "Alice" })).status).toBe(400);
  });
});
