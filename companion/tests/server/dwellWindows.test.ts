...
describe("dwell-window routes", () => {
  ...
  it("PUT updates (200), DELETE removes (204), then GET is empty; PUT/DELETE 404 on unknown id", async () => {
    const { app } = await makeApp();
    const created = (
      await request(app).post("/cases/c1/dwell-windows").send({
        label: "old",
        start: "2026-05-20T08:00:00Z",
        end: "2026-05-20T12:00:00Z",
      })
    ...
    expect(
      (
        await request(app).put("/cases/c1/dwell-windows/nope").send({
          label: "x",
          start: "2026-05-20T08:00:00Z",
          end: "2026-05-20T12:00:00Z",
        })
...
const bundleRunner: VqlRunner = async (statements) => {
  ...
  if (p.includes("hunt(") && p.includes("artifacts=["))
    return { rows: [{ Hunt: { HuntId: "H.DWELL1", state: "RUNNING" } }], raw: "" };
...
describe("run-bundle × dwell window", () => {
  it("threads dwellWindowId from run-bundle into the resulting VeloHuntJob", async () => {
    ...
    const res = await request(app).post("/cases/c1/velociraptor/run-bundle").send({
      bundleId: "super-timeline-triage",
      dwellWindowId: windowId,
      waitMinutes: 30,
    });
