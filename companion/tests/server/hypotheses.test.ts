...
describe("hypothesis routes (#140)", () => {
  ...
  it("PATCHes status/notes (marks analystTouched), 404s an unknown id", async () => {
    const { app } = await makeApp();
    const created = (
      await request(app).post("/cases/c1/hypotheses").send({ title: "Staging before encryption" })
    ).body;
    const patched = await request(app)
      .patch(`/cases/c1/hypotheses/${created.id}`)
      .send({ status: "supported", notes: "confirmed via prefetch" });
    ...
    expect((await request(app).patch("/cases/c1/hypotheses/nope").send({ status: "refuted" })).status).toBe(
      404,
    );
  });
  ...
  it("DELETEs a hypothesis (204) then 404s", async () => {
    const { app } = await makeApp();
    const created = (
      await request(app).post("/cases/c1/hypotheses").send({ title: "Lateral movement via SMB" })
    ).body;
