...
describe("bundle time scope — preview", () => {
  ...
  it("reports which artifacts get scoped and which collect in full", async () => {
    const res = await request(app)
      .post("/velociraptor/bundles/scoped-test/time-scope-preview")
      .send({ preset: "30d" });
    expect(res.status).toBe(200);
    expect(res.body.scoped).toEqual([
      {
        artifact: "Windows.EventLogs.Evtx",
        startParam: "DateAfter",
        endParam: "DateBefore",
        source: "detected",
      },
    ]);
  ...
  it("honours a saved per-bundle correction", async () => {
    ...
    const res = await request(app)
      .post("/velociraptor/bundles/scoped-test/time-scope-preview")
      .send({ preset: "7d" });
    expect(res.body.scoped[0]).toEqual({
      artifact: "Windows.EventLogs.Evtx",
      startParam: "EarliestTime",
      source: "correction",
    });
  });
  ...
  it("404s for an unknown bundle and 400s when no scope is given", async () => {
    expect(
      (await request(app).post("/velociraptor/bundles/nope/time-scope-preview").send({ preset: "7d" }))
        .status,
    ).toBe(404);
    expect(
      (await request(app).post("/velociraptor/bundles/scoped-test/time-scope-preview").send({})).status,
    ).toBe(400);
  });
...
describe("bundle time scope — run-bundle", () => {
  ...
  it("passes the window into the hunt spec for scopeable artifacts only", async () => {
    const run = await request(app)
      ...
      .send({
        bundleId: "scoped-test",
        waitMinutes: 30,
        timeScope: { start: "2026-06-01T00:00:00Z", end: "2026-06-30T00:00:00Z" },
      });
    ...
    expect(vql).toContain(
      "`Windows.EventLogs.Evtx`=dict(DateAfter='2026-06-01T00:00:00.000Z', DateBefore='2026-06-30T00:00:00.000Z')",
    );
  ...
  it("records degraded:true on the job when the catalog fetch fails but the hunt still launches", async () => {
    ...
    const catalogFailsRunner: VqlRunner = async (statements) => {
      ...
      if (p.includes("hunt(") && p.includes("artifacts=[")) {
        launched.push(p);
        return { rows: [{ Hunt: { HuntId: "H.DEG1", state: "RUNNING" } }], raw: "" };
      }
      return { rows: [], raw: "" };
    };
    const degradedApp = await makeApp(catalogFailsRunner);
    const run = await request(degradedApp)
      .post("/cases/c1/velociraptor/run-bundle")
      .send({
        bundleId: "scoped-test",
        waitMinutes: 30,
  ...
  it("REGRESSION: an unscoped run launches exactly as before and records no window", async () => {
    await request(app)
      .post("/cases/c1/velociraptor/run-bundle")
      .send({ bundleId: "scoped-test", waitMinutes: 30 });
  ...
  it("rejects an invalid window before launching anything", async () => {
    const res = await request(app)
      ...
      .send({
        bundleId: "scoped-test",
        timeScope: { start: "2026-06-30T00:00:00Z", end: "2026-06-01T00:00:00Z" },
      });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/end must be at or after start/);
...
describe("bundle time scope — inline correction", () => {
  ...
  it("saves a correction without disturbing the rest of the bundle, and the preview then uses it", async () => {
    const before = (await request(app).get("/bundles")).body.find(
      (b: { id: string }) => b.id === "scoped-test",
    );

    const put = await request(app)
      .put("/velociraptor/bundles/scoped-test/time-scope-param-names")
      .send({
        timeScopeParamNames: { "Windows.EventLogs.Evtx": { start: "EarliestTime", end: "LatestTime" } },
      });
    expect(put.status).toBe(200);

    const after = (await request(app).get("/bundles")).body.find(
      (b: { id: string }) => b.id === "scoped-test",
    );
    expect(after.artifacts).toEqual(before.artifacts);
    expect(after.name).toBe(before.name);
    expect(after.timeScopeParamNames).toEqual({
  ...
  it("404s for an unknown bundle", async () => {
    const res = await request(app)
      .put("/velociraptor/bundles/nope/time-scope-param-names")
      .send({ timeScopeParamNames: {} });
