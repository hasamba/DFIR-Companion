...
const runner: VqlRunner = async (statements) => {
  ...
  if (p.includes("hunt(") && p.includes("artifacts="))
    return { rows: [{ Hunt: { HuntId: "H.DEPLOY1", state: "RUNNING" } }], raw: "" };
  if (p.includes("hunt_results("))
    return { rows: [{ Message: "suspicious thing", Timestamp: "2026-06-01T10:00:00Z" }], raw: "" };
  return { rows: [], raw: "" };
};
...
describe("hunting feedback loop — routes (#157)", () => {
  it("deploy-hunt (fleet) launches a hunt and records a pending outcome", async () => {
    ...
    expect(profile.hunts[0]).toMatchObject({
      title: "Hunt rogue processes",
      source: "fleet",
      status: "deployed",
      huntId: "H.DEPLOY1",
  ...
  it(
    "collecting a deployed hunt fills its outcome as a hit",
    async () => {
      const { app } = await makeApp();
      await request(app)
        .post("/cases/c1/velociraptor/deploy-hunt")
      ...
      expect(
        (await request(app).post("/cases/c1/velociraptor/collect").send({ huntId: "H.DEPLOY1" })).status,
      ).toBe(202);
  ...
  it("deploy-hunt validates vql/title and collection-mode hostname", async () => {
    const { app } = await makeApp();
    expect(
      (await request(app).post("/cases/c1/velociraptor/deploy-hunt").send({ title: "no vql" })).status,
    ).toBe(400);
    expect(
      (await request(app).post("/cases/c1/velociraptor/deploy-hunt").send({ vql: "SELECT 1" })).status,
    ).toBe(400);
    expect(
      (
        await request(app)
          .post("/cases/c1/velociraptor/deploy-hunt")
          .send({ vql: "SELECT 1", title: "t", mode: "collection" })
      ).status,
    ).toBe(400);
  });
  ...
  it("deploy-hunt is 501 when Velociraptor is not configured", async () => {
    ...
    const res = await request(app)
      .post("/cases/c1/velociraptor/deploy-hunt")
      .send({ vql: "SELECT 1", title: "t" });
    expect(res.status).toBe(501);
  });

  it("GET hunt-outcomes returns an empty profile for a fresh case", async () => {
    const { app } = await makeApp();
    expect((await request(app).get("/cases/c1/hunt-outcomes")).body).toEqual({
      total: 0,
      hit: 0,
      missed: 0,
      pending: 0,
  ...
  it("suggest-hunts excludes a VQL that was already deployed", async () => {
    ...
    const { app, stateStore, huntOutcomeStore } = await makeApp({
      provider: new MockProvider("mock", canned),
    });
    ...
    expect(
      (await request(app).post("/cases/c1/velociraptor/suggest-hunts").send({})).body.suggestions,
    ).toHaveLength(1);

    // Record the same VQL as already deployed → it must be filtered out next time.
    await huntOutcomeStore.save(
      "c1",
      recordDeploy([], {
        source: "fleet",
        title: "Hunt ASPX webshells",
        vql,
        huntId: "H.OLD",
        deployedAt: "2026-06-20T00:00:00.000Z",
      }),
    );
...
describe("re-collect coalescing (#195)", () => {
  it(
    "coalesces a collect that arrives mid-collect instead of dropping it",
    async () => {
      ...
      const firstCollectStarted = new Promise<void>((r) => {
        markStarted = r;
      });
      let resultCalls = 0;
      ...
      const runner: VqlRunner = async (statements) => {
        const p = statements[0];
        if (p.includes("hunt(") && p.includes("artifacts="))
          return { rows: [{ Hunt: { HuntId: "H.RUN1", state: "RUNNING" } }], raw: "" };
        if (p.includes("hunt_results(")) {
          resultCalls += 1;
      ...
      await request(app)
        .post("/cases/c1/velociraptor/deploy-hunt")
        .send({ vql: "SELECT * FROM pslist()", title: "recurring hunt", source: "fleet" });
      ...
      releaseFirst();
      await pollFor(
        "hunt H.RUN1 to report the straggler row picked up by the coalesced re-collect",
        async () => {
          const profile = (await request(app).get("/cases/c1/hunt-outcomes")).body;
  ...
  it(
    "recovers a hunt left persisted as 'collecting' by a crashed collect",
    async () => {
...
describe("run-to-run hunt diffing (#80)", () => {
  ...
  it(
    "diffs a re-run against its own previous run's rows/hosts, not the whole case",
    async () => {
      let huntSeq = 0;
      ...
      await request(app)
        .post("/cases/c1/velociraptor/deploy-hunt")
        .send({ vql, title: "recurring hunt", source: "fleet" });
      expect(
        (await request(app).post("/cases/c1/velociraptor/collect").send({ huntId: "H.RUN1" })).status,
      ).toBe(202);
      ...
      await request(app)
        ...
        .send({ vql, title: "recurring hunt", source: "fleet" });
      expect(
        (await request(app).post("/cases/c1/velociraptor/collect").send({ huntId: "H.RUN2" })).status,
      ).toBe(202);
  ...
  it(
    "advances the baseline on same-huntId re-collects so a later re-run doesn't re-report stragglers",
    async () => {
