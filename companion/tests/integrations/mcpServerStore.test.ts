...
describe("McpServerStore delivery config", () => {
  ...
  it("stores an scp block", async () => {
    expect((await store.add({ id: "sift-mcp", delivery: SCP })).delivery).toMatchObject({
      mode: "scp",
      host: "sift.lab",
      user: "analyst",
      remoteDir: "/cases/incoming",
    });
  });
  ...
  it("fills defaults around a partial block", async () => {
    const added = await store.add({
      id: "sift-mcp",
      delivery: { mode: "scp", host: "sift.lab", remoteDir: "/tmp/x" },
    });
    expect(added.delivery).toMatchObject({ port: 22, user: "", identityFile: "" });
  });

  it("requires a host and a remote directory for scp", async () => {
    await expect(store.add({ id: "a", delivery: { mode: "scp", remoteDir: "/x" } })).rejects.toThrow(
      /needs a host/,
    );
    await expect(store.add({ id: "b", delivery: { mode: "scp", host: "sift.lab" } })).rejects.toThrow(
      /needs a remote staging directory/,
    );
  });
  ...
  it("refuses a host or user that could carry shell meaning", async () => {
    await expect(store.add({ id: "a", delivery: { ...SCP, host: "sift.lab; rm -rf /" } })).rejects.toThrow(
      /host .* may only contain/,
    );
    await expect(store.add({ id: "b", delivery: { ...SCP, user: "an$(whoami)" } })).rejects.toThrow(
      /user .* may only contain/,
    );
  });

  it("requires the remote directory to be an absolute POSIX path", async () => {
    await expect(store.add({ id: "a", delivery: { ...SCP, remoteDir: "relative/dir" } })).rejects.toThrow(
  ...
  it("requires both halves of a shared-path rewrite, or neither", async () => {
    await expect(
      store.add({ id: "a", delivery: { mode: "remote-path", localPrefix: "/srv" } }),
    ).rejects.toThrow(/both a local prefix and a remote prefix/);
    await expect(
      store.add({ id: "b", delivery: { mode: "remote-path", localPrefix: "/srv", remotePrefix: "/mnt" } }),
    ).resolves.toBeTruthy();
  });
  ...
  it("validates on update, leaving the stored block untouched when it fails", async () => {
    ...
    await expect(store.update("sift-mcp", { delivery: { host: "evil;host" } })).rejects.toThrow(
      /may only contain/,
    );
