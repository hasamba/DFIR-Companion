...
describe("sanitizeVqlDurations", () => {
  it("rewrites day suffix to seconds arithmetic", () => {
    expect(sanitizeVqlDurations("WHERE TimeCreated > now() - 30d")).toBe(
      "WHERE TimeCreated > now() - 30 * 86400",
    );
  });
  ...
  it("rewrites the exact failing query from issue", () => {
    const input =
      'SELECT * FROM parse_evtx(files="C:/Windows/System32/Winevt/Logs/Microsoft-Windows-Windows Defender%4Operational.evtx") WHERE System.EventID.Value = 5001 AND TimeCreated > now() - 30d';
...
describe("VelociraptorClient.run (server-side)", () => {
  ...
  it("rejects a query that has no runnable VQL", async () => {
    ...
    await expect(new VelociraptorClient(cfg, runner).run("-- only a comment")).rejects.toThrow(
      /No runnable VQL/,
    );
  });
...
describe("VelociraptorClient.launchHunt", () => {
  it("packages the pivots as a CLIENT artifact and launches a hunt across all clients", async () => {
    ...
    const res = await new VelociraptorClient({ ...cfg, guiUrl: "https://velo.example/" }, runner).launchHunt(
      '-- file presence\nSELECT OSPath FROM glob(globs="C:/**/x.exe")',
      "find x.exe",
    );
  ...
  it("uses a supplied relative expiry (seconds)", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      ...
      return { rows: [{ Hunt: { HuntId: "H.EXP", state: "RUNNING" } }], raw: "" };
    };
  ...
  it("makes one artifact source per pivot statement", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ Hunt: { HuntId: "H.X1", state: "RUNNING" } }], raw: "" };
    };
  ...
  it("strips backslashes/quotes from an event-label description (YAML/VQL safety)", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ Hunt: { HuntId: "H.X2", state: "RUNNING" } }], raw: "" };
    };
  ...
  it("normalizes clients() rows into inventory records", async () => {
    const runner: VqlRunner = async () => ({
      rows: [
        {
          client_id: "C.win11",
          os_info: { hostname: "WIN11", fqdn: "WIN11.windomain.local" },
...
describe("VelociraptorClient.collectOnClient", () => {
  it("packages the VQL as a CLIENT artifact and collect_clients the given client id", async () => {
    ...
    const res = await new VelociraptorClient(
      { ...cfg, guiUrl: "https://velo.example/" },
      runner,
    ).collectOnClient(
      "C.abc",
      "-- persistence\nSELECT Name FROM Artifact.Windows.System.Services()",
      "services on WIN11",
      "WIN11.windomain.local",
    );
    ...
    expect(prog).toContain(
      "collect_client(client_id='C.abc', artifacts=['Custom.Collect.Companion.services_on_WIN11'])",
    );
  ...
  it("reads the flow id leniently (FlowId / session_id fallbacks)", async () => {
    ...
    expect((await new VelociraptorClient(cfg, a.runner).collectOnClient("C.1", "SELECT 1", "x")).flowId).toBe(
      "F.aa",
    );
    const b = collectRunner([], { session_id: "F.bb" });
    expect((await new VelociraptorClient(cfg, b.runner).collectOnClient("C.1", "SELECT 1", "x")).flowId).toBe(
      "F.bb",
    );
  });
  ...
  it("defaults the echoed hostname to the client id when none is given", async () => {
    ...
    expect((await new VelociraptorClient(cfg, runner).collectOnClient("C.1", "SELECT 1", "x")).hostname).toBe(
      "C.1",
    );
  });
  ...
  it("rejects an invalid client id", async () => {
    const { runner } = collectRunner([]);
    await expect(
      new VelociraptorClient(cfg, runner).collectOnClient("not-an-id", "SELECT 1", "x"),
    ).rejects.toThrow(/invalid Velociraptor client id/);
  });

  it("throws when the collection flow id is missing/invalid", async () => {
    const { runner } = collectRunner([], {});
    await expect(new VelociraptorClient(cfg, runner).collectOnClient("C.1", "SELECT 1", "x")).rejects.toThrow(
      /flow id/,
    );
  });
  ...
  it("throws when the VQL is empty or only comments", async () => {
    const { runner } = collectRunner([]);
    await expect(
      new VelociraptorClient(cfg, runner).collectOnClient("C.1", "-- only a comment", "x"),
    ).rejects.toThrow(/No runnable VQL/);
  });
});
...
describe("VelociraptorClient.collectionResults", () => {
  it("reads a flow's rows via source(client_id, flow_id, artifact/source)", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ Name: "PSEXESVC", Pid: 42 }], raw: "" };
    };
    const res = await new VelociraptorClient(cfg, runner).collectionResults(
      "C.abc",
      "F.flow1",
      "Custom.Collect.Companion.x",
      ["Pivot0"],
    );
    expect(res.rows).toHaveLength(1);
    expect(program).toContain(
      "source(client_id='C.abc', flow_id='F.flow1', artifact='Custom.Collect.Companion.x/Pivot0')",
    );
  });
...
describe("VelociraptorClient.flowStatus", () => {
  it("surfaces an endpoint-side ERROR (status message) from the flow", async () => {
    ...
    const runner: VqlRunner = async (s) => {
      program = s[0];
      return {
        rows: [{ state: "ERROR", status: "handles: Unexpected arg process\n", total_collected_rows: 0 }],
        raw: "",
      };
    };
  ...
  it("reports a FINISHED flow with no error", async () => {
    const runner: VqlRunner = async () => ({
      rows: [{ state: "FINISHED", status: "", total_collected_rows: 3 }],
      raw: "",
    });
    const st = await new VelociraptorClient(cfg, runner).flowStatus("C.abc", "F.flow1");
  ...
  it("returns an empty state when the flow is not found", async () => {
    const runner: VqlRunner = async () => ({ rows: [], raw: "" });
    expect(await new VelociraptorClient(cfg, runner).flowStatus("C.abc", "F.flow1")).toEqual({
      state: "",
      error: "",
      rows: 0,
    });
...
describe("VelociraptorClient.huntStatus", () => {
  ...
  it("throws on an invalid hunt id", async () => {
    const runner: VqlRunner = async () => ({ rows: [], raw: "" });
    await expect(new VelociraptorClient(cfg, runner).huntStatus("not-a-hunt-id")).rejects.toThrow(
      /invalid hunt id/,
    );
  });
  ...
  it("converts the hunt's microsecond expires field to an ISO string", async () => {
    const runner: VqlRunner = async () => ({
      rows: [{ state: "STOPPED", expires: 1_783_586_781_000_000 }],
      raw: "",
    });
    const st = await new VelociraptorClient(cfg, runner).huntStatus("H.ABC123");
...
describe("VelociraptorClient.getHuntArtifacts", () => {
  it("returns the hunt's configured artifact list", async () => {
    ...
    const runner: VqlRunner = async (s) => {
      program = s[0];
      return { rows: [{ artifacts: ["Windows.NTFS.MFT", "Windows.Forensics.Usn"] }], raw: "" };
    };
  ...
  it("rejects an invalid hunt id", async () => {
    const runner: VqlRunner = async () => ({ rows: [], raw: "" });
    await expect(new VelociraptorClient(cfg, runner).getHuntArtifacts("not-a-hunt")).rejects.toThrow(
      /hunt id/i,
    );
  });
});
...
describe("VelociraptorClient.getFlowInfo", () => {
  ...
  it("returns the flow's artifacts (preferring artifacts_with_results) and resolves the host", async () => {
    const runner = flowRunner(
      {
        artifacts_with_results: ["Windows.NTFS.MFT"],
        req_artifacts: ["Windows.NTFS.MFT", "Windows.Forensics.Usn"],
      },
      [{ client_id: "C.dead", os_info: { hostname: "DESKTOP-01" } }],
    );
    const info = await new VelociraptorClient(cfg, runner).getFlowInfo("C.dead", "F.001");
  ...
  it("falls back to request.artifacts when nothing produced results", async () => {
    const runner = flowRunner({ artifacts_with_results: [], req_artifacts: ["Windows.NTFS.MFT"] }, [
  ...
  it("rejects an invalid client id", async () => {
    const runner: VqlRunner = async () => ({ rows: [], raw: "" });
    await expect(new VelociraptorClient(cfg, runner).getFlowInfo("not-an-id", "F.001")).rejects.toThrow(
      /client id/i,
    );
  });
  ...
  it("rejects an invalid flow id", async () => {
    const runner: VqlRunner = async () => ({ rows: [], raw: "" });
    await expect(new VelociraptorClient(cfg, runner).getFlowInfo("C.dead", "not-a-flow")).rejects.toThrow(
      /flow id/i,
    );
  });
  ...
  it("accepts a hunt-launched flow id (the .H suffix) and queries it verbatim", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      ...
      if (p.includes("FROM flows(")) {
        queried = p;
        return { rows: [{ artifacts_with_results: ["Windows.NTFS.MFT"] }], raw: "" };
      }
      return { rows: [], raw: "" };
    };
    const info = await new VelociraptorClient(cfg, runner).getFlowInfo(
      "C.9e5afcaf10536cd9",
      "F.D93M3ERL39HVE.H",
    );
    expect(info.artifacts).toEqual(["Windows.NTFS.MFT"]);
...
describe("VelociraptorClient.collectFromHost (live resolve)", () => {
  it("enumerates the fleet, matches the host (FQDN ⇄ short name), and collects on that client", async () => {
    ...
    const res = await new VelociraptorClient(cfg, runner).collectFromHost(
      "WIN11.windomain.local",
      "SELECT Name FROM Artifact.Windows.System.Services()",
      "services on WIN11",
    );
  ...
  it("throws when no client matches the host", async () => {
    const runner: VqlRunner = async () => ({ rows: [], raw: "" });
    await expect(
      new VelociraptorClient(cfg, runner).collectFromHost("GHOST", "SELECT 1", "x"),
  ...
  it("throws when the VQL is empty or only comments (before resolving)", async () => {
    const runner: VqlRunner = async () => ({ rows: [], raw: "" });
    await expect(
      new VelociraptorClient(cfg, runner).collectFromHost("h", "-- only a comment", "x"),
    ).rejects.toThrow(/No runnable VQL/);
  });
});
...
describe("normalizeClientRow", () => {
  it("reads client_id + os_info casing-tolerantly", () => {
    expect(
      normalizeClientRow({
        client_id: "C.a",
        os_info: { hostname: "h", fqdn: "h.d.local" },
        last_seen_at: "t",
      }),
    ).toEqual({ clientId: "C.a", hostname: "h", fqdn: "h.d.local", lastSeen: "t" });
    expect(normalizeClientRow({ ClientId: "C.b", OsInfo: { Hostname: "H", Fqdn: "H.D" } })).toEqual({
      clientId: "C.b",
      hostname: "H",
      fqdn: "H.D",
    });
...
describe("matchClient", () => {
  ...
  it("matches an FQDN-enrolled client against a short-name target", () => {
    expect(
      matchClient([{ clientId: "C.x", hostname: "", fqdn: "win11.windomain.local" }], "WIN11")?.clientId,
    ).toBe("C.x");
  });
...
describe("VelociraptorClient.huntResults", () => {
  it("reads a single-source hunt's results via artifact/source notation", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ Name: "evil.exe", ClientId: "C.1" }], raw: "" };
    };
    const res = await new VelociraptorClient(cfg, runner).huntResults("H.ABC123", "Custom.Hunt.Companion.x", [
      "Pivot0",
    ]);
    expect(program).toBe(
      "SELECT * FROM hunt_results(hunt_id='H.ABC123', artifact='Custom.Hunt.Companion.x/Pivot0') LIMIT 4",
    );
    expect(res.rows).toHaveLength(1);
  });
  ...
  it("injects an analyst WHERE filter BEFORE the LIMIT (so it drops noise at the source)", async () => {
    ...
    await new VelociraptorClient(cfg, runner).huntResults(
      "H.ABC123",
      "DetectRaptor.X",
      [],
      "NOT OSPath =~ 'pagefile'",
    ...
    expect(program).toBe(
      "SELECT * FROM hunt_results(hunt_id='H.ABC123', artifact='DetectRaptor.X') WHERE (NOT OSPath =~ 'pagefile') LIMIT 4",
    );
  });
  ...
  it("chains hunt_results across multiple sources (artifact/source refs)", async () => {
    ...
    await new VelociraptorClient(cfg, runner).huntResults("H.ABC123", "Custom.Hunt.Companion.x", [
      "Pivot0",
      "Pivot1",
    ]);
    expect(program).toContain("chain(");
...
describe("VelociraptorClient.listClientArtifacts", () => {
  ...
  it("filters the type in TS — version-tolerant of casing/spacing (CLIENT_EVENT / Client Event / client-event)", async () => {
    ...
    expect((await client.listClientArtifacts("client_event")).map((a) => a.name)).toEqual([
      "Windows.Events.ProcessCreation",
      "Windows.Events.DNSQueries",
      "Windows.Events.ServiceCreation",
    ]);
    expect((await client.listClientArtifacts("client")).map((a) => a.name)).toEqual([
...
describe("parseArtifactParams", () => {
  it("keeps only the valid entries, in order, from a mixed-validity array", () => {
    ...
    expect(out).toEqual([{ name: "DateAfter", type: "timestamp" }, { name: "EvtxGlob" }]);
...
describe("VelociraptorClient.launchArtifactHunt", () => {
  it("launches a hunt over the chosen artifacts and returns the hunt id + gui link", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ Hunt: { HuntId: "H.B1", state: "RUNNING" } }], raw: "" };
    };
    const res = await new VelociraptorClient(
      { ...cfg, guiUrl: "https://velo.example/" },
      runner,
  ...
  it("uses a supplied relative expiry (seconds)", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ Hunt: { HuntId: "H.BEXP", state: "RUNNING" } }], raw: "" };
    };
  ...
  it("adds include/exclude label + OS clauses and sanitizes label values", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ Hunt: { HuntId: "H.B2", state: "RUNNING" } }], raw: "" };
    };
    await new VelociraptorClient(cfg, runner).launchArtifactHunt(["Windows.System.Pslist"], "x", {
      includeLabels: ["workstations"],
      excludeLabels: ["servers", "bad'; DROP"],
      os: "windows",
    });
  ...
  it("ignores an unknown os and omits empty label clauses", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ Hunt: { HuntId: "H.B4", state: "RUNNING" } }], raw: "" };
    };
    await new VelociraptorClient(cfg, runner).launchArtifactHunt(["Windows.System.Pslist"], "x", {
      os: "solaris" as never,
    });
  ...
  it("puts ?org_id before the # fragment and honors a custom org", async () => {
    const runner: VqlRunner = async () => ({
      rows: [{ Hunt: { HuntId: "H.ORG1", state: "RUNNING" } }],
      raw: "",
    });
    const res = await new VelociraptorClient(
      { ...cfg, guiUrl: "https://velo:5888", guiOrg: "OACME" },
      runner,
  ...
  it("adds a collection timeout clause when timeoutSeconds is set, omits it otherwise", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ Hunt: { HuntId: "H.B5", state: "RUNNING" } }], raw: "" };
    };
    const c = new VelociraptorClient(cfg, runner);
  ...
  it("passes per-artifact parameters as a hunt spec (only for artifacts in the hunt)", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ Hunt: { HuntId: "H.SP1", state: "RUNNING" } }], raw: "" };
    };
    await new VelociraptorClient(cfg, runner).launchArtifactHunt(
      ["Windows.Hayabusa.Rules", "Windows.System.Pslist"],
      "x",
      {},
      {
        params: {
          "Windows.Hayabusa.Rules": { RuleLevel: "Critical, High, and Medium" },
          "Not.In.Hunt": { X: "y" },
        },
      },
    );
  ...
  it("omits the spec clause when there are no params", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ Hunt: { HuntId: "H.SP2", state: "RUNNING" } }], raw: "" };
    };
    await new VelociraptorClient(cfg, runner).launchArtifactHunt(["Windows.System.Pslist"], "x");
...
describe("VelociraptorClient.huntResultsByArtifact", () => {
  it("builds an artifact-map keyed by artifact, dropping artifacts with no rows yet", async () => {
    ...
    const { results, skipped } = await new VelociraptorClient(cfg, runner).huntResultsByArtifact("H.ABC123", [
      "Windows.System.Pslist",
  ...
  it("rejects a malformed hunt id and skips invalid artifact names", async () => {
    ...
    const c = new VelociraptorClient(cfg, runner);
    await expect(c.huntResultsByArtifact("bad id", ["Windows.System.Pslist"])).rejects.toThrow(
      /invalid hunt id/,
    );
  ...
  it("reads a fleet-hunt artifact's NAMED sources as artifact/source (#157 — else 0 rows / false 'no evidence')", async () => {
    ...
    const { results } = await new VelociraptorClient(cfg, runner).huntResultsByArtifact(
      "H.OK1",
      ["Custom.Hunt.Companion.x"],
      undefined,
      { "Custom.Hunt.Companion.x": ["Pivot0"] },
    );
    expect(program).toContain("artifact='Custom.Hunt.Companion.x/Pivot0'");
  ...
  it("does NOT retry a query failure (no spawnCode)", async () => {
    ...
    await expect(
      retryTransientSpawn(
        async () => {
          calls++;
          throw new Error("velociraptor exited with code 1");
        },
        { sleep: noSleep },
      ),
...
describe("translateVelociraptorError", () => {
  it("points a gRPC message-size failure at the CLIENT-side api_client.yaml's max_grpc_recv_size field, not a CLI flag or the server config", () => {
    const m = translateVelociraptorError(
      "velociraptor-v0.76.5-windows-amd64.exe: error: query: rpc error: code = ResourceExhausted desc = grpc: received message larger than max (18006256 vs. 4194304)",
    );
  ...
  it("passes through any other stderr unchanged", () => {
    expect(translateVelociraptorError("unknown long flag '--max_message_size', try --help")).toBe(
      "unknown long flag '--max_message_size', try --help",
    );
...
describe("VelociraptorClient.huntUploads", () => {
  ...
  it("uses the configured override VQL (DFIR_VELOCIRAPTOR_UPLOAD_VQL) when set", async () => {
    ...
    await new VelociraptorClient(
      { ...cfg, uploadVql: "SELECT * FROM custom(hunt='__HUNT_ID__')" },
      runner,
    ).huntUploads("H.UP2");
  ...
  it("rejects a malformed hunt id (no VQL-string injection)", async () => {
    const runner: VqlRunner = async () => ({ rows: [], raw: "" });
    await expect(new VelociraptorClient(cfg, runner).huntUploads("bad id")).rejects.toThrow(
      /invalid hunt id/,
    );
  });
...
describe("VelociraptorClient.flowUploads", () => {
  ...
  it("rejects a malformed client id or flow id (no VQL-string injection)", async () => {
    const runner: VqlRunner = async () => ({ rows: [], raw: "" });
    await expect(new VelociraptorClient(cfg, runner).flowUploads("bad id", "F.CFF001")).rejects.toThrow(
      /invalid client id/,
    );
    await expect(new VelociraptorClient(cfg, runner).flowUploads("C.dead", "bad id")).rejects.toThrow(
      /invalid flow id/,
    );
  });
});
...
describe("loadVelociraptorConfig / buildVelociraptorClient", () => {
  ...
  it("honors binary/timeout/row overrides", () => {
    ...
    expect(buildVelociraptorClient({ DFIR_VELOCIRAPTOR_API_CONFIG: "/x/api.yaml" })).toBeInstanceOf(
      VelociraptorClient,
    );
  });
});
...
describe("VelociraptorClient.monitorResults", () => {
  it("reads ONE client's monitoring set via source() with the window", async () => {
    ...
    const runner: VqlRunner = async (statements) => {
      program = statements[0];
      return { rows: [{ _ts: 1500 }], raw: "" };
    };
    const res = await new VelociraptorClient(cfg, runner).monitorResults(
      "C.abc",
      "Windows.Events.ProcessCreation",
      1000,
      2000,
  ...
  it("rejects an invalid (non-sentinel) client id", async () => {
    const runner: VqlRunner = async () => ({ rows: [], raw: "" });
    await expect(new VelociraptorClient(cfg, runner).monitorResults("bad", "A.B", 0, 1)).rejects.toThrow(
      /invalid client id/,
    );
  });
  ...
  it("honors the DFIR_VELOCIRAPTOR_MONITOR_ALL_VQL override", async () => {
    ...
    await new VelociraptorClient(
      { ...cfg, monitorAllVql: "CUSTOM __ARTIFACT__ __START__ __END__ __LIMIT__" },
      runner,
...
describe("VelociraptorClient.listMonitoredArtifacts", () => {
  ...
  it("tolerates bare-string and Name-keyed rows", async () => {
    const runner: VqlRunner = async () => ({
      rows: ["Windows.Events.DNSQueries", { Name: "Generic.Client.Stats" }],
      raw: "",
    });
    const out = await new VelociraptorClient(cfg, runner).listMonitoredArtifacts();
...
describe("extractMonitoredArtifacts", () => {
  ...
  it("tolerates PascalCase proto fields", () => {
    expect(
      extractMonitoredArtifacts([
        { State: { Artifacts: { Artifacts: ["A.B"], Specs: [{ Artifact: "C.D" }] } } },
      ]),
    ).toEqual(["A.B", "C.D"]);
  });
  it("handles override shapes (bare strings / { artifact })", () => {
    expect(extractMonitoredArtifacts(["A.B", { artifact: "C.D" }, { Name: "E.F" }])).toEqual([
      "A.B",
      "C.D",
      "E.F",
    ]);
...
describe("VelociraptorClient.huntArtifactRows", () => {
  ...
  it("degrades to the empty result when the catalog lookup fails", async () => {
    ...
    expect(res.sourcesUnknown).toBe(true); // #1635 — but the empty is "not read", never "empty"
  });

  // #1635 — TaskScheduler keeps its rows only under /Analysis. With the catalog down, its bare read is
  // empty whether or not the host has tasks; the collect must be told the named sources went unread.
  it("says the named sources went unread when the catalog fails, whatever the bare read returned", async () => {
    const runner =
      (bare: unknown[]): VqlRunner =>
      async (statements) => {
        if (statements[0].includes("FROM artifact_definitions(")) throw new Error("server down");
        return { rows: bare, raw: "" };
      };
    for (const bare of [[], [{ a: 1 }]]) {
      const res = await new VelociraptorClient(cfg, runner(bare)).huntArtifactRows(
        "H.1",
        "Windows.System.TaskScheduler",
      );
      expect(res.rows).toEqual(bare);
      expect(res.sourcesUnknown).toBe(true);
    }
  });

  it("treats an artifact missing from a successful catalog as sources unknown", async () => {
    const runner: VqlRunner = async (statements) => {
      if (statements[0].includes("FROM artifact_definitions("))
        return { rows: [{ name: "Other.Artifact", type: "CLIENT", sources: [{}] }], raw: "" };
      return { rows: [], raw: "" };
    };
    const res = await new VelociraptorClient(cfg, runner).huntArtifactRows(
      "H.1",
  ...
  it("does not flag a complete read: catalog entry found, or an explicit source list", async () => {
    const runner: VqlRunner = async (statements) => {
      if (statements[0].includes("FROM artifact_definitions("))
        return { rows: [{ name: "Windows.Sys.StartupItems", type: "CLIENT", sources: [{}] }], raw: "" };
      return { rows: [], raw: "" };
    ...
    const client = new VelociraptorClient(cfg, runner);
    expect((await client.huntArtifactRows("H.1", "Windows.Sys.StartupItems")).sourcesUnknown).toBeUndefined();
    expect((await client.huntArtifactRows("H.1", "Custom.Hunt", ["Pivot0"])).sourcesUnknown).toBeUndefined();
  });
