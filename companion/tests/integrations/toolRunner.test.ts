...
describe("loadToolConfig / loadAllToolConfigs", () => {
  ...
  it("respects the master and per-tool auto-run kill switches", () => {
    expect(loadToolConfig("snort", { DFIR_TOOL_SNORT_BINARY: "snort", DFIR_TOOL_AUTO_RUN: "false" })!.autoRun).toBe(false);
    expect(loadToolConfig("snort", { DFIR_TOOL_SNORT_BINARY: "snort", DFIR_TOOL_SNORT_AUTO_RUN: "false" })!.autoRun).toBe(false);
  });
  it("uses the standalone default update command for suricata", () => {
    expect(loadToolConfig("suricata", { DFIR_TOOL_SURICATA_BINARY: "suricata" })!.updateCommand).toBe("suricata-update");
  });
