...
const resultLine = (obj: Record<string, unknown>) =>
  JSON.stringify({ type: "result", subtype: "success", is_error: false, ...obj });
...
const assistantLine = (content: Record<string, unknown>[], stopReason?: string) =>
  JSON.stringify({
    type: "assistant",
    message: { role: "assistant", content, stop_reason: stopReason ?? null },
  });
...
describe("ClaudeCodeProvider", () => {
  it("builds the isolated stream-json invocation and encodes text + image blocks", async () => {
    ...
    const runner = fakeRunner({ stdout: resultLine({ result: '{"summary":"ok"}' }) }, (o) => {
      captured = o;
    });
    ...
    expect(a).toEqual(
      expect.arrayContaining(["--strict-mcp-config", "--setting-sources", "", "--allowed-tools", ""]),
    );
    // stdin message
    ...
    expect(msg.message.content[1]).toEqual({
      type: "image",
      source: { type: "base64", media_type: "image/webp", data: "AAA" },
    });
  });
  ...
  it("omits --model when the model is empty", async () => {
    ...
    await new ClaudeCodeProvider({ model: "", runner }).analyze({
      systemPrompt: "s",
      userPrompt: "u",
      images: [],
    });
  ...
  it("maps usage and total_cost_usd from the result event", async () => {
    ...
    const out = await new ClaudeCodeProvider({ model: "haiku", runner }).analyze({
      systemPrompt: "s",
      userPrompt: "u",
      images: [],
    });
      ...
      stdout: resultLine({
        ...
        modelUsage: {
          ...
          "claude-sonnet-5": {
            inputTokens: 2,
            outputTokens: 30874,
            cacheReadInputTokens: 12635,
            cacheCreationInputTokens: 7154,
          },
        },
      }),
    });
    const out = await new ClaudeCodeProvider({ model: "claude-sonnet-5", runner }).analyze({
      systemPrompt: "s",
      userPrompt: "u",
      images: [],
    });
  ...
  it("throws an actionable error when the CLI binary is missing", async () => {
    ...
    const p = new ClaudeCodeProvider({ model: "haiku", runner });
    await expect(p.analyze({ systemPrompt: "s", userPrompt: "u", images: [] })).rejects.toThrow(
      /not found.*claude auth login/i,
    );
  });
