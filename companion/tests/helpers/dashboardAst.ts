...
function invocationEdges(n: ts.Node): string[] {
  // `el.onclick = handler` registers it; the browser calls it later. This is the one edge that is
  // not a call expression, which is why it is here rather than in invokedNames().
  if (
    ts.isBinaryExpression(n) &&
    n.operatorToken.kind === ts.SyntaxKind.EqualsToken &&
    ts.isPropertyAccessExpression(n.left) &&
  ...
  // Every other edge is an invocation, and "what does this call invoke" has exactly ONE answer in
  // this file (#476/#477 landed two copies of it; keeping both is how the two gates drift into
  // recognising different spellings while each claims to recognise all of them).
  return ts.isCallExpression(n) ? invokedNames(n) : [];
}
