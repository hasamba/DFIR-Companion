...
import {
  ...
  type IocKind,
} from "../../src/enrichment/provider.js";
...
function downProvider(
  name: string,
  kinds: IocKind[],
  counts: { probes: number; lookups: number },
): EnrichmentProvider {
  return {
    name,
    scope: "local",
...
describe("enrichIocs", () => {
  it("enriches enrichable IOCs (hash/ip/domain/url/process) and skips file/other", async () => {
    const calls: string[] = [];
    const vt = fakeProvider(
      "VirusTotal",
      ["hash", "ip", "domain", "url"],
      { source: "VirusTotal", verdict: "malicious", score: "9/70" },
      calls,
    );
    const iocs = [
  ...
  it("re-checks an IOC only on providers that haven't checked it (a newly-enabled provider re-checks all)", async () => {
    ...
    const r1 = await enrichIocs([ioc({ value: "h1", type: "hash" })], {
      providers: [vt],
      sleep: noSleep,
      now,
    });
  ...
  it("routes a process IOC only to providers that support 'process'", async () => {
    ...
    const rr = fakeProvider(
      "RockyRaccoon",
      ["process"],
      { source: "RockyRaccoon", verdict: "suspicious", score: "LOLBIN" },
      calls,
    );
    const { iocs: out } = await enrichIocs([ioc({ value: "powershell.exe", type: "process" })], {
      providers: [vt, rr],
      sleep: noSleep,
      now,
    });
  ...
  it("routes each IOC only to providers that support its kind", async () => {
    ...
    const vt = fakeProvider(
      "VirusTotal",
      ["hash", "ip", "domain", "url"],
      { source: "VirusTotal", verdict: "harmless" },
      calls,
    );
    const mb = fakeProvider(
      "MalwareBazaar",
    ...
    expect(calls).toEqual(
      expect.arrayContaining([
        "VirusTotal:hash:h1",
        "MalwareBazaar:hash:h1",
        "VirusTotal:ip:2.2.2.2",
        "AbuseIPDB:ip:2.2.2.2",
      ]),
    );
    expect(calls).not.toContain("MalwareBazaar:ip:2.2.2.2");
    expect(out.find((i) => i.value === "h1")!.enrichments!.map((e) => e.source)).toEqual([
      "VirusTotal",
      "MalwareBazaar",
    ]);
  });
  ...
  it("honours the maxIocs cap (most-valuable kinds first: hash before url)", async () => {
    ...
    const { iocs: out, summary } = await enrichIocs(iocs, {
      providers: [vt],
      sleep: noSleep,
      now,
      maxIocs: 1,
  ...
  it("retries an errored provider on the next run (failure is not cached), and keeps a prior hit through a later error", async () => {
    ...
    const flaky: EnrichmentProvider = {
      ...
      lookup: async (k, v) => {
        calls.push(`${k}:${v}`);
        if (mode === "error") throw new Error("temporarily down");
  ...
  describe("perProviderDelayMs", () => {
    it("sleeps the provider-specific delay between calls to the same provider", async () => {
      ...
      await enrichIocs([ioc({ value: "h1", type: "hash" }), ioc({ value: "h2", type: "hash" })], {
        providers: [vt],
        now,
        monotonic: () => 0,
        sleep: async (ms) => {
        ...
        perProviderDelayMs: { VirusTotal: 500 },
        delayMs: 1500,
      });
    ...
    it("throttles each provider independently — a slow provider does not block a fast one", async () => {
      ...
      await enrichIocs(
        [
          ioc({ value: "h1", type: "hash" }),
          ioc({ value: "p1", type: "process" }),
          ioc({ value: "h2", type: "hash" }),
          ioc({ value: "p2", type: "process" }),
        ],
        {
          providers: [slow, fast],
          now,
          ...
          perProviderDelayMs: { Slow: 3000, Fast: 100 },
          delayMs: 1500,
        },
      );
    ...
    it("skips the sleep when elapsed time already exceeds the provider delay", async () => {
      ...
      const vtFast: EnrichmentProvider = {
        ...
        lookup: async () => {
          t += 600;
          return { source: "VirusTotal", verdict: "harmless" };
        },
      };
      await enrichIocs([ioc({ value: "h1", type: "hash" }), ioc({ value: "h2", type: "hash" })], {
        providers: [vtFast],
  ...
  describe("429 retry", () => {
    ...
    it("honours Retry-After (via the sleep it drives) before retrying", async () => {
      ...
      const limited: EnrichmentProvider = {
        ...
        lookup: async () => {
          attempts++;
          if (attempts === 1) throw new RateLimitError("rate limited", 7000);
          return { source: "Limited", verdict: "harmless" };
        },
      };
      await enrichIocs([ioc({ value: "h1", type: "hash" })], {
        providers: [limited],
