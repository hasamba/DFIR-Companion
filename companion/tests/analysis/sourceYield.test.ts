...
import {
  classifyImportYield,
  ZERO_YIELD_MIN_LINES_DEFAULT,
  type ImportMeta,
} from "../../src/analysis/importMeta.js";
...
describe("classifyImportYield (trigger a — zero-yield AI import)", () => {
  it("flags a large AI-triage import that produced zero events (the northpeak proxy-log case)", () => {
    const w = classifyImportYield(
      meta({ lastImportFile: "proxy_access.log", linesIn: 27290, path: "ai", addedCount: 0 }),
    );
  ...
  it("does NOT flag a tiny AI import below the line threshold", () => {
    expect(
      classifyImportYield(meta({ path: "ai", linesIn: ZERO_YIELD_MIN_LINES_DEFAULT - 1, addedCount: 0 })),
    ).toBeNull();
  });
});
...
describe("classifyImportYield (trigger b — cap-hit template truncation, #10 deferred)", () => {
  it("flags an import whose log-aggregation cap dropped distinct patterns, even when events WERE produced", () => {
    const w = classifyImportYield(
      meta({
        lastImportFile: "huge.log",
        path: "ai",
        addedCount: 120,
        linesIn: 90000,
        truncation: { distinctTemplates: 950, keptTemplates: 400 },
      }),
    );
    expect(w).not.toBeNull();
  ...
  it("does NOT flag when nothing was truncated (kept >= distinct)", () => {
    expect(
      classifyImportYield(
        meta({ path: "ai", addedCount: 50, truncation: { distinctTemplates: 120, keptTemplates: 400 } }),
      ),
    ).toBeNull();
...
describe("networkTelemetryWithoutDetector (trigger c)", () => {
  const serious = (): InvestigationState => {
    const s = emptyState("c");
    s.findings = [finding("Critical")];
    return s;
  };
  ...
  it("returns null for a non-serious case (no Critical/High finding)", () => {
    const s = emptyState("c");
    s.forensicTimeline = [ev({ id: "e1", sources: ["proxy"] })];
    expect(networkTelemetryWithoutDetector(s)).toBeNull();
  });
});
...
describe("buildKnownUnknownItems — yield_gap items (#10)", () => {
  it("emits a yield_gap from a supplied zero-yield warning", () => {
    const warning = classifyImportYield(
      meta({ lastImportFile: "proxy.log", linesIn: 10000, path: "ai", addedCount: 0 }),
    )!;
