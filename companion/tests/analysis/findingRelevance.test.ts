...
import {
  scoreFindingsRelevance,
  relevanceBucket,
  type AiRelevance,
} from "../../src/analysis/findingRelevance.js";
import {
  emptyState,
  type Finding,
  type ForensicEvent,
...
describe("scoreFindingsRelevance (#13)", () => {
  ...
  it("marks a finding on the main path 'connected' (connectedness 1)", () => {
    const [f] = scoreFindingsRelevance({
      findings: [finding({ id: "fA", relatedEventIds: ["e1"] })],
  ...
  it("marks a finding whose evidence sits in a SEPARATE component 'disconnected' with a discriminator", () => {
    const [f] = scoreFindingsRelevance({
      findings: [finding({ id: "fB", relatedEventIds: ["e3"] })],
      scopedEvents: state.forensicTimeline,
      graph,
    });
  ...
  it("marks a finding whose evidence is NOT in the causal graph 'undetermined' (never a false rabbit hole)", () => {
    const [f] = scoreFindingsRelevance({
      findings: [finding({ id: "fC", relatedEventIds: ["e9"] })],
      scopedEvents: state.forensicTimeline,
      graph,
    });
  ...
  it("lets the AI refine a disconnected finding into 'unrelated-but-real' (Parked), but never a rabbit hole into a lead", () => {
    const ai = new Map<string, AiRelevance>([
      ["fB", "unrelated-but-real"],
      ["fA", "unrelated-but-real"],
    ]);
    const scored = scoreFindingsRelevance({
      findings: [
        finding({ id: "fA", relatedEventIds: ["e1"] }),
        finding({ id: "fB", relatedEventIds: ["e3"] }),
      ],
      scopedEvents: state.forensicTimeline,
      graph,
      aiRelevanceById: ai,
  ...
  it("resolves reverse finding links (event → finding) as evidence too", () => {
    ...
    const [f] = scoreFindingsRelevance({
      findings: [finding({ id: "fRev" })],
      scopedEvents: s2.forensicTimeline,
      graph: buildEvidenceGraph(s2),
    });
    expect(f.relevance).toBe("connected");
  });
});
