...
const VALID_RESPONSE = {
  ...
  attackMapping:
    "T1059.001: Command and Scripting Interpreter: PowerShell — attacker uses encoded PS to download stager.",
  pivotQueries: [
    {
      platform: "velociraptor",
      query: "SELECT * FROM Windows.EventLogs.System WHERE EventId=4688",
      rationale: "Confirm process creation",
    },
  ],
...
describe("explainEvent()", () => {
  ...
  it("puts the focal event prominently in the prompt", async () => {
    const { pipeline, provider } = await makePipeline([
      ev({
        id: "e1",
        description: "suspiciousProcess.exe ran",
        severity: "Critical",
        processName: "suspiciousProcess.exe",
        asset: "HOST-A",
      }),
    ]);
  ...
  it("includes context events (nearby + same asset) in the prompt", async () => {
    const { pipeline, provider } = await makePipeline([
      ev({
        id: "ctx1",
        description: "earlier event on same host",
        severity: "Info",
        timestamp: "2026-01-01T00:00:00Z",
        asset: "SERVER-X",
      }),
      ...
      ev({
        id: "ctx2",
        description: "later event on same host",
        severity: "Info",
  ...
  it("PROMOTES a super-only event, then explains it from the forensic timeline (#384)", async () => {
    ...
    await superTimelineStore.append("c1", [
      ev({
        ...
        asset: "TRIAGE-HOST",
        path: "C:\\Users\\x\\evil.exe",
      }),
      ev({
        id: "super2",
        description: "amcache: EVIL.EXE first seen",
        severity: "Info",
        asset: "TRIAGE-HOST",
  ...
  it("throws when the event id does not exist", async () => {
    const { pipeline } = await makePipeline([ev({ id: "e1", description: "some event", severity: "Low" })]);
