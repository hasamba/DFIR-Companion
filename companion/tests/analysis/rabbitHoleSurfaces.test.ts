...
function marker(
  partial: Partial<FalsePositiveMarker> & {
    kind: FalsePositiveMarker["kind"];
    ref: string;
    reason: FalsePositiveMarker["reason"];
  },
): FalsePositiveMarker {
  return {
    id: `${partial.kind}:${partial.ref}`,
    note: "",
...
describe("buildAuthorizedContextBlock (#13 FP-context retention)", () => {
  it("retains authorized-test / known-good-tool markers as shaping context, not erasure", () => {
    const markers = [
      marker({
        kind: "finding",
        ref: "Nessus scan burst",
        reason: "authorized-test",
        note: "quarterly pentest",
      }),
...
describe("derivePlaybookTasks rabbit-hole down-weight (#13)", () => {
  ...
  it("demotes a disconnected High finding one notch and reframes it as verify-before-chasing", () => {
    const state: InvestigationState = {
      ...emptyState("c1"),
      findings: [highFinding("f1"), highFinding("f2", "disconnected")],
    };
