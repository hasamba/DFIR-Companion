...
describe("detectImportKind — JSON formats", () => {
    ...
    expect(
      detectImportKind(
        ...
        ndjson({
          ...
          object: "FLOW",
          action: "CONNECT",
          properties: { src_ip: "1.2.3.4", dst_ip: "10.44.30.10" },
        }),
      ),
    ).toBe("ecar");
    expect(
  ...
  it("sandbox: Falcon summary", () => {
    expect(detectImportKind("summary.json", j({ verdict: "malicious", threat_score: 90, sha256: "x" }))).toBe(
      "sandbox",
    );
  });
  it("aws: CloudTrail { Records: [...] }", () => {
    expect(
      detectImportKind(
        "ct.json",
        j({ Records: [{ eventName: "RunInstances", eventSource: "ec2.amazonaws.com" }] }),
      ),
    ).toBe("aws");
  });
  it("cloud: GCP protoPayload", () => {
    expect(
      detectImportKind(
        "gcp.json",
        j([{ protoPayload: { methodName: "x" }, logName: "projects/p/logs/cloudaudit.googleapis.com" }]),
      ),
    ).toBe("cloud");
  });
  ...
  it("m365: Unified Audit Log JSON", () => {
    expect(
      detectImportKind(
        "ual.json",
        j([{ Operation: "New-InboxRule", Workload: "Exchange", AuditData: "{}" }]),
      ),
    ).toBe("m365");
  });
  it("m365: Entra sign-in", () => {
    expect(
      detectImportKind(
        "signin.json",
        j([{ userPrincipalName: "u@x.com", status: { errorCode: 0 }, appDisplayName: "X" }]),
      ),
    ).toBe("m365");
  });
  ...
  it("chainsaw: hunt output with document+rule", () => {
    expect(
      detectImportKind(
        "cs.json",
        j([{ group: "Sigma", document: { data: { Event: { System: {} } } }, rule: { name: "x" } }]),
      ),
    ).toBe("chainsaw");
  });
  it("chainsaw: raw evtx_dump { Event: { System } }", () => {
    expect(detectImportKind("evtx.json", ndjson({ Event: { System: { EventID: 1 }, EventData: {} } }))).toBe(
      "chainsaw",
    );
  });
  it("chainsaw: flat Sigma-mapping JSON (Velociraptor-shelled-out shape)", () => {
    expect(
      detectImportKind(
        "cs_flat.json",
        ndjson({
          EventTime: "2025-12-05T02:43:41Z",
          Detection: "Uncommon New Firewall Rule Added",
          Severity: "medium",
          "Rule Group": "Sigma",
          Computer: "WIN-01",
          Channel: "Microsoft-Windows-Windows Firewall With Advanced Security/Firewall",
          EventID: 2097,
          SystemData: { EventID: 2097 },
          EventData: { RuleName: "x" },
          Authors: ["frack113"],
        }),
      ),
    ).toBe("chainsaw");
  ...
  it("velociraptor: _Source-tagged rows", () => {
    expect(
      detectImportKind(
        "vr.json",
        ndjson({ _Source: "Windows.EventLogs.Evtx", System: { EventID: 1 }, EventData: {} }),
      ),
    ).toBe("velociraptor");
  });
  it("velociraptor: artifact map", () => {
    expect(detectImportKind("vr.json", j({ "Windows.Detection.Yara.Glob": [{ Rule: "x" }] }))).toBe(
      "velociraptor",
    );
  });
  it("velociraptor: Elastic-indexed push (artifact_* index / flattened Detection.* keys)", () => {
    ...
    expect(
      detectImportKind(
        "e.json",
        j([{ _index: "logs-foo", "Detection.StringHit": "x", "Artifact.keyword": "Y" }]),
      ),
    ).toBe("velociraptor");
  });
  it("NOT velociraptor: a non-artifact Elastic index (MemProcFS mp_timeline) stays SIEM", () => {
    expect(
      detectImportKind(
        "mp.json",
        j([{ _index: "mp_timeline", "@timestamp": "t", desc: "x", action: "MOD", type: "REG" }]),
      ),
    ).toBe("siem");
  });
  ...
  it("securityonion: raw SOC API export (SO data-stream _index, no _Source stamp)", () => {
    expect(
      detectImportKind(
        ...
        j([
          {
            _index: ".ds-logs-suricata-so-2026.06.19-000001",
            "@timestamp": "t",
            "event.module": "suricata",
            "rule.name": "ET SCAN x",
          },
  ...
  it("NOT securityonion: a generic Elastic alert without SO markers stays SIEM", () => {
    expect(
      detectImportKind(
        "e.json",
        j([{ _index: "logs-generic", "@timestamp": "t", "rule.name": "x", message: "y" }]),
      ),
    ).toBe("siem");
  });
  ...
  it("socrates: extension push stamped _Source SO-CRATES (not velociraptor)", () => {
    expect(
      detectImportKind(
        "socrates-1.json",
        j([{ _Source: "SO-CRATES", event_type: "alert", alert: { signature: "ET x" } }]),
      ),
    ).toBe("socrates");
  });
  it("socrates: YARA filealerts record", () => {
    expect(
      detectImportKind(
        "fa.json",
        j([{ event_type: "filealerts", filealerts: { rule_name: "X", sha256: "a" } }]),
      ),
    ).toBe("socrates");
  });
  it("socrates: Sigma alert record", () => {
    expect(
      detectImportKind(
        "sig.json",
        j([{ rule_title: "Suspicious PowerShell", rule_id: "r1", level: "high" }]),
      ),
    ).toBe("socrates");
  });
  it("network: a plain Suricata eve.json alert (no SO-CRATES markers) stays network", () => {
    expect(
      detectImportKind(
        "eve.json",
        j([{ event_type: "alert", src_ip: "1.2.3.4", alert: { signature: "ET x", severity: 1 } }]),
      ),
    ).toBe("network");
  });
  ...
  it("hayabusa: json-timeline", () => {
    expect(detectImportKind("hb.json", j([{ Timestamp: "t", RuleTitle: "x", Level: "high" }]))).toBe(
      "hayabusa",
    );
  });
  ...
  it("velociraptor: artifact-named export with no content signature → filename hint (not 'siem')", () => {
    ...
    const mem = {
      ...
      Pid: 2432,
      FullPath: "x.dmp",
      CrashDump: { sha256: "a".repeat(64) },
    };
    expect(detectImportKind("Velociraptor-Windows.Triage.HighValueMemory-sample.json", ndjson(mem))).toBe(
      "velociraptor",
    );
  ...
  it("network: Suricata eve.json", () => {
    expect(detectImportKind("eve.json", ndjson({ event_type: "alert", alert: { signature: "x" } }))).toBe(
      "network",
    );
  });
  ...
  it("network: Zeek PER-STREAM json with no _path (conn/dns/files/x509) — #197", () => {
    ...
    expect(
      detectImportKind(
        ...
        ndjson({
          ts: 1710770412.4,
          uid: "C2",
          "id.orig_h": "10.10.20.30",
          query: "cdn.onenote.net",
          qtype_name: "AAAA",
        }),
      ),
    ).toBe("network");
    expect(
      detectImportKind(
        "files.json",
        ndjson({ ts: 1710770420.1, fuid: "F1", mime_type: "application/x-dosexec", sha256: "a".repeat(64) }),
      ),
    ).toBe("network");
    expect(
      detectImportKind(
  ...
  it("thor: JSON-Lines findings", () => {
    expect(
      detectImportKind(
        "thor.json",
        ndjson({ time: "t", module: "Filescan", level: "Warning", message: "x" }),
      ),
    ).toBe("thor");
  });
  it("siem: Windows event JSON (catch-all)", () => {
    expect(
      detectImportKind("win.json", j({ data: [{ event_id: 4624, log_name: "Security", event_data: {} }] })),
    ).toBe("siem");
  });
        ...
        j([
          {
            epoch_timestamp: 1769593923,
            message: "/x",
            scoreDescription: "Yara pattern detected",
  ...
  it("memory: Volatility 3 pslist array (__children + ImageFileName)", () => {
    expect(
      detectImportKind(
        "pslist.json",
        j([{ __children: [], PID: 4, PPID: 0, ImageFileName: "System", CreateTime: "t" }]),
      ),
    ).toBe("memory");
  });
  ...
  it("memory: Volatility 3 malfind (Protection + Tag)", () => {
    expect(
      detectImportKind(
        "malfind.json",
        j([{ __children: [], PID: 4, Process: "x.exe", Protection: "PAGE_EXECUTE_READWRITE", Tag: "VadS" }]),
      ),
    ).toBe("memory");
  });
  it("memory: combined { plugin: rows } map — claimed ahead of the Velociraptor artifact-map", () => {
    expect(
      detectImportKind("vol.json", j({ "windows.pslist.PsList": [{ PID: 4, ImageFileName: "System" }] })),
    ).toBe("memory");
  });
    ...
    expect(
      detectImportKind(
        ...
        j([
          ...
          ["t", [], {}],
          ["r", { _EPROCESS: { name: "System" }, ppid: 0 }],
        ]),
      ),
    ).toBe("memory");
  });
...
describe("detectImportKind — CSV formats", () => {
  it("kape: Prefetch (PECmd) header", () => {
    expect(
      detectImportKind(
        "pf.csv",
        "SourceFilename,ExecutableName,RunCount,LastRun\nx,Y.EXE,1,2023-01-01 00:00:00",
      ),
    ).toBe("kape");
  });
  it("kape: Amcache header", () => {
    expect(
      detectImportKind("am.csv", "ApplicationName,FullPath,SHA1,FileKeyLastWriteTimestamp\na,c:/x,abc,t"),
    ).toBe("kape");
  });
  ...
  it("plaso: dynamic header", () => {
    expect(
      detectImportKind(
        "tl.csv",
        "datetime,timestamp_desc,source,message,parser\n2023-01-01T00:00:00+00:00,ctime,FILE,hi,filestat",
      ),
    ).toBe("plaso");
  });
  ...
  it("hayabusa: csv-timeline header", () => {
    expect(
      detectImportKind(
        "hb.csv",
        "Timestamp,Computer,Channel,EventID,Level,RuleTitle,Details\nt,c,Sec,4625,med,Failed,x",
      ),
    ).toBe("hayabusa");
  });
  it("m365: UAL CSV with AuditData column", () => {
    expect(
      detectImportKind("ual.csv", "RecordType,CreationDate,UserIds,Operations,AuditData\n8,t,u,Op,{}"),
    ).toBe("m365");
  });
  it("cybertriage: timeline CSV header", () => {
    expect(
      detectImportKind(
        "tl.csv",
        "event_timestamp,epoch_timestamp,message,timestamp_description,item_type,threat_level\n2026-01-28T01:52:03,1769593923,/x,File Modified,File,None",
      ),
    ).toBe("cybertriage");
  });
  it("velociraptor: Elastic Discover CSV export of DetectRaptor data", () => {
    const header =
      '"@timestamp",Artifact,"Artifact.keyword","Detection.Name","Detection.StringHit",_index,_Source';
    const row =
      '"May 7, 2026 @ 16:31:04.000",DetectRaptor.Windows.Detection.Amcache,DetectRaptor.Windows.Detection.Amcache,"Execution - PsExec",PsExec.exe,artifact_detectraptor_windows_detection_amcache,"-"';
...
describe("detectImportKind — Linux evidence sources (#62)", () => {
  it("auditd: raw audit.log / ausearch record format", () => {
    const log =
      'type=SYSCALL msg=audit(1490451217.272:270): arch=c000003e syscall=59 comm="cat" exe="/usr/bin/cat"\ntype=EXECVE msg=audit(1490451217.272:270): argc=2 a0="cat" a1="/etc/shadow"';
  ...
  it("sysdig: sysdig -j event JSON (dotted evt.* keys)", () => {
    expect(
      detectImportKind(
        ...
        ndjson({
          "evt.num": 1,
          "evt.rawtime": 1717200000000000000,
          "proc.name": "curl",
          "evt.type": "connect",
        }),
      ),
...
describe("detectImportKind — logs & edges", () => {
  ...
  it("thehive: single case object", () => {
    expect(detectImportKind("case.json", j({ _type: "case", title: "Incident", severity: 3 }))).toBe(
      "thehive",
    );
  });
  it("thehive: array of alerts", () => {
    expect(detectImportKind("alerts.json", j([{ _type: "alert", title: "TOR login", severity: 2 }]))).toBe(
      "thehive",
    );
  });
  it("thehive: search result container { data: [...] }", () => {
    expect(detectImportKind("search.json", j({ data: [{ _type: "case", title: "x", severity: 1 }] }))).toBe(
      "thehive",
    );
  });
...
<Event xmlns="http://schemas.microsoft.com/win/2004/08/events/event">
  ...
  it("evtxxml: namespace-stripped envelope still detected", () => {
    expect(
      detectImportKind("sysmon.xml", "<Events><Event><System><EventID>1</EventID></System></Event></Events>"),
    ).toBe("evtxxml");
  });
...
describe("detectImportWithCustom precedence", () => {
  ...
  it("velociraptor: pslist/pstree NDJSON (CallChain + Pid, no _Source)", () => {
    const row = {
      ...
      StartTime: "2026-06-12T11:12:45Z",
    ...
    const text = j([
      {
        ...
        LastModified0x10: "2026-06-01T00:00:00Z",
        InUse: true,
        FileName: "evil.exe",
      },
    ]);
  ...
  it("external-first: a custom importer can override even a specific built-in", () => {
    const r = parseImporterSpec({
      ...
      id: "my-evtx",
      match: { format: "json", requireKeys: ["EventID"], priority: 1 },
    });
