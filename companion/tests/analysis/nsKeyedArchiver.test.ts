...
import { parseBplist, BplistUid, type BplistValue } from "../../src/analysis/bplistReader.js";
import { resolveKeyedArchive, NsKeyedArchiverError } from "../../src/analysis/nsKeyedArchiver.js";
...
describe("resolveKeyedArchive — real NSKeyedArchiver fixtures", () => {
  ...
  // #1190: NSDate accepted any finite number/bigint for NS.time with no range check on the
  // resulting Date, so a huge-but-finite value silently flowed through as an Invalid Date. Built
  // directly as the parsed structure resolveKeyedArchive expects, rather than real bplist bytes.
  it("rejects an NSDate whose NS.time is finite but produces an out-of-range Date", () => {
    const objects: BplistValue[] = [
      "$null",
      new Map<BplistValue, BplistValue>([
        ["$class", new BplistUid(2)],
        ["NS.time", 1e300],
      ]),
      new Map<BplistValue, BplistValue>([["$classname", "NSDate"]]),
    ];
    const root = new Map<BplistValue, BplistValue>([
      ["$archiver", "NSKeyedArchiver"],
      ["$objects", objects],
      ["$top", new Map<BplistValue, BplistValue>([["root", new BplistUid(1)]])],
    ]);
    expect(() => resolveKeyedArchive(root)).toThrow(NsKeyedArchiverError);
    expect(() => resolveKeyedArchive(root)).toThrow(/out of representable range/);
  });
});
