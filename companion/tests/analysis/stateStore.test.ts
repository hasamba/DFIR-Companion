  ...
  it("migrates every complete entity payload from JSON with deep equality", async () => {
    ...
    state.findings = [
      {
        ...
        severity: "High",
        confidence: 87,
        confidenceReason: "two tools",
        title: "PowerShell",
        description: "Encoded command",
        relatedIocs: ["i1"],
        sourceScreenshots: ["one.png"],
        mitreTechniques: ["T1059.001"],
        relatedEventIds: ["e1"],
        firstSeen: "2026-07-30T10:00:00Z",
    ...
    state.iocs = [
      {
        ...
        firstSeen: "2026-07-30T10:00:00Z",
        aliasValues: ["www.example.invalid"],
        extractedFrom: ["e1"],
        note: "sinkholed",
        enrichments: [
          { source: "test", verdict: "suspicious", fetchedAt: "2026-07-30T11:00:00Z", tags: ["c2"] },
        ],
      },
    ];
    state.forensicTimeline = [
      upgradeForensicEvent({
        ...
        endTimestamp: "2026-07-30T10:01:00Z",
        description: "PowerShell connected",
    ...
    state.keyQuestions = [
      { id: "q1", question: "How?", status: "partial", answer: "PowerShell", pointer: "e1", pinned: true },
    ];
    state.nextSteps = [
      {
        id: "n1",
    ...
    state.iocExcludeRules = [
      { id: "x1", type: "domain", pattern: "*.example.org", addedAt: "2026-07-30T12:00:00Z" } as never,
    ];
  ...
  it("queries indexed event fields with a stable cursor", async () => {
    ...
    state.forensicTimeline = [
      ...
      },
      {
        ...
        sourceScreenshots: [],
        asset: "HOST-2",
        artifactName: "EDR.Processes",
      },
    ];
    await stateStore.save(state);

    expect(
      (await stateStore.queryForensicTimeline("c1", { host: "HOST-1" })).entities.map((event) => event.id),
    ).toEqual(["e1"]);
    expect(
      (await stateStore.queryForensicTimeline("c1", { source: "EDR.Processes" })).entities.map(
        (event) => event.id,
      ),
    ).toEqual(["e2"]);
    expect(
      (await stateStore.queryForensicTimeline("c1", { severity: "High" })).entities.map((event) => event.id),
    ).toEqual(["e1"]);
    expect(
      (await stateStore.queryForensicTimeline("c1", { technique: "T1059.001" })).entities.map(
        (event) => event.id,
      ),
    ).toEqual(["e1"]);
    expect(
      (await stateStore.queryForensicTimeline("c1", { ioc: "192.0.2.10" })).entities.map((event) => event.id),
    ).toEqual(["e1"]);

    const first = await stateStore.queryForensicTimeline("c1", { limit: 1 });
    const second = await stateStore.queryForensicTimeline("c1", {
...
describe("StateStore save format", () => {
  it("writes SQLite as the primary state and does not create a JSON mirror", async () => {
    ...
    await expect(access(join(caseStore.stateDir("c1"), "investigation.json"))).rejects.toMatchObject({
      code: "ENOENT",
    });
...
describe("StateStore load on an oversized state file", () => {
  ...
  const oversize = ["Cannot create a string longer than 0x1fffffe8 characters", "Invalid string length"];
  ...
  it("leaves malformed legacy JSON untouched and can migrate after it is repaired", async () => {
    ...
    await expect(access(join(caseStore.stateDir("c1"), "investigation.sqlite"))).rejects.toMatchObject({
      code: "ENOENT",
    });
