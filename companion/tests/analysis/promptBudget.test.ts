...
describe("promptBudget", () => {
  ...
  afterEach(() => {
    if (prevCtx === undefined) delete process.env.DFIR_AI_CONTEXT_TOKENS;
    else process.env.DFIR_AI_CONTEXT_TOKENS = prevCtx;
    if (prevMax === undefined) delete process.env.DFIR_AI_MAX_TOKENS;
    else process.env.DFIR_AI_MAX_TOKENS = prevMax;
  });
