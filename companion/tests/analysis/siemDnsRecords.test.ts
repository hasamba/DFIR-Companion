...
describe("Sysmon 22 — what one record establishes", () => {
  ...
  it("the queried name is a domain indicator; the returned addresses are not indicators", () => {
    ...
    const wild = parseSiemExport(
      elastic(
        sysmon22({ QueryName: "*.attacker.example", QueryStatus: "0", QueryResults: "::ffff:192.0.2.9;" }),
      ),
    );
    expect(wild.iocs.map((i) => i.value)).toContain("*.attacker.example");
    expect(wild.events[0].description).not.toContain("not a valid name");
...
describe("DNS Client operational log", () => {
  ...
  it("a missing value and a literal that spells its words stay two rows after import", () => {
    const rec = (results: string) =>
      dnsClient(3008, { QueryName: "txt.example", QueryType: "16", QueryStatus: "0", QueryResults: results });
    const r = parseSiemExport(elastic(rec("type: 16;"), rec("type: 16 (value not in this record);")));
    expect(r.events).toHaveLength(2);
    expect(afterImport(r.events)).toHaveLength(2);
  });
