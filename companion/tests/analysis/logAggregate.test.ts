...
import {
  ...
  type AggregateStats,
} from "../../src/analysis/logAggregate.js";
...
describe("templateizeLine", () => {
  it("masks digit runs, #ids and hex but PRESERVES IP addresses", () => {
    expect(templateizeLine("Failed password for root from 10.0.0.5 port 22")).toBe(
      "Failed password for root from 10.0.0.5 port N",
    );
    expect(templateizeLine("initiating Main Mode to replace #871204 for 'S_REF_Ips2office_0'.")).toBe(
      "initiating Main Mode to replace #N for 'S_REF_IpsNoffice_N'.",
    );
  ...
  it("groups lines that differ only by volatile numbers into the same template", () => {
    const a = templateizeLine(
      "starting keying attempt 269 of an unlimited number for 'S_REF_Ips2asihome_0'.",
    );
    const b = templateizeLine(
      "starting keying attempt 1315 of an unlimited number for 'S_REF_Ips2asihome_0'.",
    );
    expect(a).toBe(b);
  });

  it("keeps different message structures (and different IPs) as distinct templates", () => {
    expect(templateizeLine("Failed password from 1.1.1.1")).not.toBe(
      templateizeLine("Failed password from 2.2.2.2"),
    );
  });
});
...
describe("aggregateLogLines", () => {
  it("collapses repeated lines into counted patterns, most frequent first", () => {
    const lines = [
      ...Array.from(
        { length: 20 },
        (_, i) => `May 28 09:00:${String(i).padStart(2, "0")} sshd: Failed password for root from 10.0.0.5`,
      ),
