...
} from "../../src/analysis/mobileSummary.js";
import {
  emptyState,
  type Finding,
  type ForensicEvent,
...
describe("worstVerdict", () => {
  ...
  it("returns the most malicious verdict across engines", () => {
    expect(
      worstVerdict({ enrichments: [enrich("harmless"), enrich("malicious"), enrich("suspicious")] }),
    ).toBe("malicious");
...
describe("buildMobileSummary", () => {
  ...
  it("caps heavy lists while reporting the pre-cap total", () => {
    ...
    const iocs = Array.from({ length: 5 }, (_, i) => ioc({ id: `i${i}`, type: "ip", value: `10.0.0.${i}` }));
...
describe("mobileSummaryEnvOptions", () => {
  ...
  it("reads overrides and ignores invalid values", () => {
    withEnv(
      { DFIR_MOBILE_MAX_EVENTS: "10", DFIR_MOBILE_MAX_IOCS: "nope", DFIR_MOBILE_MAX_FINDINGS: "-3" },
      () => {
