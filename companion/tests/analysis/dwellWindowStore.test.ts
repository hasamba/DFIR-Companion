...
describe("DwellWindowStore", () => {
  ...
  it("rejects invalid input (propagates sanitizeDwellWindowInput's error)", async () => {
    await expect(
      store.add("c1", { label: "", start: "2026-06-01T00:00:00Z", end: "2026-06-02T00:00:00Z" }),
  ...
  it("updates a window by id; returns null for an unknown id", async () => {
    ...
    const updated = await store.update("c1", w.id, {
      label: "Session 1 (refined)",
      start: w.start,
      end: w.end,
    });
