...
describe("parseVeloRef", () => {
  ...
  it("parses a hunt GUI URL", () => {
    expect(parseVeloRef("https://velo:8889/app/index.html?org_id=root#/hunts/H.CABC123/overview")).toEqual({
      kind: "hunt",
      huntId: "H.CABC123",
    });
  });
  it("parses a client+flow pair (either order, slash/space separated)", () => {
    expect(parseVeloRef("C.deadbeef/F.CFF001")).toEqual({
      kind: "flow",
      clientId: "C.deadbeef",
      flowId: "F.CFF001",
    });
    expect(parseVeloRef("F.CFF001 C.deadbeef")).toEqual({
      kind: "flow",
      clientId: "C.deadbeef",
      flowId: "F.CFF001",
    });
  ...
  it("parses a collection GUI URL", () => {
    expect(
      parseVeloRef("https://velo:8889/app/index.html?org_id=root#/collected/C.deadbeef/F.CFF001/overview"),
  ...
  it("keeps a hunt-launched flow's trailing .H suffix (does not truncate it)", () => {
    expect(parseVeloRef("C.9e5afcaf10536cd9/F.D93M3ERL39HVE.H")).toEqual({
      kind: "flow",
      clientId: "C.9e5afcaf10536cd9",
      flowId: "F.D93M3ERL39HVE.H",
    });
    expect(
      parseVeloRef("https://velo/app/index.html#/collected/C.9e5afcaf10536cd9/F.D93M3ERL39HVE.H/overview"),
  ...
  it("flags a notebook URL — a flow/hunt's notebook shows the analyst's own filtered VQL, not raw rows", () => {
    expect(
      parseVeloRef("https://velo:8889/app/index.html?org_id=root#/collected/C.deadbeef/F.CFF001/notebook"),
    ).toEqual({ kind: "flow", clientId: "C.deadbeef", flowId: "F.CFF001", isNotebookUrl: true });
    expect(parseVeloRef("https://velo:8889/app/index.html?org_id=root#/hunts/H.CABC123/notebook")).toEqual({
      kind: "hunt",
      huntId: "H.CABC123",
      isNotebookUrl: true,
    });
  ...
  it("flags an uploads-tab URL (hunt and flow), leaving other tabs unflagged", () => {
    expect(
      parseVeloRef("https://velo:8889/app/index.html?org_id=root#/collected/C.deadbeef/F.CFF001/uploads"),
    ).toEqual({ kind: "flow", clientId: "C.deadbeef", flowId: "F.CFF001", isUploadsUrl: true });
    expect(parseVeloRef("https://velo:8889/app/index.html?org_id=root#/hunts/H.CABC123/uploads")).toEqual({
      kind: "hunt",
      huntId: "H.CABC123",
      isUploadsUrl: true,
    });
