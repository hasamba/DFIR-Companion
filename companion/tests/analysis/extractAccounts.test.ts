...
describe("extractAccounts — path segments containing a space are not accounts", () => {
  const PATH_NOISE: ReadonlyArray<[string, string]> = [
    [
      "Registry set: HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Shell Folders\\Common Startup",
      "Folders\\Common",
    ],
    ["ImageLoaded=C:\\Program Files\\Windows Defender\\MpOAV.dll", "Files\\Windows"],
    [
      "Registry set: HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings\\MigrateProxy = DWORD",
      "Settings\\MigrateProxy",
    ],
    [
      "Registry set: HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Memory Management\\ClearPageFileAtShutdown",
      "Manager\\Memory",
    ],
    [
      "Registry set: HKU\\S-1-5-21-1013\\Software\\Microsoft\\Office\\16.0\\PowerPoint\\File MRU\\Item 9",
      "MRU\\Item",
    ],
    [
      "Registry set: HKCU\\Software\\Microsoft\\Office\\16.0\\Word\\Reading Locations\\Document 12\\Datetime",
      "Locations\\Document",
    ],
    [
      "Sysmon Process create (EID 1) - Image=C:\\Program Files\\Google\\Drive File Stream\\97.0.1.0\\GoogleDriveFS.exe",
      "Stream\\97.0.1",
    ],
...
describe("extractAccounts — registry hive roots are not accounts", () => {
  ...
  it("does not read a hive\\SID pair out of a registry TargetObject", () => {
    const d =
      "Image=C:\\Windows\\System32\\RuntimeBroker.exe - TargetObject=HKU\\S-1-5-21-1474774169-3792502490-1226900992-1001\\Software";
  ...
  for (const hive of ["HKU", "HKLM", "HKCU", "HKCR", "HKCC"]) {
    it(`does not treat ${hive}\\… as an account`, () => {
      expect(extractAccounts(`Registry set: ${hive}\\Software`).some((a) => a.startsWith(`${hive}\\`))).toBe(
        false,
      );
    });
  }
...
describe("extractAccounts — real accounts still extracted", () => {
  ...
  it("extracts a user account from a 4624 logon line", () => {
    const d =
      "Windows Security Successful logon (EID 4624) - FAIRHAVEN\\eleanor.voss, NT AUTHORITY\\SYSTEM - LogonType=3";
  ...
  it("extracts a machine account and a service account", () => {
    expect(extractAccounts("Successful logon (EID 4624) - FAIRHAVEN\\WS-HR-01$, LogonType=3")).toContain(
      "FAIRHAVEN\\WS-HR-01$",
    );
    expect(extractAccounts("Logon with explicit credentials (EID 4648) - FAIRHAVEN\\svc_sqlagent")).toContain(
      "FAIRHAVEN\\svc_sqlagent",
    );
  });
