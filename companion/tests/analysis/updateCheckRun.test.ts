...
function fakeStore(initial: UpdateCheckRecord = {}, opts: { failSet?: boolean } = {}) {
  ...
  return {
    ...
    async setResult(result: UpdateResult) {
      if (opts.failSet) throw new Error("disk full");
      rec = { ...rec, result };
    },
