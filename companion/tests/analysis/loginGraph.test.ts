...
const LOGON = (
  ...
  host: string,
  type: number,
  o: { failed?: boolean; ip?: string; count?: number; ts?: string; endTs?: string } = {},
) =>
  ev({
...
describe("parseLoginEvent", () => {
  ...
  it("parses a successful domain-account logon with type, source ip and workstation", () => {
    const p = parseLoginEvent(
      ev({
        description:
          "Windows Security Successful logon (EID 4624) - CORP\\jdoe - LogonType=10 - IpAddress=203.0.113.7 - WorkstationName=WKSTN-02 @ SRV-01 [RemoteInteractive/RDP from 203.0.113.7]",
        asset: "SRV-01",
      }),
    );
    expect(p).toEqual({
  ...
  it("parses a failed logon (4625) as outcome failed", () => {
    const p = parseLoginEvent(
      ev({
        description:
          "Windows Security Failed logon (EID 4625) - CORP\\admin - LogonType=3 - IpAddress=10.0.0.9 @ DC-01",
        asset: "DC-01",
      }),
    );
  ...
  it("parses NT AUTHORITY pseudo-accounts (space in domain)", () => {
    const p = parseLoginEvent(
      ev({
        description:
          "Windows Security Successful logon (EID 4624) - NT AUTHORITY\\SYSTEM - LogonType=5 @ WKSTN-01",
        asset: "WKSTN-01",
      }),
    );
  ...
  it("takes the FIRST account (the logon target) when subject account is also rendered", () => {
    const p = parseLoginEvent(
      ev({
        description:
          "Windows Security Successful logon (EID 4624) - CORP\\jdoe, CORP\\WKSTN-02$ - LogonType=3 @ SRV-01",
        asset: "SRV-01",
      }),
    );
  ...
  it("decodes an unknown numeric logon type as `type N` and a missing one as Unknown", () => {
    const withNum = parseLoginEvent(
      ev({
        description: "Windows Security Successful logon (EID 4624) - CORP\\jdoe - LogonType=13 @ SRV-01",
        asset: "SRV-01",
      }),
    );
    expect(withNum?.typeName).toBe("type 13");
    const noType = parseLoginEvent(
      ev({
        description: "Windows Security Successful logon (EID 4624) - CORP\\jdoe @ SRV-01",
        asset: "SRV-01",
      }),
    );
  ...
  it("rejects a marker embedded in a field value (log-content injection)", () => {
    expect(
      parseLoginEvent(
        ev({
          description:
            "Sysmon Process create (EID 1) - CommandLine=echo Successful logon (EID 4624) - EVIL\\fake @ x - Image=C:\\evil.exe @ HOST1",
          asset: "HOST1",
        }),
      ),
    ).toBeNull();
  ...
  it("returns null for: non-logon rows, rows with no account segment, rows with no asset", () => {
    expect(
      parseLoginEvent(
        ev({ description: "Sysmon Process create (EID 1) - CommandLine=cmd.exe @ H1", asset: "H1" }),
      ),
    ).toBeNull();
    expect(
      parseLoginEvent(
        ev({ description: "Windows Security Successful logon (EID 4624) - LogonType=3 @ H1", asset: "H1" }),
      ),
    ).toBeNull();
    expect(
      parseLoginEvent(
        ev({ description: "Windows Security Successful logon (EID 4624) - CORP\\jdoe - LogonType=3" }),
      ),
    ).toBeNull();
  });
});
