...
describe("buildManualEvent", () => {
    ...
    const e = buildManualEvent(
      {
        ...
        severity: "High",
        asset: "DC01",
        mitreTechniques: "T1059.001, t1003 garbage",
      },
      deps,
    );
  ...
  it("rejects an event with no description", () => {
    expect(() => buildManualEvent({ timestamp: "2026-06-04T00:00:00Z", description: "" }, deps)).toThrow(
      ZodError,
    );
  });
});
...
describe("buildManualIoc", () => {
  it("builds an IOC with id/firstSeen", () => {
    ...
    expect(i).toEqual({
      ...
      type: "ip",
      value: "8.8.8.8",
      firstSeen: "2026-06-04T00:00:00.000Z",
    });
  });
