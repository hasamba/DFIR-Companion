...
export function findingsCsv(state: InvestigationState): string {
  ...
  const rows = state.findings.map((f) =>
    row([
      ...
      f.description,
      f.relatedIocs.join("|"),
      f.mitreTechniques.join("|"),
      f.sourceScreenshots.join("|"),
      f.firstSeen,
      f.lastUpdated,
      f.status,
    ]),
...
export function timelineCsv(state: InvestigationState): string {
  ...
  const rows = state.timeline.map((t) =>
    row([t.timestamp, String(t.windowSequence), t.description, t.sourceScreenshots.join("|")]),
  );
  return [header, ...rows].join("\n") + "\n";
}
  ...
  const rows = [...state.forensicTimeline]
    ...
    .map((e) =>
      row([
        ...
        e.severity,
        e.description,
        e.mitreTechniques.join("|"),
        (e.sources ?? []).join("|"),
        e.relatedFindingIds.join("|"),
        e.sourceScreenshots.join("|"),
      ]),
    );
  return [header, ...rows].join("\n") + "\n";
}
