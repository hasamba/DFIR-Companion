...
export const REPORT_SECTION_DEFS = [
  ...
  // Off by default (#930 item 4): a per-resource exposure table belongs in a report the analyst
  // chose to carry it.
  {
    key: "servedExposure",
...
export function normalizeReportTemplate(input: unknown): ReportTemplate {
  ...
  return {
    ...
    releaseRequirements: {
      ...
      requiredSections: [...new Set(t.releaseRequirements.requiredSections)].filter(
        (key): key is ReportSectionKey => SECTION_KEY_SET.has(key),
      ),
    },
  };
}
...
export const BUILT_IN_REPORT_TEMPLATES: readonly ReportTemplate[] = [
  ...
  normalizeReportTemplate({
    ...
    description: "A short client-facing brief: cover, executive summary, compliance impact, and conclusions.",
    accentColor: "#0b6e4f",
  ...
  normalizeReportTemplate({
    ...
    headerText:
      "{{#if companyName}}{{companyName}} — {{/if}}Technical Investigation{{#if incidentId}} ({{incidentId}}){{/if}}",
...
export function defaultReportTemplate(): ReportTemplate {
  return normalizeReportTemplate(BUILT_IN_REPORT_TEMPLATES.find((t) => t.id === DEFAULT_TEMPLATE_ID) ?? {});
}
...
export function buildBrandingContext(state: InvestigationState, meta: ReportMeta): Record<string, string> {
