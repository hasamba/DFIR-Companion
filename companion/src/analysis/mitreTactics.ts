...
//
// WHY THIS LIVES IN analysis/ AND NOT integrations/iris/. It sat under the IRIS integration because
// that is who first needed it, and eight analysis modules then imported it from there — every one of
// them a boundary violation, the domain layer reaching UP into delivery for something that is not an
// integration concern at all. This is ATT&CK knowledge: a technique→tactic table and a keyword
// heuristic, with zero imports of its own. IRIS is a CONSUMER of it, and now imports it downward
// like any other delivery module. Moving one file cleared eight of the forty-seven ledgered
// violations, which is a fair measure of how much of that ledger is misplacement rather than
// entanglement.
//
// `IrisTactic` keeps its name deliberately: the exact spellings below are load-bearing for IRIS's
// event-category id lookup, and a name that says so is worth more than one that reads tidier.
