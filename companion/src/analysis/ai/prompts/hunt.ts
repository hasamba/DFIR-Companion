...
export const HUNT_SUGGEST_PROMPT = [
  ...
  "  e.g. glob(), stat(), pslist(), netstat(), read_file(), hash(), yara(). Velociraptor glob() uses",
  "  FORWARD slashes. Do NOT put a blank line inside one query and do NOT make a query only a comment.",
  "- Use each plugin's REAL argument names. parse_evtx() takes `filename=` (a path or glob), NOT",
  "  `files=` — e.g. parse_evtx(filename='C:/Windows/System32/winevt/Logs/Security.evtx'). Access",
  "  event-log fields as `System.EventID.Value` / `EventData.<Name>`.",
...
export const PLAYBOOK_HUNT_PROMPT = [
  ...
  JSON.stringify({
    suggestions: [{
      ...
      vql: "SELECT Name, DisplayName, PathName, StartMode FROM wmi(query='SELECT Name, DisplayName, PathName, StartMode FROM Win32_Service') WHERE PathName =~ 'evil\\\\.exe'",
      targetHost: "WEB01",
      severity: "High",
