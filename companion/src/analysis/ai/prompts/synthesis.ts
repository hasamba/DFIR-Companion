...
export const SYNTHESIS_PROMPT = [
  ...
  "- abstention: if the evidence shows ONLY activity you can positively attribute to routine or expected",
  "  operations — no attacker technique, tool, or malicious behavior anywhere in the timeline — return an",
  "  EMPTY findings array instead of inventing a 'nothing suspicious found' or 'routine activity' finding",
  "  just to have something to return; state that conclusion in 'summary', scoped to what you were shown",
  "  (e.g. 'no attacker activity identified in the provided evidence', not a clean bill of health). This",
  "  is about the ABSENCE of malicious activity, not about confidence — a genuinely suspicious finding",
  "  stays a finding at ANY confidence, including low; only omit findings entirely when nothing in the",
  "  timeline suggests attacker activity at all. If the evidence itself is too narrow or sparse to be",
  "  confident the sample is representative, that is NOT the clean case — say so as an uncertainty (and",
  "  keep the relevant keyQuestions open) instead of reporting routine activity. Only when the case is",
  "  genuinely resolved with nothing open, unknown, or in doubt about the sample do threadsOpened/",
  "  keyQuestions/nextSteps stay empty too.",
  ...
  "  One paragraph per phase of the intrusion. Plain language — no ATT&CK T-codes, no hashes. Cite",
  ...
  "  when the question is fully answered. If a finding shows attacker-relevant file activity in one",
  "  subfolder of a tree whose SIBLING subfolders you have NOT seen (e.g. a subfolder surfaced via",
  "  $MFT/Autoruns/a detection hit), and those unseen siblings could plausibly answer THIS question, prefer",
  "  collecting them on the SAME host before an external log or a different host — often the cheapest,",
  "  most direct source. Do not apply this to a directory with no attacker-relevant activity in it, or when",
  "  an unseen sibling could not plausibly bear on this question.",
  ...
  "  Include the scoping step a senior responder takes next for each confirmed activity, not only host",
  "  artifact pulls. Examples: after a mailbox takeover or BEC, review the account's payment and",
  "  accounts-payable activity; after a phishing delivery, search the other recipients' mailboxes for the",
  "  same attachment or sender; after lateral movement, pull logon events on the SOURCE host; on a Linux",
  "  host, pull auth.log; after suspected exfiltration, pull EDR network telemetry for the host; after",
  "  insider data movement, pull DLP logs; after a cloud sign-in anomaly, review the identity provider's",
  "  sign-in and audit logs for the user. Name the exact host, account and log in each step, because the",
  "  analyst acts on the pointer as written.",
  ...
  "  For EVERY hypothesis, ALSO give 'subjectScope': exactly 'hosts' or 'caseWide' — never",
  "  omit it. Use 'hosts' when the claim concerns specific host(s) (e.g. 'the payload did not execute on",
  "  WS-042') and ALSO give 'subjectHosts' naming every one of them, using a real host seen in the timeline",
  "  below — this is DIFFERENT from relatedEventIds (supporting evidence) and discriminator (the separating",
  "  artifact): subjectHosts is which machine(s) the claim ITSELF is about. Use 'caseWide' only when the",
  "  claim genuinely spans the entire case across every host — not merely because you are unsure which host",
  "  it concerns; when unsure, still pick 'hosts' and name the host(s) the evidence actually points to.",
  ...
  JSON.stringify(
    {
      ...
      hypotheses: [
        { title: "Initial access was spear-phishing", expectedOutcome: "an .eml attachment or a malicious URL click in web-proxy logs on the first-compromised host", status: "open", relatedTechniques: ["T1566.001"], relatedEventIds: ["e3"], relatedIocIds: ["i1"], contradictingEventIds: [], discriminator: "email gateway logs on MAIL01 for the delivery event", subjectScope: "hosts", subjectHosts: ["ALClient07"] },
        { title: "Data was staged before exfiltration", expectedOutcome: "an archive (.zip/.7z/.rar) written shortly before an outbound transfer", status: "supported", relatedTechniques: ["T1560.001"], relatedEventIds: ["e7"], relatedIocIds: [], contradictingEventIds: ["e9"], discriminator: "$MFT on FS01 for the archive-creation timestamp", subjectScope: "hosts", subjectHosts: ["FS01"] },
      ],
      uncertainties: [
...
export const NARRATIVE_PROMPT = [
  ...
  "- One paragraph per phase of the intrusion.",
  "",
