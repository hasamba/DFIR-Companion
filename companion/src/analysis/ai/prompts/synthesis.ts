...
export const SYNTHESIS_PROMPT = [
  ...
  "  can be stale or simply wrong. Count ORIGINS, not providers: several providers may relay one report,",
  "  and a hit with no recorded origin counts as none. A verdict tagged [lone-intel] or [multi-origin],",
  "  with no other timeline evidence of real activity (an execution, a data transfer, a credential use)",
  "  ON that indicator, is NOT by itself sufficient for a High/Critical finding — treat it as a lead worth",
  "  mentioning at Low/Medium with confidence capped well below 70, not a confirmed compromise; two named",
  "  origins do not make it independent. A 'malicious' verdict PLUS corroborating timeline activity",
  "  ([corroborated]) can justify a higher severity — the activity, not the provider's name. If a verdict is",
