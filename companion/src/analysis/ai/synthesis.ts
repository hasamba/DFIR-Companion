...
async function autoGenerateHypotheses(
  ...
  // Eligibility for the material-change reading (#933 item 22) is the same as the dashboard's: in
  // the timeline and not marked false positive. A link outside it cannot change a frozen
  // conclusion's footing — the ledger the analyst sees would not show it either.
  await ctx.opts.hypothesisStore.applyAutoGenerated(caseId, seeds, new Date().toISOString(), {
    eligibleEventIds: validEventIds,
    falsePositiveEventIds: falsePositiveEventIds([...markers]),
  });
}
