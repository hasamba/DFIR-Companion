...
const HISTORY_FILE_RE =
  /(?:^|[._-])(?:bash|zsh|sh|ash|ksh|fish)_history$|\.history$|consolehost_history\.txt$/i;
...
export function looksLikeBashHistory(filename: string, text: string): boolean {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (BASH_TS_RE.test(lines[i].trim())) {
      const next = lines[i + 1]?.trim() ?? "";
      if (next && !next.startsWith("#")) bashTs++;
    }
...
const CMD_RULES: CmdRule[] = [
  ...
  {
    re: /\b(?:curl|wget)\b[^\n]*(?:--data-binary|--upload-file|\s-T\b|\s-F\b|--form|-d\s+@|--data\s+@)/i,
    severity: "Medium",
...
export function userFromHistoryFilename(filename: string): string {
  ...
  n = n
    .replace(/(?:[._-](?:bash|zsh|sh|ash|ksh|fish))?_history$/i, "")
    .replace(/\.history$/i, "")
