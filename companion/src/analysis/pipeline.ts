...
export const SYSTEM_PROMPT = [
  ...
  "ARE the evidence.",
  "EXCLUDE the act of OPERATING the tool / navigating the UI / narrating your own workflow (no",
  "incident data): hunts created/started/expired, a notebook or page 'accessed', a query/VQL/search",
  "being run, 'EventLog analysis performed', 'Response and Monitoring accessed', clicking/scrolling.",
  "Those are the analyst's work log, NOT forensic events.",
  "",
  "HARD RULE — NEVER emit a forensicEvent that describes the ANALYST, the INVESTIGATION PROCESS, or",
  "this app. These are BANNED as events (they are not things that happened on the system under",
  "investigation): 'data collection' / 'performed/initial/continued/ongoing/final data collection',",
  "'data analyzed' / 'analysis completed/continued/ongoing', 'surveying/reviewing the dashboard',",
  "'investigation context', 'Surveying the DFIR Companion Dashboard', or any sentence whose SUBJECT",
  "is the analyst or a tool rather than the system under investigation. If you catch yourself writing",
  "a vague event like 'continued data collection with Velociraptor', DELETE it.",
  "",
  "ALSO BANNED — opening/navigating to a tool, dashboard, console or web UI is NOT an event:",
  "'Access to VolWeb', 'VolWeb access observed', 'Access to Syslog Dashboard - Elastic', 'DFIR",
  "Companion dashboard access observed', 'Kibana/Splunk/Timesketch/VirusTotal opened/viewed'. The",
  "DATA inside those tools is evidence; the act of opening them is not. Skip these entirely.",
  "",
  "Every forensicEvent MUST (a) name a CONCRETE artifact value from the system under investigation —",
  "a specific process, file path, account, host, IP/domain, registry key, service, scheduled task, or",
  "event-log record — and (b) carry a timestamp READ FROM THAT ARTIFACT's own time column. If you",
  "cannot point to a concrete artifact value AND its own real timestamp, emit NOTHING. An empty",
  "forensicEvents array is the CORRECT answer for any screenshot that shows only the tool UI, a",
  "dashboard, navigation, a login/welcome screen, an empty/loading panel, or your own activity.",
  "Rule of thumb: a row of artifact DATA with a real timestamp = forensic event; a sentence about",
  "you using the tool or 'collecting/analyzing data' = skip. NEVER stamp an event with the screenshot",
  "capture time — if the only time you can see is the capture time, the artifact has no event time.",
  "",
