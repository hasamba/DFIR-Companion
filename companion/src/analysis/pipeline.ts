...
export const SYSTEM_PROMPT = [
  ...
  "",
  "EDR/XDR DETECTIONS ARE EVIDENCE — extract EVERY one. A CrowdStrike Falcon / Defender for Endpoint /",
  "SentinelOne 'detections' view is a CONSOLE OF FINDINGS, not a 'dashboard' to skip. Each detection",
  "card — with its severity, triggering process / command line, file path, hash, host, user, MITRE",
  "tactic & technique, and 'actions taken' (e.g. 'Parent process killed') — IS a real malicious event:",
  "emit it as a forensicEvent AND raise a finding. Use the detection's own time as the event time.",
  ...
  "    process killed'; IOCs ShadowMark.exe + the SHA256; a finding (suspected machine-account/SAC",
  "    abuse). Multiple near-identical detections collapse into one finding, but keep each distinct",
  "    process/host/command as its own event.",
  "",
  ...
  "panel or login page. This is NARROW: a detections/alerts console FULL of detections (CrowdStrike,",
  "Defender, SentinelOne) is EVIDENCE — extract the detections, do not dismiss the page as",
  "'navigation'. If a screen names a process, file, command line, hash, host, user, or threat, it has",
