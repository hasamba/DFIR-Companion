...
export class CorrelationProfileStore {
  ...
  async load(caseId: string): Promise<CorrelationProfile> {
    try {
      ...
      const profileName = (
        ["strict", "moderate", "aggressive", "custom"] as CorrelationProfileName[]
      ).includes(raw.profileName as CorrelationProfileName)
        ? (raw.profileName as CorrelationProfileName)
        : DEFAULT_PROFILE.profileName;
      const windowSeconds =
        typeof raw.windowSeconds === "number" && raw.windowSeconds >= 0
          ? raw.windowSeconds
          : DEFAULT_PROFILE.windowSeconds;
