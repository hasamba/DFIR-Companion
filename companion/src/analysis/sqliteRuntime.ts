...
//   2. `node:sqlite` only exists on Node 22.5+. A top-level value import produces a less actionable
//      startup failure on an unsupported runtime just because this module is in the import graph.
//
...
// it (Node < 22.5). The indexed case store is now the primary backend, so this is a startup/runtime
...
export function loadDatabaseSync(): DatabaseSyncCtor {
  ...
  if (!mod?.DatabaseSync) {
    throw new Error(
      "node:sqlite is unavailable — DFIR Companion requires Node 22.5 or later to open case storage",
    );
  }
