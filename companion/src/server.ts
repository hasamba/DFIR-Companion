...
export function createApp(store: CaseStore, options: AppOptions = {}): Express {
  ...
  app.post("/cases/:id/import", async (req: Request, res: Response) => {
    ...
    if (kind === "unknown") {
      return res.status(400).json({ error: "could not detect the file type — not recognized as any supported import (THOR / SIEM-EDR / Chainsaw-EVTX / Hayabusa / Velociraptor / Suricata-Zeek / KAPE / Cyber Triage / M365-Entra / AWS / GCP-Azure / Plaso / Sandbox / Email-eml-msg / auditd / journald / sysdig-Falco / CSV / log)" });
    }
