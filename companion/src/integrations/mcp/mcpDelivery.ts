...
//   - No progress. A 16 GB copy is opaque until it finishes or fails. The job it runs under can say
//     "transferring" and nothing more.
...
export async function deliver(server: McpServer, localPath: string, ctx: DeliveryContext): Promise<DeliveredTarget> {
  ...
  const result = await retryTransientSpawn(() => ctx.runner("scp", args, { timeoutMs, signal: ctx.signal }));
  if (result.code !== 0) {
    throw new Error(
