...
export function renderThemeCss(
  ...
  return atColumnZero(`    /* === dfir-theme tokens (issue #53) ===
...
${renderImportedThemes(IMPORTED_THEMES)}

${renderThemeChrome(IMPORTED_THEMES, values)}

    /* === end dfir-theme tokens === */`);
  // No trailing newline: findTokenRegion() ends the region at the closing brace, so the
