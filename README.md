...
- **Velociraptor live monitoring** (opt-in, API config) — stream a **CLIENT_EVENT** artifact (e.g. `Windows.Events.ProcessCreation`) from one endpoint into a case **as events fire**; a server-side poller imports new rows on an interval (`DFIR_VELO_MONITOR_POLL_S`) with a persisted cursor so a restart never re-ingests. Settings → Velociraptor → Live Monitoring + a 🔴 LIVE dashboard badge.
...
- **Import from DFIR-IRIS** — the reverse: pull an existing IRIS case's assets/IOCs/timeline into a case via the **Import from IRIS** picker (or `npm run iris:import -- <case> <irisCaseIdOrName>`); deterministic, no AI.
