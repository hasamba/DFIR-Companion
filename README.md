...
| `DFIR_ALLOWED_HOST_SUFFIXES` | _(none)_ | Same as above but matched on a domain suffix, e.g. `.killercoda.com`, for platforms that mint a fresh hostname per session. Matching is on a label boundary, so `.acme.com` never matches `evilacme.com` |
