...
captures screenshots of your investigation (Velociraptor, VirusTotal, etc.) as
...
Full configuration, all npm scripts (`reanalyze`, `synthesize`, `coverage`,
`verify:ai`, `clean-timeline`), HTTP endpoints, the case-folder layout, and the
analysis model are documented in **[companion/README.md](companion/README.md)**.
...
Implemented from the four plans in `docs/superpowers/plans/` (companion core → analysis
pipeline → reports/dashboard → extension), then extended with: granular findings,
forensic timeline, attacker path, key questions, IOC panel, investigation threads,
two-tier models, AI on/off + backfill, mark-legitimate (false positives), and
investigation time-window scoping. All on the `master` branch.
