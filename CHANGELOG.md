...
### Fixed
- **bash-history upload rule now also tags T1567.002** — `bashHistoryImport.ts`'s curl/wget-upload rule was T1041-only (Exfiltration Over C2 Channel), mislabeling a plain HTTPS upload to cloud storage; it now also tags T1567.002 (Exfiltration to Cloud Storage), keeping T1041 so `exfilCorrelate.ts`'s archive-staging→upload stitching still fires (EvidenceForge northpeak-insider-codetheft benchmark).
- **Threat-intel verdicts no longer silently anchor synthesis on the org's own infrastructure** — a malicious/suspicious verdict on a value that's ALSO one of the case's own host assets (a shared internal DB/DC/file server naturally scores high on cross-host "connective reach" for reasons that have nothing to do with being a C2 backbone) is now flagged as an `internalConflict` in `iocAnchors.ts`'s connective-indicator ranking and `synthSelect.ts`'s verdict digest, with the malicious-verdict score bump capped instead of stacking on top of the connective score. `SYNTHESIS_PROMPT` now explicitly instructs the model that a lone third-party verdict (especially a flagged CONFLICT) is corroborating evidence, not a standalone conclusion, and must not alone justify a High/Critical finding. Root-caused a Critical false-positive "C2 via malicious domain" finding on the northpeak-insider-codetheft benchmark's own DB server.
- **`logAggregate` no longer drops rare/unique lines when truncating to the AI prompt budget** — the per-template cap kept the MOST frequent patterns and silently discarded the rest, which is backwards for a log dominated by repetitive baseline noise: a one-off attack line (a single unusual upload, a lone 403 on a sensitive path) is exactly the kind of thing that gets outvoted by thousands of routine health-checks. Truncation now protects the rare end and drops the noisiest, best-explained-already templates instead — root cause of `proxy_access.log`/`web_access.log` AI extraction returning zero events on a 27K-line real-world log in the northpeak-insider-codetheft benchmark.
- **AI log-triage no longer over-grades routine sudo/auth activity into a fake "campaign"** — `LOG_SYSTEM_PROMPT` now explicitly grades a named account running a specific, scoped sudo command (service restart/status, package install, listing firewall rules) as Info/Low even when it recurs or several different accounts each do their own such commands, and only escalates on a genuine anomaly (new account gaining sudo, a sudoers/group addition, an unexpected interactive root shell, sudo immediately followed by credential-dumping/exfil). Scattered auth failures across many unrelated users/hosts stay Info instead of "brute force". `SYNTHESIS_PROMPT` gained a matching guard against stitching many different people's ordinary admin work (each individually tagged sudo/High) into one "Privilege Escalation"/"Brute-Force Campaign"/"Lateral Movement" finding without a coherent single-account, bounded-window narrative. Root-caused a 6→122 findings blow-up (almost entirely High-severity noise from ordinary company-wide syslog activity) when validating the northpeak-insider-codetheft benchmark with `syslog.log` included.

### Changed
- **Timeline "⛏ Sources" filter button now shows with a single source too** — it previously stayed hidden until the timeline had events from 2+ distinct tools; now it appears as soon as there's at least one, so the analyst can see/toggle which tool contributed events even on a single-source case.

### Added
- **Cisco ASA firewall syslog importer** (`analysis/ciscoAsaImport.ts`) — a new deterministic importer for the classic BSD-syslog-framed `%ASA-#-######:` message format (Built/Teardown/Deny), detected ahead of the generic AI log-triage fallback and its year-less-timestamp guessing risk. Firewall Built/Teardown telemetry stays Info by default (mirrors Zeek `conn.json`); an explicit Deny bumps to Low (a block is prevention, not confirmed compromise, but still worth surfacing — blanket-demoting denies previously hid a real lateral port-scan in the branch-office benchmark). Dynamic-NAT-translation messages (305011/305012), which carry only the NAT mapping and never the real destination, are dropped as pure noise. Public destination IPs become IOCs.
- **Apache/Nginx/Squid combined access-log importer** (`analysis/combinedLogImport.ts`) — a new deterministic importer for the near-universal "combined" access-log format used by web-server access logs AND Squid forward-proxy logs, detected ahead of the generic AI log-triage fallback. Raw web/proxy telemetry, so severity stays Info by default with a conservative bump only for a clear signal: access-denied (401/403/407) → Low; the git smart-HTTP clone/push URL signature (`…/repo.git/info/refs?service=git-upload-pack`, the canonical way any git client clones from any self-hosted git server) → T1213 without escalating severity. Every distinct destination host (absolute-URL request or CONNECT tunnel target) becomes a domain IOC; the authenticated user (Squid's `%u` field) is folded into the description for free UPN detection. Replaces the AI log-triage path for these two formats entirely, closing the needle-in-haystack gap where a 27K-line proxy/web log's rare high-signal lines (a single unusual upload, a lone denied clone) never reached the model at all (northpeak-insider-codetheft benchmark).
