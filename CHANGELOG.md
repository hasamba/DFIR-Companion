...
### Added
- **Missed evidence review** — an analyst-pressed fast model (Jev) grades the Info rows the tagger left behind; tick rows and promote them with the model's grade; off until `DFIR_JEV_ENABLED` is set (closes #1540, closes #1568)
- **Second look is a button and a panel** — it waits to be pressed, previews what it would promote, and re-runs the conclusions afterwards (closes #1554)
- **Story so far** — one card per kill-chain stage in the Now cockpit, with intrusion shape, greyed missing stages and a copy-as-brief button (closes #1487, closes #1493)
- **2nd-opinion referee** — models labelled A/B; a configurable referee (default: model A) judges each disagreement from the cited events (closes #1466)
- **Follow the referee in one click** — accept or reject every pending delta the way the referee said
- **Negative answers name their evidence** — a per-host collection inventory reaches synthesis, so a "not observed" answer says what was collected (closes #1588)
- **Other commands in this session** — each finding lists the session's command lines and file writes that no finding names (closes #1594)
- **Prompt seats for session commands** — quiet Low/Medium command lines near Critical/High rows on the same host reach synthesis (closes #1622)
- **Discovery commands in script blocks** — `nltest`, `Get-AD*`, `ntdsutil … ifm` and similar are read out of 4104/4103 records with ATT&CK tags and a Medium finding (closes #1531)
- **"Which host did this file come from?"** — a log export with no collector asks for its host; a drop-folder `asset=<HOST>` subfolder does the same (closes #1496)
- **Promoted rows show in the forensic timeline** — a `✓ Promoted` badge at any severity, and a count of the rows the filters hide
- **Execute-assembly traces in CLR usage logs** — a .NET usage log named after rundll32, mshta or a similar host grades High (T1620) (closes #1559)
- **Egg-Cellent Resume playbook** — the playbook matcher can name the more_eggs → Cobalt Strike → Pyramid intrusion (closes #1560)
- **Hayabusa Full bundle** — a built-in Velociraptor bundle that runs every Hayabusa rule except the noisy ones
- **Case picker shows case names** — same-name cases show as `name (id)`; the id stays in the URL and exports (closes #1523)
- **Toolbar cluster labels** — Case / Modes / AI / Evidence / Report / View labels under the buttons, at no extra height
- **Model version on job rows** — the jobs popover shows the model each AI job ran on, not only the alias (closes #1601)
- **Deep pass says when it has nothing new to read** — Run stays off unless you tick *Run anyway* (closes #1457)
- **Deliberately set-aside rows outlive the super-timeline cap** — ordinary telemetry is evicted first, and the import card reports what the cap dropped (closes #1535)

### Changed
- **Synthesis starts only when asked** — AI-on, Re-synthesize, import completion and the last duplicate-host resolve start a run; other changes mark conclusions out of date (closes #1599)
- **A host's build history is not dwell time** — rows before a renamed host's first rename are set aside from gap analysis; dwell findings need attacker activity on both sides (closes #1503)
- **AI eval gate averages three runs** — a no-regression attestation needs three real runs per section, with a 5-point tolerance (part of #1579)

### Fixed
- **The case's own collector is not evidence** — Velociraptor downloads, installs, spawned PowerShell, PersistenceSniper sessions and rule files grade Info with `origin: collector` (closes #1460, closes #1477, closes #1486, closes #1488, closes #1500, closes #1555)
- **Loopback is never the collector server** — a Companion on the Velociraptor server keeps local connections at their grade (closes #1471)
- **Mentioned is not observed** — an indicator read from free text is *mentioned* or *referenced*, earns no risk or corroboration points, and draws no network line (closes #1459, closes #1461, closes #1474)
- **VM provisioning stops becoming the top findings** — rows in a build window cap at Low with a `build-time` note (closes #1529)
- **Vendor updater traffic is not C2** — OneDrive, Edge and Defender traffic to Microsoft ranges grades Info (closes #1530)
- **A renamed host is one asset** — a case-wide rename ledger; bare log exports, flow exports and the super-timeline follow it (closes #1458, addresses #1489, closes #1495, closes #1508)
- **Facts the prompt cut dropped ride as tags** — rclone destination, fetched URL, `ip:port` and a renamed-shell flag reach the AI (closes #1502)
- **Web-server → shell chains grade High with T1190** (closes #1501)
- **Run-key grading** — enumerated PersistenceSniper Run keys stay Info; a message-less 4657 Run-key write grades Medium (closes #1666, closes #1670)
- **Stock Store apps and OneDrive tasks stay Info** on a clean Windows 11 image (closes #1665)
- **Collector scripts and ordinary OS behaviour no longer become findings** (closes #1593)
- **Own-child EID 10 and AppX firewall churn rules cover Hayabusa and evtx XML imports** (closes #1621)
- **DNS joins** — Event XML DNS joins to connections; a split query links IOCs to every row; DNS Server analytical records stay out (closes #1636, closes #1642, closes #1643)
- **Import timestamps** — a copied file is dated by creation time; Autoruns compact time reads as UTC; an unreadable Velociraptor time is missing, not stored (closes #1603, closes #1618, closes #1631)
- **Copied binaries are not timestomping** (closes #1558)
- **Correlation keeps distinct events** — a write and its launch, renamed copies, and separate launches stay separate rows (closes #1476, closes #1557)
- **Correlation stays linear on big hash and path groups** (closes #1483)
- **No duplicate auto findings** (closes #1556)
- **Batched Velociraptor import** — a full `$MFT` imports in batches instead of running out of memory (closes #1439)
- **No route loads the whole super-timeline** — IOC provenance, TLS graph, tagger and Timesketch push stream it in batches (closes #1444, closes #1447)
- **IOC provenance is indexed at write time** — 1.4 s instead of 75 s on a 900k-event case (closes #1452)
- **The dashboard stays live during a big import** — read-only workers and WAL; the jobs chip rides the socket (closes #1453, closes #1454)
- **A failed bulk import leaves nothing behind** (closes #1480)
- **Every import logs start, progress, outcome and failure** (closes #1438)
- **One import run at a time** (closes #1511)
- **Archiving tolerates an in-flight write** (closes #1442)
- **An empty hunt settles only what it covered** — per-client counts; only finished hosts; never a time-scoped or filtered hunt (closes #1604, closes #1612, closes #1625)
- **An unread artifact is never "no rows"** — partly read or unresolved sources are recorded as not read (closes #1635, closes #1645, closes #1651)
- **Hunt parameters** — sent byte for byte; a bundle parameter the hunt cannot send is refused (closes #1606)
- **A re-collect is not read as done** (closes #1662)
- **A cleared log demotes only steps on its own host** (closes #1605)
- **Synthesis reads what the review promotes** (closes #1586)
- **Re-synthesis keeps finding ATT&CK tags** (closes #1684)
- **A failed synthesis answer is kept and retried** with what it missed (closes #1602)
- **An automatic kick waits for a running synthesis** instead of paying twice (closes #1608)
- **Scenario findings are capped** once a case is judged a simulation (closes #1595)
- **"Conclusions out of date" only when true** — never before the first synthesis; an empty case says "nothing to synthesize" (closes #1676)
- **Output-limit failures say so** — one failure naming the limit; Ollama asks for low reasoning unless 🧠 is ticked
- **Deep reasoning works on Claude Code and current Claude models** — recorded per run, greyed out where unsupported (closes #1468)
- **Cancelling a CLI provider call kills its process tree** (closes #1627)
- **Drop-folder imports name their model on the job row** (closes #1629)
- **Deep-pass result card** — sits under the verdict, names its question, and says when a later synthesis replaced it
- **Second opinion pairs findings by cited events** — accepting B lowers A's severity instead of adding a duplicate (closes #1682)
- **Accepted 2nd-opinion decisions follow their finding** — they survive new runs and show when they no longer apply (closes #1590)
- **The referee cannot dismiss the only evidence for an open question** (closes #1596)
- **A failed referee is shown** — with a `↻ re-run referee` button (closes #1587)
- **Missed evidence review hardening** — rate-limited, 404 on an unknown case, 409 on a second run, server-kept grades, 15-minute cap with *Stop waiting* (closes #1549, closes #1551, closes #1552, closes #1577, closes #1578)
- **Missed evidence review key hint and counts** — they refer only to what the analyst can see; the own-tooling filter no longer hides collected rows (closes #1547, part of #1554)
- **Second look spends its budget on new rows** — no double run; repeats held back after three (closes #1576, part of #1554)
- **Live updates survive sleep and reconnect** — backoff reconnect; every push-driven panel re-reads its data (closes #1675, closes #1681)
- **Confidence filter and report-template picker update live** (closes #1691)
- **Session-command notes skip ordinary system activity** — at most 12 per finding (closes #1683)
- **Pagers keep your page across a background refresh** — forensic timeline and IOCs (closes #1649, closes #1652)
- **Jump-to-event lands on the right page** — under a severity floor, a lens, and after clearing a search (closes #1658, closes #1663)
- **A slow IOC answer for the previous case no longer repaints the current one** (closes #1653)
- **The status pill shows ingest, not AI, during an import** (closes #1525)
- **Story so far cards** — headline drops importer labels; contrast meets 4.5:1 (closes #1616)
- **Demo case button keeps its icon when collapsed** (closes #1581)
- **No write creates a case by accident** — a write to an unknown case gets a 404 (closes #1570)
- **`assetHost` refused where it is ignored** (closes #1509)
- **Path redaction closes two leaks** (closes #1512)
- **Nightly e2e races** — `import-aws` and origin-lens tests are deterministic (closes #1445, closes #1448)

## [0.37.0] - 2026-09-20

### Added
- **Handoff Brief** — a shift-change panel: findings by owner, open questions, hypotheses, next steps, unchecked IOCs, the outgoing analyst's note; copy as Markdown, opt-in report section (closes #1406)
- **Now — Next recommended step** — the cockpit shows the top three playbook steps after Open hypotheses (closes #1424)
- **Playbook finding cards are tasks** — each Critical/High finding becomes an imperative, evidence-named task with steps and a Done-when line (closes #1418)
- **Ask the Case sees your decisions** — hypotheses, host-scope decisions, dwell windows, hunt outcomes and notes reach the prompt; a three-turn thread; trimmed timelines are disclosed (closes #1411)
- **Attack outcome on two axes** — each finding records execution (observed / not) and control (blocked / remediated / failed / allowed / none) separately, analyst-set and synthesis-proof (part of #930)
- **Import progress bar** — every job row with progress draws a bar with rate and ETA; an external hunt/flow import is a cancellable job (closes #1428)
- **Fleet Collection panel** — running a bundle, importing a GUI hunt/flow and live monitoring moved from Settings into a case-scoped panel
- **Pre-export evidence-safety check** — every human-readable export is checked for live indicators and unescaped evidence; a hit ships with a banner and a status warning (closes #1006)
- **Audit-log export to a SIEM** — forward each case's activity log to Splunk HEC, Elasticsearch or syslog, opt-in per destination, replay-safe (closes #929)
- **Related Cases** — a panel that lists other investigations sharing an indicator; off unless `DFIR_CROSS_CASE=on` (closes #679)
- **Sigma → VQL hunts** — paste a Sigma rule, compile it deterministically, launch it as a recorded fleet hunt; `process_creation` rules also hunt Sysmon/4688 history; mixed-category drafts compile to one hunt with several sources (part of #764; closes #796, #797, #798, #802)
- **Chainsaw as a built-in tool** — runs on raw `.evtx` beside Hayabusa; a parser run is recorded in custody with version, arguments and rule hash; the raw upload is kept; one record read by two parsers is one row (#688)
- **Intel assertion history and retirement review** — every provider hit is a dated assertion; expired and revoked ones are marked and kept; the **Intel Retirement Review** panel lists findings whose intel is no longer actionable (part of #933, addresses #1024)
- **A threat-intel verdict says when it applies** — VirusTotal and AbuseIPDB dates are compared with the case time on every surface; every enrichment record says where its claim comes from and distinct origins count, not providers (part of #933)
- **Post-remediation recurrence checks** — declare a remediation boundary and Verify returns facts only, with coverage stated; a **Remediation Checks** panel and report section (part of #930, addresses #969)
- **Phishing campaign scope** — per message who was addressed, what was delivered, which attachments, and per host what its own rows establish about execution and control (part of #930)
- **Served exposure** — declare a web root and read, per file, suspected exposure → retrieval requested → response size recorded → corroborated disclosure (part of #930)
- **Kerberoast chain** — per service account the 4769 requests, the baseline, later use by the exact account and first-seen-host use; panel and report section (part of #930)
- **Sensitive access** — declare the files that matter and read 4663 / 4656 / 5145 / 4660 per object with the process and logon session as candidates; panel and report section (part of #930)
- **Defender episodes** — a process start from a path Defender acted on, dated after that action, is annotated and raised; a same-digest start after remediation is a High finding (part of #930, addresses #964)
- **Defender detections say what Defender did** — 1116–1119 rows lead with the control disposition and name the threat and file (part of #930)
- **Web access logs decode the request and name the attack shape** — traversal, command, expression and SQL injection graded Medium + T1190; the status is stated, never read as an outcome (part of #930)
- **A web-log line says what its own fields establish** — Squid result and hierarchy codes, `CONNECT` tunnels, 3xx and 304 semantics, byte counts on bodiless responses (part of #933)
- **Injection and hollowing sequences by process GUID** — Sysmon 10 / 8 / 25 / 1 joined only through a matching GUID; injection-shaped and hollowing sequences graded High + T1055 (part of #932, addresses #987)
- **Process access and remote threads graded by the record** — a Sysmon 10 handle by its rights, call trace and target; a Sysmon 8 thread by where it starts; the blanket T1003 is gone (part of #932)
- **A download mark corroborated by execution** — a `Zone.Identifier` row is read against Prefetch, process starts and presence records of the same file and raised only when execution is dated after it (part of #932, addresses #985)
- **A hidden stream payload is not a download mark** — ADS rows graded by content, not name; download marks carry no technique until execution is corroborated (part of #932)
- **Copied-binary lead on MFT rows** — a file whose `$SI` modified time predates its created time was copied here; DetectRaptor MFT rows get the timestomp check (addresses #1422)
- **Mobile rows say where the content came from** — every iLEAPP / ALEAPP row carries an origin tag (recorded / synced / received / not established) from a pinned registry; an infection window opens on a malicious app-inventory row (part of #932, addresses #988)
- **iOS app-identity corroboration** — an app named by two or more independent iLEAPP artifacts is noted on its earliest row (part of #932)
- **Mobile evidence with no clock is kept** — LEAPP tables without a time column import as undated rows that sort last (part of #932)
- **Sandbox reports are lab evidence** — CAPEv2 and Falcon Sandbox rows go to the super-timeline only; the verdict reaches the AI as a tag on host events; a "Sandbox reports" report section (part of #932)
- **Memory run envelope** — a JSON envelope written by the script that invoked Volatility imports beside the export and states what the run establishes (part of #933, addresses #1016)
- **A memory export says what it establishes** — a zero-row export is one Low row; `windows.info` / `crashinfo` become an image row every row carries (part of #933)
- **A memory socket record says what it establishes** — `netscan` / `netstat` rows keep the stored state, scan vs traversal, and a consistency note against the upload's process rows (part of #933)
- **Intact (trimmed VolWeb) import** — `memory_payload.json` and `yarascan_results.jsonl` read deterministically with the two YARA sets deduped (closes #776)
- **TLS Relationships** — Zeek `ssl` / `x509` and Suricata `tls` records become one row per relationship and per certificate; a case-wide graph with JA3S nodes and decoded chain members; a dashboard panel (part of #933, addresses #997)
- **DNS answer → connection lead** — Zeek and Suricata DNS answers are joined to the same client's later connections inside the answer's TTL window (part of #933, addresses #996)
- **A DNS record says what it establishes** — Sysmon 22 and the DNS Client log read the query, status and returned values, never "resolves to" (part of #933)
- **Web request chains** — Zeek `http` / `files` and Suricata `http` / `fileinfo` joined only through identifiers both carry; partial digests are no indicator (part of #933, addresses #993)
- **macOS quarantine chain** — the LSQuarantine record reads as a download event; the xattr is decoded by its own form; database ↔ file attribute, download → launchd persistence, origin page ↔ browser visit and marked file ↔ process start are all joined (part of #933; closes #1037)
- **macOS launchd persistence** — plists read as configuration, never as a run, with `# target:` and `# launchctl:` collection lines carrying what the plist cannot (part of #908, #933)
- **macOS login items from every container** — classic `loginitems.plist`, `SessionLoginItems.sfl2` and both BTM generations; binary files upload as bytes (closes #1301)
- **Linux persistence artifact analysis** — SSH keys, cron, systemd units, shell profiles, SUID listings and PATH from one collection (part of #908)
- **Instance-metadata credential access, cloud bulk-read summaries, rclone / MEGAsync evidence, Docker container escape, Shellbag user attribution** — five detections (part of #908)
- **Ransomware precursor correlation, certutil transfer explanation, Squiblydoo and obfuscation-resistant command matching** — four Windows detections (part of #908)
- **Memory cross-view, deobfuscation layers, malfind context, WER reports, process lifetime, USN lifecycle, timestomp corroboration, SRUM network totals and Prefetch resource leads** — ten evidence and correlation passes (part of #909)
- **Cloud logging changes read as states** — CloudTrail, GCP and Azure logging deltas read for the state their request establishes (part of #931)
- **GCP identities and policy deltas** — principals typed, delegation chains whole, one row per `SetIamPolicy` binding delta (part of #931)
- **Google Drive sharing, Takeout and job lifecycle** — sharing direction, `visibility_change`, one row per Takeout job (part of #931)
- **Google Workspace OAuth rows and lifecycle** — which app, which scopes, what it called; one summary row per client whose export records form a lifecycle (part of #931, addresses #983)
- **AWS rows say which credential, role and account** — every CloudTrail row carries the caller's kind, key class and issuer; STS issuance rows are findable by the key they issued (part of #931)
- **AWS credential lineage and compute lifecycle** — one summary row per credential and per EC2 instance whose upload records form a lineage (part of #931, addresses #979)
- **IAM changes say what the record says** — a verb, every identity changed, the policy document's words, who a role trusts (part of #931)
- **Cloud remote execution graded with its evidence** — SSM `SendCommand` / `StartSession` and Azure Run Command; "requested", never "ran" (part of #931)
- **Mailbox records and the mailbox chain** — inbox rules, forwarding, permissions and MailItemsAccessed read per record; one summary row per session whose Exchange records form a chain (part of #931, addresses #975)
- **Entra application changes and privilege path** — every credential, grant, consent and role assignment as a row; credential → grant → sign-in → consistent action as one path (part of #931, addresses #973)
- **An observation says whether it distinguishes the explanations** — every hypothesis carries an evidence assessment across the set; a frozen judgment whose footing changes is flagged for review (part of #933)
- **Attribution gap leads** — beside each attribution assertion, the techniques that group is documented to use that this case has not shown (closes #1405)
- **An imported `.dfircase` is checked against its own manifest** — each entry verified, the manifest kept, the arrival recorded in custody (closes #904)
- **A bare VQL run or fleet hunt is on the activity log** — with the query, the hunt id and how it ended (closes #832)

### Changed
- **A renamed host is one host** — Velociraptor, Chainsaw and Hayabusa rows take the asset from the collector's identity; a differing `Computer` is the former name (addresses #1417)
- **Process command lines keep their tail** — Sysmon / 4688 command lines render in full up to 400 chars, so a C2 `ip:port` reaches the timeline and the AI (addresses #1416)
- **Copied binaries dated at the drop** — nested MFT columns prefer `$FN` Created, then `$SI` Created, before LastModified (addresses #1415)
- **A failed Entra sign-in is not automatically brute force** — T1110 only on codes that mean credential validation failed; an absent status is unknown, not success (part of #931)
- **A ROPC sign-in no longer claims an observed MFA bypass** — the legacy-auth observation stays, the bypass claim goes (part of #931)
- **The scope presets mean the last N hours from now** — matching the Velociraptor collection presets (closes #738)
- **The dead CloudTrail `PassRole` rule is gone** — AWS has no PassRole API (part of #931)

### Fixed
- **Popup and hotkey captures reach the case** — the companion accepts the extension's `manual` trigger (closes #1434)
- **Hayabusa emergency rows import as Critical** (closes #1433)
- **A row promoted from the super-timeline stays promoted** — the demote pass keeps stamped rows (closes #1432)
- **Super-timeline first page in milliseconds on a big store** — count, facets and page from indexed SQL (100k rows: 92 s → 0.03 s) (closes #1429)
- **Live monitor enables its artifact in Velociraptor** — and Poll flags an artifact removed behind its back (closes #1409)
- **Boundary figure no longer committed** — ARCHITECTURE.md drops the pair that made every concurrent PR conflict (closes #1368)
- **Binary plists refused on the text path** with the byte-native hint (closes #1392)
- **Static-report attestation reads forensic ∪ super-timeline** (closes #1389)
- **Cloud flow-log hardening** — bounded ids, bracket-stripped text (closes #1388, #1371, #1370)
- **Bulk-read truncation count travels with its groups** (closes #1372)
- **Severity floor keeps IOCs** — across all 35 remaining importers, and capa / olevba (closes #1353, #1337, #1366)
- **Host attribution restored on older cases** — logons persisted before the edge-observed stamp are re-stamped on read (closes #1352)
- **Gated logon samples are disclosed on the host-identity match routes** (closes #1345)
- **sqlite Row ID forgery guard** (closes #1330)
- **Drop-folder reads are cap-bounded on the descriptor** (closes #1329)
- **Client-reported IOCs are documented and marked in the CSV exports** (closes #1354, #1333)
- **Duplicate-host panel hardening** (closes #1281, #1282, #1284)
- **Alias-record unicode names use one spelling** (closes #1358)
- **A cloud bulk-read summary keeps its own id and words** (closes #1356)
- **Container-escape pass no longer clips long descriptions** (closes #1340)
- **A collection request is satisfied only by evidence imported after it** — the Sigma hits that prompted a request no longer mark it satisfied
- **Every `esc()` copy escapes both quote flavours** — all nine copies, tested (closes #1050, #1004)
- **Sysmon file events name the file** — a FileCreate / FileDelete row's path is the TargetFilename
- **Team-mode auth resolves a session once per request** — ~76 µs → ~10 µs (closes #1003)
- **The live feed strips absolute paths from AI-status errors** (closes #1029)
- **An Elastic export auto-detects as SIEM, not Velociraptor** (closes #1022)
- **An edit that omits `enabled` keeps the saved state** (closes #998)
- **Every dedicated import route runs the import spine** — all 22 `import-*` endpoints lock, snapshot, dual-write, tag, demote, record and checkpoint (closes #956)
- **An undated row never acquires a time** — and Timesketch exports count the rows they omit (closes #957)
- **The typed event envelope survives a merge** (closes #965)
- **The AI reads the whole description** — head and tail of every long row (closes #959)
- **A starred or tagged raw row survives the super-timeline cap** (closes #958)
- **Super-timeline cap keeps the most recently imported rows** (part of #932)
- **MISP and DFIR-IRIS pushes drop false positives** (closes #951)
- **A raised event always states its reason** — no pass can clip another's note (closes #939)
- **Cloud rows that differ only past a clip stay two rows** (closes #940)
- **A MITRE technique no longer cites a finding the analyst confirmed benign** (closes #938)
- **Import-from-path classifies a JSON-array export larger than 256 KB** (closes #953)
- **Switching cases cannot filter the new case through the old case's false-positive markers** (closes #937)
- **A redaction beside a secret no longer hides it** (part of #930)
- **A hung SO-CRATES request cannot stall an analysis job forever** — every request has a deadline (closes #927)
- **A web access log records which client made the request** (part of #930)
- **Hunt audit-write failures are visible to the analyst**
- **Every standard Settings control has an associated label** — all 348 fields
- **Presidio's environment settings are documented**; the playbook-match manual uses the real default
- **An OIDC `returnTo` cannot redirect off-site after login** (closes #839)
- **A Velociraptor result filter cannot smuggle a second statement** (closes #843, #853)
- **Case-lock and `.dfircase` scrypt derivations no longer block the event loop** (closes #862, #863)
- **A Slack/Teams `response_url` is matched in the resolver's spelling** (closes #840)
- **An MCP scp identity file must be an absolute path** (closes #850)
- **A deleted case is evicted from the cross-case IOC cache** (closes #822)
- **An oversized VQL body is refused with a 400** (closes #825, #828)
- **Report, STIX and release-pack downloads build `Content-Disposition` through the shared helper** (closes #845, #849)
- **The opencode review workflows pin the action to a commit** (closes #817)
- **SECURITY.md names the trust boundaries that are deliberate** (closes #824)
- **A symlink or hardlink in a case directory is no longer served as evidence** (closes #818)
- **Sigma compiler fixes** — unused selections no longer shape the hunt, a missing path root refuses, `CommandLine` stays with its category, RE2 named groups are accepted, an empty live-snapshot result is not a miss, an AI-suggested hunt can be deployed as a snapshot (closes #803, #808, #809, #810, #815, #816)
- **A new automatic detector cannot lose its findings in silence** — an unregistered id fails the build (closes #758)
- **Intact import fixes** — every injected region is its own row, a service's file IOC is its executable, `N/A` is not a file, registry rows keep their time, a large payload imports from a path, a one-hit YARA scan is kept, the matched string survives either import order (#776)
- **The filter box visibly reaches the super-timeline** — and only the newest request may paint
- **Lynx ransomware impact is detected** — a word-like family tag counts when the victim file kept its own extension; widen with `DFIR_RANSOM_EXTS_STRICT`
- **A collecting Velociraptor hunt says what it is doing** (closes #770)
- **The command palette reaches every panel again**
- **An AI-imported event's year no longer depends on the server's timezone** (closes #757)
- **A markdown-style link in evidence text yields both of its URLs** (closes #755)
- **One C2 URL is the same indicator whichever tool collected it** (closes #756)
- **A recorded timestamp is never re-dated to match the rest of an import** (closes #739)
- **A clock months out of true is no longer invisible** (closes #740)
- **Two hosts on one subnet are no longer treated as one machine** (part of #740)
- **Concurrent edits no longer overwrite each other in team mode** — tagger rules, analyst decisions, dwell windows, playbook tasks, ticket references, hunts, monitors, second-look results and more (closes #682 and follow-ups)
- **Expired team-mode sessions are swept every six hours** (closes #676)
- **The timer-lifecycle test suite no longer flakes** (closes #685)
- **Switching a webhook channel to another provider requires the new provider's URL** (closes #683)
- **`"false"` disables a notification channel or event toggle** (closes #684)
- **A detection seen on two machines is two findings again** — Sigma, YARA, download, task, persistence and renamed-binary keys keep the host (closes #670)
- **The bundle hunt label picker stays responsive on a large fleet** (closes #665)

### Security
- **First-administrator setup cannot be brute-forced and always needs the bootstrap token** — lockout after five guesses, a short token stops the server, no loopback fallback (closes #920, #945)
- **Two whole-file reads that could crash the companion are capped** — `DFIR_DROP_MAX_BYTES` bounds the SO-CRATES upload and `DFIR_MAX_IMPORT_FILE_MB` bounds Import-from-path (closes #921)
- **A crafted `.dfircase` cannot plant paths outside the drop folder** (closes #919)
- **Teams and Telegram webhook secrets cannot be brute-forced** — lockout per platform, a short secret stops the server (closes #944)
- **The production HTTP query parser is pinned to patched `qs` 6.16.0**
- **The AI cannot sign its own findings as deterministic** — an id that names no finding is renamed on the way in (closes #787)
- **The KEV feed URL cannot be pointed at the host's own network** — `https` to a public address only, redirects re-checked, DNS pinned; `DFIR_KEV_ALLOW_INTERNAL_URL` for a mirror (closes #760)
- **A coding-agent run cannot grow its output buffer without end** (closes #762, #763)
- **A hostile "NSRL export" cannot suppress evidence** — identifiers are escaped (closes #761)
- **The cross-case pivot is off unless a deployment asks for it** (closes #723)
- **Importing an old `.dfircase` archive warns that its encryption is weaker** (closes #672)
- **The EVTX parsers fail closed** — a non-zero exit rejects the run (#688)
- **A failed OIDC sign-in no longer shows the identity provider's error text** (closes #674)
- **Case unlock tokens carry a payload version** — older unlock cookies stop working (closes #671)
