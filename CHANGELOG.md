...
### Added
- **Import change tracking on the Forensic Timeline — "what was added in the last import".** Mirroring the Findings "🧠 Last synthesized N ago / what-changed" view, the Forensic Timeline now shows a **📥 Last import N ago — +N new events** banner (with the source file + detected kind, and an expandable list of exactly what was added), and the events the last import brought in are **highlighted in the table with a green accent + a `NEW` badge** — so after dropping in a new artifact you can immediately see what it contributed instead of hunting through a re-sorted timeline. Backed by a pure, unit-tested `diffTimeline()` (matches events by normalized time + description — the same key correlation uses — so a re-import of the same file shows *no* new events rather than every row reappearing under a fresh id) and a new `ImportMetaStore` (`state/import-meta.json`) that the unified `/import` route writes after each import completes (before re-synthesis, which preserves the timeline); read via `GET /cases/:id/import-meta`, pushed live to the dashboard over the WS (`import_meta_changed`). The detail list is capped at 500 (the banner still shows the true total); the per-row highlight clears on the next import.
