...
### Added
- **Case password protection** — set a password on a case (🔒 Password… in the case lifecycle menu) so opening it in the dashboard requires that password; a "remember on this computer" option skips the prompt on future visits from the same browser. Enforced server-side (an unlock cookie gates every `/cases/:id/*` route), not just a UI prompt — the capture extension's evidence ingestion keeps working while a case is locked.
...
### Changed
- **Findings and IOCs panels redesigned as dense tables** — severity/ID/confidence (Findings) and type/value (IOCs) now sit in proper grid columns instead of one crowded line of bracket-style severity text, raw emoji action buttons, and ad-hoc inline-styled pills. A Finding's evidence now expands into the confidence explanation, supporting IOCs, and a real cited-event timeline (timestamp + description per event, each with its own jump link) instead of a bare `[1][2][3]` footnote line. Comment/tag/pin/false-positive/hunt/explain/chain-provenance buttons across every panel that shares these controls (Findings, IOCs, Forensic Timeline, Super Timeline, Key Questions, Threads) now render inline icons instead of emoji glyphs. The Super Timeline also now renders through the same row markup as the Forensic Timeline instead of an independently-styled inline row, and both timelines' occurrence-count/corroboration/parent-chain badges moved off inline styles onto CSS classes.
- **Full case export/import now an encrypted archive, not a JSON-only snapshot** — "Export encrypted case archive (.dfircase)" replaces "Full case export (shareable JSON)": the whole case directory (screenshots, raw evidence, timeline, findings, IOCs, all analyst decisions) is zipped and AES-256-GCM encrypted under a password you choose, instead of a JSON bundle that only carried references to evidence. Only openable via another DFIR Companion's Import. The downloaded file is named `<caseId> - <case name>.dfircase` (just `<caseId>.dfircase` when the case has no distinct name) instead of the bare case id.
...
### Fixed
- **A case's password wasn't actually asked for again unless you fully closed the browser** — two compounding issues: (1) the browser that just set/changed a password was auto-granted a session cookie, so it never got prompted for that password itself; (2) even after fixing that, an unlock made *without* "remember on this computer" still silently survived switching to a different case and back, or a plain page refresh — a browser-session cookie persists across in-page navigation, only clearing on an actual browser close. Setting/changing a password no longer auto-unlocks the setter, and a not-remembered unlock is now explicitly forgotten the moment you leave that case (switch away, refresh, or close the tab) — so, matching the "remember on this computer" checkbox literally, not checking it means the password is asked for again every time you (re)open that case, while checking it keeps working across browser restarts as before.
- **"Remove password" always worked and always said "removed," even when the case had no password set** — the case-password modal now checks the case's actual lock state when opened: "Remove password" is disabled when there's nothing to remove, and both "Save" and "Remove" are disabled (with an explanation) when the case is currently locked, since changing or removing a password requires being unlocked first.
- **Timeline Anomalies (and other deterministic derived panels) didn't refresh when the time scope changed** — applying/clearing the scope window only ever re-rendered scope-derived panels via the AI re-synthesis's WebSocket broadcast, so with AI off (or no provider configured) Timeline Anomalies, Phases, Beacon Candidates, Timeline Gaps, Host Ranking, Adversary Hints, D3FEND, ATT&CK Mitigations, Geo Map, Swimlane, IOC Sources/Provenance, and the Super Timeline all kept showing stale, unscoped data indefinitely. Applying a scope now reloads all of these immediately, since they're derived straight from the forensic timeline and need no AI call.
- **Customer Exposure never auto-discovered a domain from the case's own FQDN hosts** — the breach/leak check only ever searched domains the analyst manually typed in; a case full of FQDN hosts (already known internally, e.g. to the anonymizer) showed "no domains" until you retyped one by hand. The victim domain implied by each event's FQDN asset is now added automatically as a dashed "auto" chip (same pattern as auto-discovered customer emails), and case emails under that domain are checked too.
...
- **Duplicate-looking playbook items** — an AI-recommended "next step" and the auto-generated "Investigate & remediate" task for the finding it pointed at were two separate playbook entries describing the same thing. A next step whose pointer cites a finding that already gets its own auto-task is no longer added as a second entry — its actionable detail (rationale + what to collect) is folded into that finding's task instead.
- **Marking a finding false positive didn't hide it until the background AI re-synthesis finished** (many seconds later) — unlike events/IOCs, the Findings panel had no client-side false-positive filter, so it kept showing the stale list until a fresh state arrived. It now disappears from the Findings panel the instant the mark is confirmed.
- **Key Investigative Questions kept their stale "answered" status after their supporting finding was marked false positive** — a question could stay "answered" citing a finding the analyst had just rejected, since nothing tracked which finding(s) backed which answer. Synthesis now records each answer's supporting finding ids, forces a re-answer of any question whose support was just marked false positive, and deterministically resets a question to "unknown" (clearing the stale answer) if none of its supporting findings survive.
...
[Unreleased]: https://github.com/hasamba/DFIR-Companion/compare/v0.30.0...HEAD
