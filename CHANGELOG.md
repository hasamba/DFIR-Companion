...
### Added
- **Corroboration filter (lens)** — a global filter-bar control to show only Timeline events / IOCs / Findings observed by 2+ or 3+ distinct tool sources, cutting single-source background noise (internet scanners, benign telemetry) so the multi-source attack path stands out. A LENS not a gate — nothing is dropped from state; single-source evidence (a Sysmon-only process, a syslog-only logon) returns at "any". Per-browser (`dfir.corrobMin`).
