...
### Added
- **Audit-log export to a SIEM** — each case's activity log forwards to Splunk HEC, Elasticsearch or RFC 5424 syslog, opt-in per destination in Settings, with a durable per-case position so a restart neither repeats nor skips, the entry id and success/failure carried so a retry cannot duplicate a row, and the record saying whether the actor was a verified session or a client-supplied name (closes #929)
