...
### Added
- **Velociraptor data pushed from Elasticsearch routes to the Velociraptor importer** — when Velociraptor/DetectRaptor output is indexed into Elastic and pushed from Kibana, `detectImportKind` now recognises it (the `artifact_<name>` index or flattened `Detection.*`/`Artifact` keys) and routes to `importVelociraptor` for full severity + MITRE + Sigma/YARA classification, instead of the generic SIEM path. The importer normalizes the ES-reshaped rows back to native form (un-flatten dotted keys, collapse `.keyword`/`.text` multi-fields, derive the artifact from the index name) — gated so native Velociraptor JSON is untouched — and reads DetectRaptor keyword-hit verdicts from `Detection.StringHit`/`HitString`.
