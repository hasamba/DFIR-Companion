...
### Added
- **Duplicate hostnames are confirmed before the AI reasons about them** — one machine that appears under both a short name and its FQDN (`WIN11` and `WIN11.windomain.local`) was treated as two hosts by every derivation, and merging it in the asset graph reached only that panel. Synthesis now stops on an unresolved pair before a prompt is built or a token spent, and a header badge offers "Same host — merge" or "Different hosts" per pair; resolving the last one re-runs the held synthesis automatically. The analyst's decision now reaches the prompt, the playbook, the Suspicious Hosts panel, the exported report, Q&A, hunt suggestions and the known-unknown collection targets. Detection also runs at import, so a case with AI disabled — which never reaches the gate — still raises the badge. Pairs the fleet roster already links are never flagged, and "Different hosts" is remembered permanently (closes #572).
- **Two finding-origin lenses in the Findings panel** — "Hide auto-flagged" and "Hide coverage-gap" let an analyst see only what AI synthesis concluded, hiding the deterministic backfill findings raised for uncovered Critical/High events and silent-source gaps. Per-case and remembered like the confidence floor; a lens, not a gate — nothing is deleted, pinned findings stay visible, and the header switches to an "N of M" count so a hidden finding is never silently absent. The seeded demo case now carries one finding of each kind, so the lenses have something real to hide the first time an analyst tries them.
- **Per-case Presidio switch** — the anonymization panel's "Real names (people)" row is now a live checkbox when an analyzer is configured, so an analyst can stand the layer down for a case when it is down, slow or noisy without clearing `DFIR_PRESIDIO_URL` and restarting the server. Takes effect immediately, keeps the configuration for when the container is healthy, says plainly that names then reach the model unmasked, and records the flip in the case activity log.
...
### Changed
- **Milestone notifications now default to on** — they were off because milestones are lifecycle chatter (case opened, report generated, drop import finished) and a channel that wanted findings did not want a feed. That stopped holding when a milestone became the only push a *blocked* case produces: with the default off, the duplicate-hostname gate could stop synthesis and tell nobody. **This reaches existing channels, not only new ones** — the default fills any alert kind a saved configuration never explicitly set, so channels created before this begin receiving milestones, including case-opened and report-generated. Turn the kind off per channel to opt out (closes #574).
...
### Fixed
- **An external hunt or flow import reads one artifact at a time** — both branches read EVERY artifact into one object and then stringified a second full copy of it. Under the 1,000-row dashboard cap that was survivable; at the 100,000-row collection cap a multi-artifact hunt holds millions of rows and takes the process down — the same heap exhaustion the bundle collector was rewritten to avoid, reintroduced by raising the cap. Each artifact is now read, ingested and released before the next is touched, and is persisted as its own evidence file. One consequence is deliberate and worth knowing: the severity floor is gate-aware (an all-Info feed has no verdicts to discriminate on and imports whole), and the batch it judges is now an artifact rather than the whole hunt — the same granularity the bundle collector has always used.
- **A merged multi-source read obeys the row cap** — an artifact with both an unnamed source and named ones is read twice and concatenated, and only the constituent reads were capped: two 75,000-row reads under a 100,000 cap merged to 150,000 rows reporting `truncated: false`, over the ceiling and with the new incomplete-collection warning suppressed exactly when it was most deserved.
- **A THOR scan's warnings stop collapsing into a handful of rows** — the finding's SUBJECT (the log line, the file) is what tells two THOR rows apart, and it was being truncated in both places that decide identity: clipped to 90 characters in the description, and sliced off a fixed-width aggregation key. Two Defender detections sharing a 300-character prefix therefore became one aggregated row, and then one again in correlate — whose exact-duplicate step keys on the description. A scan reporting 40 warnings reached the timeline as 27, with nothing logged. The key now hashes the whole subject and rule, and the description keeps its distinguishing tail (the dashboard already truncates titles for display and shows the rest in [details]). Merging still deliberately does NOT add counts together: `count` is occurrences of the activity, not the number of rows reporting it, so two tools describing one event stay "×1" and a re-import cannot double a timeline.
- **THOR's scan-lifecycle notices keep the grade THOR gave them** — Init/Startup notices (the Lite licence banner) were graded Info to keep them out of the forensic timeline. It cost the reconciliation an analyst actually performs: THOR's summary reports 1 Alert / 40 Warnings / 2 Notices and the case showed no Medium at all. A timeline that disagrees with the scanner's own totals is worth less than one carrying two banners.
- **A THOR scan's findings stop collapsing into two timeline rows** — mapping each row correctly was not enough: correlate merges events that share a hash (step 1) or a path (step 2), and a THOR LogScan row carries two fields that belong to other things — `file` is the LOG the suspicious line was read from, which every hit in that log shares, and `sha256_1` is a file merely NAMED in the line. Handing either to the event as its own identity collapsed 18 distinct Windows Defender detections AND the mimikatz file finding into a single row, so a scan that mapped to 20 forensic events showed 2. Identity fields now describe the finding's own subject: a file finding keeps its path and hash, a process finding keeps its image, and a log-entry finding keeps neither — both still reach the analyst in the details panel, and the IOC scrape still harvests the hashes.
- **A THOR finding collected by a hunt now says WHAT it found** — the row read "Velociraptor [Generic.Scanner.ThorZIP/ThorResultsJson]: Possibly Dangerous file found": the artifact plumbing named twice, the finding not at all. Which file? Which log entry? Which rule fired? All of it was in the row and none of it was read — the generic mapper looks for `OSPath`/`Exe`/`Image` and THOR writes `file`, `entry`, `process_name`, `parent`, `reason_N`, `rulename_N`, so path, process, parent and the reasons were all dropped and the [details] panel had nothing to show. THOR rows are now described the THOR way — "THOR Warning [Filescan]: Possibly Dangerous file found — mimikatz.exe", with the full path, hashes, owner, score and every matched rule (with its own sub-score and reference) in [details] — and they carry path/hash/process/parent/ATT&CK through for correlation. The artifact label is dropped from the title, which already names the tool and the scanner module, and stays on `artifactName` for the origin facet. Findings are also keyed by SUBJECT as the native importer keys them, so 39 different suspicious log entries are 39 findings the analyst can read rather than one "×39" row that names none of them.
- **A collect no longer truncates evidence at the dashboard's row limit** — `DFIR_VELOCIRAPTOR_MAX_ROWS` caps what a result TABLE renders, and the hunt/flow collection reads used the same 1000. THOR opens a scan with ~1000 lines of module init and per-file progress and only then reports what it found, so a real scan was cut off mid-run: the 40 warnings it counted never left the server, and the collect reported success. Collection reads now use `DFIR_VELOCIRAPTOR_COLLECT_MAX_ROWS` (default 100000) — the same split the stdout byte cap already made — and an artifact that still hits it is recorded as `truncatedArtifacts` on the hunt job and named in the server log, so a PARTIAL collect can no longer look like a complete one.
- **THOR findings collected by a hunt are graded by THOR's own levels** — the generic Velociraptor severity words put Warning at Medium and Notice at Low, one step below the native THOR importer, so the same scan got two different verdicts depending on whether it arrived through a hunt or the import button. Alert/Warning/Notice now grade Critical/High/Medium and reach the forensic timeline, and Info stays Info in the super-timeline. Notice at Low used to sit exactly ON the default forensic floor, so raising the floor one step silently lost THOR notices. THOR's scan-lifecycle modules (Init/Startup/Control/ThorDB/Report) grade Info whatever level they carry, so a THOR Lite licence banner is kept as a record instead of landing in the forensic timeline as a Medium finding.
- **A hunt's THOR findings now reach the forensic timeline, not just the super-timeline** — the `Line` payload un-wrap that grades a THOR `Alert` as Critical never ran on a real hunt read. Velociraptor's `hunt_results()` stamps every row with the identity of the collection it came from, and `FlowId` was missing from the metadata set the un-wrap ignores — so the row presented TWO payload columns (`Line` plus `FlowId`), the un-wrap bailed out, and the finding stayed an opaque string. A 1000-row scan imported as 1000 undated `Info` events, the default Low forensic floor demoted every one of them, and the case showed a full super-timeline with an empty forensic timeline. `FlowId`/`SessionId`/`ClientName`/`OrgId` now count as collection metadata alongside `ClientId`/`Fqdn`/`_OrgId`, and the regression test uses the row shape a real hunt returns rather than a bare `{ Line }`.
- **A hunt's THOR results now reach the case** — three faults hid a completed scan, and each one reported success. Velociraptor stores a multi-source artifact's rows only under `Artifact/Source`: importing the collection dropped THOR outright, because `flows()` reports those sources one at a time (`Generic.Scanner.ThorZIP/ThorResultsJson`) and the artifact-name check rejected the slash — the whole scan was lost behind a per-artifact log line while the collection's 17 single-source artifacts imported normally. Collecting the hunt failed more quietly: the bare-name read returned zero rows and no error, so the artifact was recorded as having found nothing on every host. Both readers now merge the bare read with every named source the catalog reports, which also recovers the dozens of shipped bundle artifacts that declare an unnamed source ALONGSIDE named ones (`DetectRaptor.Windows.Registry.NetworkProvider` and friends), whose named sources were silently dropped because the bare read looked like a success. Finally, a source that streams a text file emits one row per line as an opaque `Line` string — THOR's findings arrived as undated, host-less `Info` rows that the forensic floor then discarded; the payload is now unwrapped, so an `Alert` grades Critical with its own timestamp, host, hash and file IOC. A source name is also free-form text rather than an identifier — 40 of the 180 source names in the shipped artifact set carry a space, a hyphen or a slash (`Recent Entries`, `Microsoft-Windows-Hyper-V-VMMS-Admin`, `Login/Logout Hooks`) — so source names are now judged by what a quoted VQL literal cannot hold rather than by the artifact charset that was silently discarding them.
- **A tool the analyst has uploaded is no longer treated as missing** — the bundle pre-flight judged tool availability from `artifact_definitions()`, which echoes the artifact's YAML: uploading THOR or re-pointing its URL under Server Artifacts → Tools changes nothing there, so the artifact stayed dropped and a THOR-only bundle was refused outright. Availability is now read from `inventory()`, the server's own tool state, with the declared URL as the fallback when that read fails. Only a recorded hash counts as "the server holds the file" — every inventory row carries a `filestore_path` and most carry a `serve_url`, including the unconfigured tool that failed the hunt.
- **A failed launch names the permission that call actually needs** — one shared message told every path to grant `START_HUNT`, which cannot fix a refused `collect_client()`. Per `acls/acls.go`, a bundle hunt needs `START_HUNT`, a single-client collection needs `COLLECT_CLIENT`, and the two paths that define a `Custom.*` artifact first also need `ARTIFACT_WRITER`.
- **One unconfigured tool no longer kills a whole triage bundle, and a refused hunt says why** — the built-in "Best Practice" bundle collected nothing on any server where THOR Lite had not been uploaded: Velociraptor resolves every artifact's third-party tools while it compiles the hunt request, `Generic.Scanner.ThorZIP` still carried the shipped placeholder URL `todo.thor-lite.zip.download.url`, that fetch failed, and `hunt()` then returned NULL for all 45 artifacts. The run-bundle pre-flight now drops an artifact whose tool the server cannot download (44 of 45 run) beside the existing does-it-exist check, and the dashboard says what was skipped. The failure was also undiagnosable: the CLI logs the reason and still exits 0 with an empty stderr, so every launch path guessed at ACLs — wrongly, since the api_client is usually an administrator and `hunt()` needs `START_HUNT`, not `COLLECT_CLIENT`/`ARTIFACT_WRITER`. The runner now runs verbosely and quotes Velociraptor's own sentence in the error.
- **An import's "+N events" counts only that import's own work** — the state snapshot an import is diffed against was taken when the request was accepted, not when its turn in the job queue came, so a file that waited diffed against the case as it stood before the files ahead of it landed and claimed their events: seventeen artifact files dropped at once made the last of them report the whole batch, "308 forensic events · 2 super-timeline events" for a two-row registry artifact — the super-timeline count being the honest one, since it counts rows actually inserted. The same snapshot backs the undo checkpoint, so undoing that one file would have reverted all seventeen. Every import path now takes a per-case import lock and snapshots inside it: both `/import` routes, the streamed ingest behind `/push` and the MCP tools, the Velociraptor monitors, hunt collects, and the external hunt/flow ingest. The job queue was never a substitute — `DFIR_JOBS_PER_CASE` above 1 makes admission stop meaning exclusivity, and the push/MCP/monitor paths never queued at all.
- **A Velociraptor hunt collect reports its super-timeline count, and waits its turn to write** — it snapshotted the case before its result fetches, which for a large hunt is minutes of network reads during which anything else that wrote was counted into the collect's "+N events" and swept into its undo checkpoint. It now claims the case's import slot after the fetches and snapshots inside it, so the exclusive window covers the writes and not the download. It also records `superTimelineAddedCount`, which this path never did: a super-only bundle collect used to read "0 forensic events · super-timeline count unavailable" on the cockpit card, which looks exactly like a collect that imported nothing.
- **A superseded background job disappears instead of reporting itself cancelled** — a multi-file import fires one re-synthesis per file and each kick supersedes the one before, but the superseded job was marked `cancelled`, the same status the analyst's own ✕ Cancel produces; an eleven-file import filled the jobs popover with eleven aborts nobody ordered. The row is now removed outright, from the queue and from the durable ledger, so a restart cannot bring it back offering Resume on work a newer run already took over. The popover also showed rows the badge did not count — it rendered the newest twelve, so a burst of finished rows pushed the still-queued work off the end and "⚙ 3 jobs" sat above a list with no queued row in it; every active job is now shown, with recent history filling whatever budget is left.
- **The Presidio approval buttons are one size on every row** — a long finding no longer squeezes them, and the value wraps instead.
- **Two timeout budgets that were still too small to survive the load they were raised for** — the login-limiter tests were given a 30s timeout directly beneath a comment recording a 31s worst case, so the flake outlived the fix; they now get 60s. And the drop-folder re-arm test makes two waits of up to 10s each while inheriting the suite's 15s default, so two healthy-but-slow sweeps would be killed partway through the second wait — its timeout is now derived from the wait budget rather than written down separately, which is the part that can drift.
- **Three tests that failed at random under a loaded parallel run** — the drop-folder watcher test waited a fixed number of event-loop turns for a sweep to finish, but the watcher only schedules its next timer after that sweep's async scan completes; when the budget ran out first, no timer was armed, the following clock advance fired nothing, and a sweep vanished. It now waits for the poller to actually re-arm. The two remaining offenders were simply out of headroom — the suite-wide spawn audit uses 7.7s of the 15s budget on an idle machine and the login-limiter tests 10.0s — so both now carry explicit 30s timeouts with the measured baseline recorded beside them.
- **The AI status pill is derived from the case, not remembered from the event stream** — `ai_status` is pushed from 155 call sites and the pill was its only consumer, so its state existed nowhere else and nothing ever reconciled it. Three bugs came out of that one gap: a wrong event, an absent event, and no event at all — the last being a page reload, which set the pill from `/health` (server-wide, and unable to know anything about a case) and so read "ready (waiting for activity)" on a case held at a gate, beside a cockpit reading "on hold". A new `GET /cases/:id/ai-state` derives the answer from pending gates, running jobs and the AI toggle; the dashboard reads it on case connect, on websocket reconnect, after every idle event, and after either gate is resolved. Pushed events remain the fast path for live progress. A hold is reported alongside the state rather than instead of it, so a running import still reads as work in progress with "(analysis on hold)" appended.
- **Clearing the last Presidio approval restarts the analysis it was holding** — approving or suppressing the final pending finding removed the chip but kicked nothing, so the header pill sat on "AI: on hold — Presidio…" indefinitely and the analyst had to know to press Re-synthesize. Both resolve paths now kick the held run once the pending list empties, matching the duplicate-host gate. Only on the last one: kicking per approval would spend a run per finding, and every run but the last would stop again on what is still pending.
- **Turning AI on for a text-only install now synthesizes the imported evidence** — an OCR-less deployment (only `DFIR_AI_SYNTH_PROVIDER` set, no vision model) hit a guard that asked whether a VISION model was configured, answered "AI on — no AI model configured" — which was false — and returned. No synthesis was ever scheduled, so switching AI on for an import-only case did nothing at all. "No AI model configured" now means neither model, and an install with a text model but no vision model skips the screenshot catch-up (nothing could have produced screenshots) and goes straight to synthesis.
- **The AI status pill no longer claims to be synthesizing while the case is held** — turning AI on for a case with an unresolved duplicate pair showed "AI: synthesizing …" in the header while the Now cockpit showed "AI analysis is on hold", because both synthesis paths announce optimistically (the debounce is 8s, and the alternative was 8s of silence) and neither consulted the gate before announcing. A held case now reports `blocked` as its first word on the subject, registers no synthesis job, and never reaches the model. A case with no pending pair announces and runs exactly as before.
- **The duplicate-host merge decision is reachable, and no longer reported as an AI failure** — the gate stopped synthesis correctly but left the analyst nowhere to go: the Now cockpit lists only its own section and hid the Duplicate Hosts panel outright, and the header chip scrolled to a `display:none` element, which does nothing. The panel is now data-gated — invisible until a pair is pending, shown the moment one is, whichever view is active — the chip reveals it, and the cockpit carries its own blocker card whose Open button lands on the merge buttons without leaving the cockpit.
- **A gate is no longer painted as a crash** — an unresolved pair (and a pending Presidio approval) reported through `ai_status: "error"` and a `failed` job, so the header pill turned red and the cockpit said "synthesis failed" for a run that was never allowed to start. Both now report a new `blocked` status — an amber "AI: on hold — …" — and cancel the job instead of failing it. Fixed on all five paths that call `synthesize()`: the manual Re-synthesize button, second opinion, deep pass, live auto-synthesis and the post-import background run. A genuine failure is still an error.
- **A host spelled two ways no longer inflates a finding's corroboration** — `Finding.corroboration.distinctHosts` counted the raw `asset` string, so a finding supported only by `WIN11` and `WIN11.windomain.local` reported evidence on two hosts. That defeated the single-source confidence cap and let an uncorroborated finding keep a confidence it had not earned. The same split also doubled an indicator's cross-host reach, promoting it up the connective-indicators block on breadth it did not have. Conclusions drawn on a case with a split host before this are worth re-reading (#572).
- **A timed-out Presidio scan is no longer retried** — retrying made it strictly worse: aborting a request does not cancel the analyzer's work, so each attempt queued behind the scan just abandoned, and at four attempts on the new 60s budget an analyst waited ~240s to be told what the first attempt already knew. Other scan failures (a refused connection) are still retried.
- **Presidio scans no longer miss PII split across a chunk edge** — a line longer than the 50,000-character chunk had no line boundary to split on and was cut mid-value, so a name straddling the cut matched nothing in either half and the intact value went to the model with the scan reporting clean. Chunks now overlap by 256 characters after a mid-line cut.
- **A very large `DFIR_PRESIDIO_TIMEOUT_MS` no longer means "fail instantly"** — values above Node's 32-bit timer ceiling silently became 1ms, so raising the budget to tolerate a slow analyzer aborted every scan immediately. Clamped to the largest delay a timer can hold.
- **A Presidio timeout no longer claims the analyzer is reachable** — the same budget expires whether it is busy or the connection is hanging, and the error now names both instead of asserting a fact it cannot know.
- **Presidio scans no longer time out against a healthy analyzer** — the per-request budget is now 60s (was 10s, tunable with `DFIR_PRESIDIO_TIMEOUT_MS`) and the AI-call gate splits the prompt into 50,000-character chunks like the import pre-scan already did, instead of sending the whole thing as one request. The old budget covered an idle analyzer but not a queued one: the stock image runs a single worker, where a chunk taking ~1.7s alone takes ~9.6s with five scans in flight. Because an aborted fetch does not cancel the analyzer's work, each retry queued behind the abandoned scan, so a fail-closed layer took the whole AI stack down while the container sat there healthy.
- **A Presidio timeout says it timed out** — the error now names the budget, the payload size and `DFIR_PRESIDIO_TIMEOUT_MS`, instead of reporting "not reachable … This operation was aborted" and sending the analyst to restart a running container.
- **Cancelling a case load retires the case, not just its fetches** — the abort controller only ever covered fetches, so the WebSocket and the 5-second capture-count poll survived cancellation: the socket kept pushing `state` (which rendered, because the active case still named the cancelled one) plus ~25 other message types that each re-fetch a panel, and a return to the tab resurrected the poller from its stale closure. The socket is closed with its handlers detached, the poller forgets the case rather than pausing, and `render()`'s stale guard now rejects a case-scoped state when no case is active instead of falling through the null.
- **Cancelling a case load no longer accuses a healthy server** — an abandoned panel request used to reach the loaders' `.catch` handlers, ~73 of which report a missing endpoint on the status line and several of which tell the analyst to restart the companion server. An abort is now hidden from the loader entirely; a genuine network failure or 5xx still reports exactly as before.
- **The panel-load cap now counts connections, not loaders** — a lane is taken per REQUEST and held until the response body has been read, rather than per loader and released when the headers land. Counting loaders let one that fans out internally (`loadVeloTriage` issues four requests, `loadMcpRun` three) put its whole burst on the pool under a single slot, and a header-release admitted the next request against a connection whose body was still streaming. Measured warm on the same 82 MB case: the "+ New case" request waits 108ms → 15ms, `/health` 90ms → 4ms, median queue wait 23ms → 14ms, and the whole fan-out finishes 17.7s → 14.5s, with all 60 panels still loading.
- **A loading case no longer locks the dashboard** — the ~60 secondary panel loaders now run on the case's abort generation and at most four at a time, so cancelling or switching cases actually stops them and two of the browser's six HTTP/1.1 connections stay free. Opening a large case used to queue 86 requests behind a six-connection pool, leaving "+ New case" and connecting to another case stalled for ~8s on routes the server answered in 2ms.
- **The case-loading overlay can be cancelled** — a Cancel button from the first frame, instead of a 15-second stall timer that turned the backdrop into the only way out. It abandons the whole load (state, lifecycle and panels) and clears `?caseId=` so a refresh does not walk straight back into it.
- **The dashboard no longer opens the last case by itself** — a bare `/dashboard` leaves the choice to the analyst and pre-fills the picker with the remembered case; an explicit `?caseId=` still opens, so deep links, bookmarks and refresh-in-a-case are unchanged.
- **A deleted case's background jobs no longer haunt the next case** — the job table is keyed by case id alone and a delete never cleared it, so re-creating a case with the same id inherited its predecessor's jobs, each still offering a Resume that would replay the dead case's import into the live one. Delete now aborts and forgets that case's jobs, and retires the id so the new-case wizard (which now asks the server, and so can also see archived cases) never reissues a number that has been used.
...
- **Panels that could not be switched on** — six sections (Scope & Clearance, GeoIP Map, Hunting Profile, Suggested Fleet Hunts, MCP Analysis, Memory Next Steps) were offered as checkboxes but missing from the view section registry, so a save silently dropped them and every reload wiped the choice again. The registry is now derived from the page and asserted both ways, so a new panel cannot ship unsaveable.
...
[Unreleased]: https://github.com/hasamba/DFIR-Companion/compare/v0.34.0...HEAD
