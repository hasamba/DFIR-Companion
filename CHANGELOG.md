...
### Added
- **Self-signed IRIS is configurable from Essential settings and the setup wizard** — Skip TLS verify (`DFIR_IRIS_INSECURE`) now shows in the Essential view and on the wizard's DFIR-IRIS step, alongside a new global "Allow insecure TLS to external hosts" field (`DFIR_TLS_ALLOW_INSECURE_EXTERNAL`, default false) that the non-loopback MITM guard already required but no UI offered. The new key saves and reloads live, so the wizard's connection test and any reconnect pick it up without a restart.
