...
### Fixed
- **A markdown-style link in evidence text yields both of its URLs again** — `[http://a](http://b)`, routine in chat exports, notes and markdown files imported as evidence, matched as ONE indicator: the unresolvable `http://a](http://b)`, with the link's real target never harvested at all. Every free-text URL scraper now stops at the `](`, so both URLs become indicators, and an IPv6 authority keeps the bracket it requires. (closes #755)
- **One C2 URL in a Windows script block is the same indicator as in a Velociraptor row** — the SIEM/EDR free-text scraper kept a private trailing-punctuation rule that had drifted from the shared one. It cut the final dot off a quoted URL and stripped a balanced `)` the path itself opened, so the same URL became two different IOCs depending on which tool collected the evidence. It now uses the shared rule, and a new cross-scraper test holds all four scrapers to the same answer. (closes #756)
