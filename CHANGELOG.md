...
- **A macOS quarantine record says what it establishes** — an LSQuarantine row reads as a download EVENT: the kind (web download, email attachment, …), the agent, the downloaded resource and the referring origin as two URLs, the sender, the event identifier — and never the local file, which the database does not name. The native Cocoa timestamp is decoded as such (a converted dump declares its epoch on import; nothing is guessed from magnitude); the `com.apple.quarantine` xattr is decoded by its own form (flags per Apple's SPI, Unix hex time, agent, event id) when a persistence collection carries it. A non-http origin mints no indicator; a sender never does (part of #933)
