...
### Added
- **Sort timeline by date or severity** — a **Sort** control in the Forensic Timeline header toggles the row order between event-time (chronological, default) and severity (most-severe-first, chronological tie-break); client-side only, persisted across reloads, no server round-trip (closes #104).
