...
### Added
- **Chain-of-Thought synthesis (extended thinking)** — opt-in `DFIR_AI_SYNTH_THINKING_TOKENS` (≥1024) lets the model reason step-by-step before writing findings on complex cases; maps to Anthropic extended thinking and OpenRouter's unified `reasoning` (other providers ignore it), applies to the synthesis call only, and leaves prompt-caching/OPSEC invariants intact (#121, feature 1).
