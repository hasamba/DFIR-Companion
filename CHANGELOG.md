...
### Added
- **Adversary emulation — likely next techniques** — from the matched ATT&CK groups, surfaces the techniques they're known to use that the case *hasn't* observed yet (base-level), ranked by **distinctiveness** (TF-IDF: matched-group consensus × global rarity) with ubiquitous tradecraft dropped above a prevalence cap, so it points at telling tradecraft (e.g. email/clipboard collection, web-service C2) not "everyone does recon"; predictive hunt priorities, still hypothesis fuel not attribution; offline/deterministic, `DFIR_ADVERSARY_NEXT_MAX` (default 10) / `DFIR_ADVERSARY_NEXT_MAX_PREVALENCE` (default 0.33), in the dashboard *Adversary Hints* panel + report §4.6.1 (closes #121).
