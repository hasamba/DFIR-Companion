...
### Security
- **Web pages can no longer drive the companion API** (#211) — the localhost API answered every origin with `Access-Control-Allow-Origin: *` plus a private-network opt-in, and custom-tool registration sits outside the per-case password gate, so any page you happened to be browsing could register a custom tool and then trigger its update command, which the tool runner spawns as your user. Binding to `127.0.0.1` never prevented this: it stops other machines, not other origins. The API now trusts the capture extension, loopback, the host the dashboard was served from, and anything in the new `DFIR_ALLOWED_ORIGINS`; every other browser origin gets a 403 with no CORS headers, so the preflight fails too. Callers that send no `Origin` (curl, scripted pushes, Velociraptor) are unaffected, and localhost/LAN/Docker setups need no configuration. A tool's update command also now requires the case to exist before it spawns anything — the password gate deliberately passes unknown case ids through, which had left the process-spawning route reachable without naming a real case.
- **Live WebSocket subscriptions now honour the case password** (#212) — subscribing to `/ws` hands a socket every future broadcast of that case's full investigation state, and the handler took `?caseId=` on faith: Express middleware never runs for a WebSocket upgrade, so the password gate did not apply. Anyone who knew a locked case's id received its contents on the next broadcast. Upgrades are now authorized before the handshake completes — trusted origin, existing case, and a valid unlock cookie when the case has a password.
- **Template, view and bundle ids can no longer escape their folder** (#213) — four global stores concatenated a caller-supplied id into a filesystem path, so a traversal component read, overwrote, or deleted `.json` files anywhere on disk; combined with the wildcard CORS policy above, the read was exfiltratable to a web page. Ids are now checked against a strict allowlist and every resolved path is verified to stay inside its store.
- **A malicious IP in imported evidence can no longer run code in the dashboard** (#217) — event IP values were interpolated into a single-quoted inline `onclick`, and the attribute escaper did not escape apostrophes, so an IP-shaped string from an imported log or model output could close the string and execute when an analyst clicked the event's map button. The IP now travels as a data attribute read back by a delegated listener, and the escaper handles both quote flavours, which also hardens the other inline handlers that use it.

### Fixed
- **One unsendable capture no longer blocks every later one** (#215) — the extension's offline queue treated every failure as retryable and stopped at the first one. A capture queued during an outage can later get a permanent answer (404 the case was deleted, 423 it was closed, 410 removed), and that entry then sat at the head of the queue forever: every valid capture behind it was never uploaded, with nothing to indicate why. The queue now distinguishes transient from permanent: a transient failure still stops the drain and preserves order, while a permanently-rejected capture is discarded so the rest can go through — and, because discarding it loses evidence, the popup diagnostic says how many were dropped and with which status, and the toolbar badge turns amber.
- **The extension can be type-checked again, and CI now does it** (#218) — `tsc --noEmit` failed on a clean install before reaching any project code (`TS2688`: the tsconfig asked for `node` types that were not a declared dependency), while Vite and Vitest kept passing because both only strip types. Type errors outside an exercised test path could therefore reach the build unnoticed — and three real ones had: a `Uint8Array`/`BlobPart` mismatch in artifact extraction, a browser-vs-Node timer type, and an under-typed fetch mock. `@types/node` is now a direct dependency, the three errors are fixed, and CI runs a standalone `npm run typecheck` that fails the build on any TypeScript error.
- **Concurrent ingestion no longer overwrites evidence** (#214) — capture and import sequence numbers were derived from "audit-log length + 1" with no reservation, and the audit line is appended only *after* the evidence file is written. Two ingestions arriving together therefore read the same length and took the same number; when their derived filenames also matched, one evidence file silently replaced the other while both audit records were written — evidence destroyed and provenance broken, with nothing in the log to show it. Measured before the fix: ten concurrent captures were all assigned sequence number 1, leaving one file on disk. Allocation is now serialized per case and tracks the highest number handed out, so a number is never reused (only ever skipped, which is the safe direction). Evidence writes are additionally create-exclusive, so a name collision raises instead of overwriting a file that already exists.
- **Comments, tags, notebook entries and pins no longer vanish when analysts work at the same time** (#216) — these stores each load a whole JSON collection, modify it and save it back, with nothing serializing that sequence. Two simultaneous requests both read the same snapshot and the second save discarded the first. Measured before the fix: of twelve comments posted at once, **one** survived; a delete racing an add lost the added comment entirely. Each store now serializes its own per-case critical section, which also makes the previously unreliable guarantees truthful — the tag de-duplication check, the pin cap, and the "already pinned" check could all be defeated by two requests racing.
