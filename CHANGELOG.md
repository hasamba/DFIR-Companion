...
### Fixed
- **Setting a case password silently kept the setting browser unlocked forever** — the browser that just set/changed a password was auto-granted a session cookie, so it never actually got prompted for that password again unless a completely different browser opened the case; a page refresh, a server restart, or switching cases and back all showed the unchanged (unlocked) data. Setting or changing a password no longer auto-unlocks the browser that did it — it must unlock the same way anyone else does, so the password takes effect immediately.
