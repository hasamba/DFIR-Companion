...
### Security
- **A coding-agent run can no longer grow its output buffer without end** — the Claude Code and Codex CLI runners retained every byte of stdout and stderr until the child exited, so a looping agent run over evidence it does not control could exhaust the Node heap. Both streams now keep a bounded tail by default, and a caller that needs the whole stream asks for it explicitly. (closes #762, closes #763)
