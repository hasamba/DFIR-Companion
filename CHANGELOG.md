...
- **Attacker session segmentation** (part of #229) — `GET /cases/:id/sessions` splits the forensic timeline into per-host attacker sessions: a run of events on one host with no gap larger than 5 minutes (override with `DFIR_SESSION_GAP_S`). Each session reports its dominant ATT&CK tactic, event count, time span, and severity range under an auto-generated label. Pure and deterministic — no AI call. Events whose host was never recorded are bucketed under an explicit "(unknown host)" rather than dropped. API only for now; the story view, per-session summaries, and report integration from #229 are still to come.
