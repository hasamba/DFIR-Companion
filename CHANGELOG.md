...
### Fixed
- **A raised exfil or initial-access event always states its reason** — the base description is clipped and the marker sentence appended after it, so a long description cannot push the marker off the end; the dead `.slice` on the exfil marker is gone (closes #939)
- **Cloud keys that end in a resource are bounded, not sliced** — the M365 file-read, Entra sign-in and GCP keys go through `boundedAggKey`, so two rows sharing a 400-character prefix stay two rows (closes #940)
