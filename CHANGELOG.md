...
### Added
- **Import from DFIR-IRIS** — pull an existing IRIS case's assets/IOCs/timeline into a Companion case (the reverse of the push); deterministic `importIris` / `irisImport.ts` maps timeline events (severity from colour, MITRE/asset/hash from tags+content), IOCs (type from the IRIS ioc-type or value shape), and assets → evidence events; one **"Import case…"** dashboard menu (Investigation snapshot or From DFIR-IRIS) + `npm run iris:import`; **Settings → DFIR-IRIS "Test / reconnect"** rebuilds the client from `.env` and pings so config (or IRIS coming back online) applies without a restart (closes #88).
...
- **Velociraptor live monitoring** — stream a CLIENT_EVENT artifact (e.g. `Windows.Events.ProcessCreation`) from one endpoint into a case as events fire; server-side poller (`DFIR_VELO_MONITOR_POLL_S`, default 30s) with a persisted cursor (`state/velo-monitor.json`) so a restart never re-ingests; Settings → Velociraptor → Live Monitoring + a 🔴 LIVE dashboard badge (closes #84).
