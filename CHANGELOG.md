...
### Added
- **Chainsaw importer: flat Sigma-mapping JSON** — recognizes the flattened Chainsaw output shape seen from tools that shell out to Chainsaw and stream rows back (e.g. a Velociraptor artifact), with the verdict (`Detection`/`Severity`/`Rule Group`) at the top level instead of a nested `rule{}` object; reuses the same per-EID Windows mapping as the existing nested-document Chainsaw format. `velociraptorImport.ts` recognizes the same shape too (a Velociraptor artifact that shells out to Chainsaw), so pulling it straight from a Velociraptor hunt/flow no longer misreads it as a generic DetectRaptor verdict and silently downgrades a real Critical to a keyword-guessed Medium.
- **Severity-gated forensic timeline** — Info telemetry routed to the super-timeline only (the forensic timeline keeps Low+), configurable via `DFIR_FORENSIC_MIN_SEVERITY` + a per-case override, so synthesis isn't swamped by raw telemetry; promotion bypasses the gate and IOCs are still extracted from every event.
- **IOC provenance** — each IOC classed detection-linked (seen in a Low+ event) vs telemetry-only (Info only), derived on read and distinct from the threat-intel verdict, with a dashboard badge + All/Detection-linked/Telemetry-only filter.
- **Super-Timeline full message + Velociraptor deep-link** — each super-timeline row can now expand a `[details ▶]` toggle to reveal the full untruncated event message (e.g. the raw EVTX rendered Message / ScriptBlock text) beyond the truncated summary, and a Velociraptor-sourced row shows a `↗ Velociraptor` link back to the originating hunt/flow in the Velociraptor GUI (closes #8, closes #9).
- **Super-Timeline** — a Timesketch-style complete record of every imported event (filter by time/origin/label, save timeframes, label + promote events to the forensic timeline for AI synthesis) + a "Super-Timeline Triage" Velociraptor bundle that collects raw host artifacts (MFT/USN/registry/execution) into the super-timeline only, keeping the forensic timeline detections-focused.
...
- **Import external hunt/flow** — pull results from a Velociraptor hunt or collection launched in the Velociraptor GUI (paste its id or URL); a flow's host is resolved automatically, with an optional super-timeline-only route.
...
- **IOC list noise-reduction controls (dashboard)** — a case can accumulate thousands of IOCs (every file/process/hash observed anywhere is one). Three independent, composable display-only filters (nothing is deleted), all default ON and persisted per-browser: **"Hide FP/no-intel"** checkbox drops IOCs marked as a false positive or checked against threat intel with no result; **"Hide OS system paths"** checkbox drops `file` IOCs under well-known Windows/Linux/macOS system-binary dirs (`C:\Windows\System32\`, `/usr/bin/`, …); **"🎯 Signal only"** button narrows further to IOCs that are flagged (malicious/suspicious), corroborated by 2+ tools, or carry any enrichment data. The header shows "N of M IOCs shown" when a filter is active.
- **Exclude filter (dashboard)** — next to the toolbar search bar, a chip-list control hides timeline events / IOCs / findings matching ANY of several exclude terms (each term can be a multi-word phrase, added via Enter/comma). Case-insensitive substring match, mirrors the existing include search; view-only, persisted per-browser via `localStorage` (closes #216).
- **Velociraptor deep-link on the forensic timeline** — a `↗ Velociraptor` row link back to the originating hunt/flow, mirroring the super-timeline.
- **`Windows.EventLogs.Chainsaw` added to the built-in Best Practice bundle** — runs Chainsaw against EVTX via Velociraptor for Sigma-mapped detections, alongside the existing Hayabusa artifact.
- **Origins filter on the forensic timeline** — a new "⛏ Origins" dropdown filters by the artifact that produced each event (e.g. `DetectRaptor.Windows.Detection.MFT`), one level more specific than the existing Sources (tool) filter, matching the super-timeline's Origins filter.
- **False-positive feedback loop** — renamed "Mark Legitimate" to "Mark False Positive" with a structured reason (known-good tool / authorized test / detection misfire / duplicate / other) and analyst attribution, plus a deterministic + optional-AI "find similar items" suggestion step at mark-time and a one-click promotion to the IOC whitelist (closes #227).

### Changed
- **Forensic + super-timeline row redesign** — both timelines now render a compact, plain-weight (not bold) title line; the full description, MITRE, related findings, decoded payloads, evidence, and the raw tool message all collapse into one shared `[details ▶]` panel per row instead of always-on inline text, and the super-timeline's disclosure (previously shown only when the raw message differed from the description) now broadens to any row carrying extra context. The row title itself is now just the finding name — a raw technical trailer many importers append after " - " (EID/Image/CommandLine/etc.) moves into the details panel too — and the `[details ▶]` toggle sits inline after the host chip instead of on its own line.
- **Super-timeline toolbar merged** — the time-scope bar and the origins/hosts/tags/starred-only bar are now one row with a divider, instead of two stacked bars.
- **Forensic timeline severity filter drops "Info"** — the checkbox is removed since `forensicGate` already keeps Info-severity events out of the forensic timeline server-side (they live in the super-timeline instead).
- **Forensic timeline pagination moved above the event rows** — the Prev/Next bar now sits right below the header row instead of at the bottom of the list.

### Fixed
- **Correlated events could show a different tool's artifact as their origin** — `correlateEvents`'s merge took the description from the worst-severity/longest-description contributor but let `artifactName` fall back to *any* contributing event, so a genuine cross-tool correlation (e.g. a Chainsaw/Sigma detection merged with a Velociraptor Pstree record sharing host+pid) could show the Pstree artifact as the origin of a Chainsaw finding. `artifactName` now always comes from the same event as the shown description.
- **Dashboard main filter now applies to the super-timeline** — the toolbar search term, exclude terms, and time range previously only scoped the forensic timeline/IOCs/findings; the super-timeline query route gained `q=`/`excludeText=` params and the dashboard intersects its own time range with the main filter's.
- **IOC panel header always showed only the flagged count** — now shows "N of M IOCs" when a filter narrows the list (matching the forensic/super-timeline badges), with the flagged count appended.
- **Super-timeline rows showing the tool/origin name twice** — several importers (Volatility/Rekall, Falcon Sandbox, …) prefix the description with the same tool name already shown in the row's origin column; the duplicate leading prefix is now stripped, including the bracketed `"Velociraptor [artifact] …"` shape the Velociraptor/DetectRaptor importer itself produces and Chainsaw's own slash-separated `"Chainsaw/Sigma: …"` shape.
- **Forensic timeline's Velociraptor deep-link never appeared** — `pipeline.importVelociraptor` never accepted a `veloUrl`, so the two callers that know the hunt/flow's GUI URL (external hunt/flow import, triage-bundle collection) only stamped it on the super-timeline-only branch; a normal (forensic-timeline-bound) import or bundle collection never carried one.
- **"Import external hunt/flow" pasting a notebook URL silently imported the flow's complete raw rows** — a notebook URL shows the analyst's own filtered VQL query results, which this server-side VQL/gRPC client has no way to re-run; it's now rejected with a message pointing at the browser extension's "Push rows" button, which reads the GUI's own rendered (filtered) table instead.
- **Duplicate clear (✕) icon in the filter / OCR search boxes** — both inputs used `type="search"`, so the browser's own native clear icon rendered on top of the custom clear button; switched to `type="text"`.
- **`cleanIp` accepted any colon-bearing free text as a "valid" IPv6 IOC** — the IPv6 branch only checked `value.includes(":")`, so a large free-text field (e.g. a PowerShell cmdletization proxy-function dump, full of `::new(...)`, `cim:ModifyInstance`, etc.) reaching a loosely-`ip`/`addr`-named key was accepted whole as a bogus IPv6 IOC. Now validated against a real IPv6 shape (full + all `::`-compressed forms) in `siemImport.ts`, shared by every importer.
- **Velociraptor "grpc: received message larger than max" — corrected fix (2nd pass)** — an earlier `[Unreleased]` entry had every spawned query pass `--max_message_size`; that CLI flag does not exist on at least some Velociraptor builds (`unknown long flag '--max_message_size'`), broke ALL queries including the startup client-inventory refresh, and was reverted. A follow-up entry then blamed the SERVER's `Frontend.resources.max_upload_size` — also wrong (a different data path, HTTP client uploads, not this gRPC connection). The actual setting, confirmed from Velociraptor's own source (`config/proto/config.proto`, `ApiClientConfig.max_grpc_recv_size`, "4mb by default"), is a field in the CLIENT-side `api_client.yaml` file `DFIR_VELOCIRAPTOR_API_CONFIG` already points to — add a top-level `max_grpc_recv_size: 67108864` (or larger) to that file. No CLI flag, no server restart. The in-app error message now points there, and a persistent note explaining it lives in Settings → Velociraptor (dashboard) so it's discoverable before hitting the error.
- **IOC list: duplicate rows and unstable order in the dashboard** — the IOC panel now dedupes by id and sorts deterministically (type, then value) instead of raw insertion order, so duplicates can't render as separate unsorted rows. Root cause: concurrent imports racing on the same case's state can each read a stale snapshot before either saves and independently assign the same next-sequential IOC id; `mergeDelta`'s own dedup is also now case-insensitive (a hostname/domain routinely arrives with different casing across importers). A `npm run dedupe-iocs -- <caseId> [--apply]` script cleans up already-corrupted case files (mirrors `clean-timeline`). Full concurrency-safe state writes (a per-case lock) is tracked as a follow-up.
- **Velociraptor bundle-hunt collection surfaces per-artifact failures instead of silently dropping them** — a multi-artifact bundle hunt (e.g. Best Practice) fetches each artifact's results independently; an artifact whose fetch fails (oversized output, timeout) previously vanished with only a server-log line, which could read as "the import only picked up one artifact" with no way to diagnose why. The hunt-job record now carries `skippedArtifacts` (name + failure reason) and `emptyArtifacts` (fetched cleanly, nothing to report), and the dashboard's bundle-hunt card shows an "N/M artifacts returned results" line plus the failure reasons.
- **Velociraptor DetectRaptor hits on `.yms` Sigma-rule files graded Info, not extracted as IOCs** — a keyword match on a compiled Sigma signature filename (e.g. an MFT scan turning up `..._mimikatz_....yms`) is a hit on the rule's own name, not attacker content: no longer escalates to High/Critical via the tool-keyword table, and the `.yms` path itself is no longer added as a file IOC.
