...
### Fixed
- **The filter box now visibly reaches the super-timeline** — the panel says "Loading super-timeline…" while the query runs, instead of showing the previous unfiltered rows in silence for the seconds a full-store scan takes, and only the newest request may paint: an unfiltered load still in flight when the analyst types can no longer land second and repaint every event over the filtered answer.
