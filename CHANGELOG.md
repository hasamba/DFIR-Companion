...
### Fixed
- **A row promoted from the super-timeline now stays in the forensic timeline** — the next import's demote pass removed every promoted row again (it is still Info, so it failed the severity cut), silently undoing Super-Timeline promotes, explain-event, starred-report and second-look promotions. Promoted rows carry `promotedAt` and the forensic gate never demotes them.
- **A fleet hunt no longer folds the same finding on WKSTN-01..WKSTN-50 into one row on the first host** — the Velociraptor (usn, mft, pslist, netstat, generic artifact), Hayabusa and generic SIEM aggregation keys stripped digits from the host (and Hayabusa's event id) before comparing, so hosts that differ only by a number, and 4624 vs 4625 on one host, became one counted row; only the volatile row text is normalized now.
- **Marking an IOC false-positive is reversible again** — the next synthesis deleted a false-positive IOC from the persisted case (with its provenance), so un-marking could not bring it back; an IOC already in the case is kept and hidden by every reader, and only a model re-introduction is still dropped.
- **Hayabusa's `emergency` level (abbreviated `emer`) maps to Critical** — it was unmapped and fell back to Medium, so an import with the recommended "High" severity floor dropped Hayabusa's highest-level detections.
- **The extension's hotkey and popup captures are accepted** — the server's capture schema rejected the `manual` trigger type the extension sends for the Ctrl+Shift+S toggle-on capture and the popup's one-off capture (400 "invalid payload").
