...
### Security
- **The AI can no longer sign its own findings as deterministic** — the synthesis prompt shows the model every existing finding id, the automatic backfills' ids included, and nothing stopped it from sending one back. A finding wearing a borrowed id took the real detection's place: the matching backfill saw its work as already done and skipped, so the genuine "activity came in waves" or "log coverage gap" finding was never created — and the impostor survived every narrowed-scope run as if the tool itself had produced it. Such an id is now renamed before the response is read. (closes #787)
