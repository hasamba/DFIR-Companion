...
- **Missed evidence review** — an analyst-pressed pass grades the archive rows the content tagger left at Info with a fast decision model (Jev), ranked with the model's own confidence and a filter for the collection tooling's own binaries and rule files; it promotes nothing, writes no case state, masks through the same gate the chat models use, and is off until `DFIR_JEV_ENABLED` is set (closes #1540)
- **A row a rule set aside on purpose is the last thing the raw record forgets** — the super-timeline's cap evicts ordinary telemetry first and a row the collector-footprint, first-party-egress or build-time rule deliberately graded Info behind it (bounded at half the cap so it can never crowd out new evidence), and the import card now says how many raw rows the cap dropped, how many were set aside and the span they covered (closes #1535)
- **Discovery commands in a script block become evidence** — `nltest`, `Get-AD*`, `Get-ADGroupMember`, `gpresult`, `Get-Process`, `ntdsutil … ifm`, a psexec-style named pipe and PowerShell remoting are read out of a logged 4104/4103 record, stamped with their ATT&CK techniques at import, carried to the AI as `<script-commands:>` / `<script-techniques:>` tags the 240-char cut cannot reach, and raised as a Medium "commands present in logged script content" finding when no other finding accounts for them; the case's own collector rows are never read (closes #1531)
...
- **Re-synthesize on an empty case** — says "nothing to synthesize" instead of a false run, and clears "conclusions out of date" when the case has no findings (closes #1676)
...
- **Copied binaries are not timestomping** — the MFT note leads with "copied file, not timestomp", and copies sharing one source modified time are marked as one source file (closes #1558)
...
- **OneDrive's update traffic is not a C2 channel** — a first-party Windows updater (OneDrive, Edge update, Defender) connecting on 80/443 to a Microsoft service range nobody can rent grades Info with the reason in the row, so it stays in the super-timeline instead of being fused into a C2 finding; an address in that range is recorded with a `vendor:` note instead of as a candidate-C2 `ip` indicator, while a CDN edge (Akamai, Cloudflare) is named and stays a full indicator; loopback written the long way (`0:0:0:0:0:0:0:1`) and a module version (`1.0.0.0`) no longer become indicators; a network row now carries its image and destination as prompt tags the 240-character cut cannot reach (closes #1530)
...
- **A rename learned by one file reaches the whole case** — the case keeps a ledger of every hostname rename its Windows-log imports find (and every collector identity, which is never folded); later Chainsaw / Hayabusa / Velociraptor files that name only the old name fold on import, and rows imported earlier are re-homed at the next settle — recomputed from the name each record wrote, so a later contradiction unfolds them (closes #1495)
...
- **The case's own PersistenceSniper hunt is not a Mimikatz finding** — the PowerShell the Velociraptor client spawns to run an artifact (collector parent + a Tools-tree module on the command line), and that module's 4103 / 800 pipeline records (SYSTEM + the engine-written script file under the Tools tree), grade Info with the collector-footprint note; every collector-graded row now carries `origin: collector`, which the post-import tagger honours instead of raising it back to High on the same `Add-Type … AdjPriv` text (closes #1477)
- **Hunt import keeps every distinct file and launch** — correlation no longer folds renamed copies of one binary (one hash, several paths) or several launches of one binary (one path, several command lines) into one row; Hayabusa hunt rows are parsed for pid, command line and record id; the rename fact rides along any merge (closes #1476)
- **A mentioned indicator earns no risk points from the event that mentions it** — a hash or address read out of a script or command line no longer scores "seen in a High event" or "observed by N tools" from that same text; the factors read *mentioned in* / *referenced in events from N tools*, and the dashboard corroboration badge and lenses no longer count a mention as corroboration (closes #1474)
- **The dashboard keeps updating during a big import** — case reads run on their own read-only workers and the case database uses WAL, so a panel or poll no longer waits behind the import's write batches; a read still sees every write posted before it, a restore drains the readers first, and both archive writers package a consistent snapshot instead of the live file (closes #1454)
- **Loopback is never the collector server, and real network records keep their solid mark** — a Companion that runs on the Velociraptor server (`localhost` / `127.0.0.1` in Settings) no longer demotes every loopback connection on every host; only the system `msiexec.exe` counts as the collector install; the collector rules read Hayabusa rows; Cyber Triage active connections and Plaso browser history are *observed*, not *mentioned*; a mentioned IP/domain/URL never counts as behaviourally corroborated in the synthesis context or the risk score; and the dashboard IOC table, Notion, MISP and IRIS exports show the `mentioned` mark (closes #1471)
- **Deep reasoning works on Claude Code, is recorded, and is greyed out where it cannot work** — the 🧠 checkbox now maps its budget onto the Claude Code CLI's effort level (8000 → high; the env budget picks the tier); every synthesis run records `thinkingTokens` as a number (the run manifest had redacted it as a secret because the key contains "token") plus `thinkingSource` (toggle / env / off); `/health` says whether the configured provider supports it and the dashboard disables the box with an honest tooltip otherwise (closes #1468)
- **A hash inside a logged script is *mentioned*, not observed** — an indicator scraped from free text (script block, command line, message) carries a `mentioned` mark; a hash or address read from a structured column or a network record stays plain and wins when both arrive; a mentioned hash never counts as a file on the host (risk score, synthesis context, STIX label say so) (closes #1459)
- **An address a process was told about is *referenced*, not contacted** — a mentioned IP/domain/URL draws no geo flow line, its asset-graph edge is dashed and labelled *referenced*, and every network surface (reports, dashboard, connective-indicator digest) says `referenced in free text; no network record` (closes #1461)
- **The case's own Velociraptor collector is not evidence** — a download from the configured Velociraptor server, the client's MSI install and service, tools it runs from its install root, and an `msiexec.exe` creation-time change (an MSI artifact, never T1070.006) are annotated and graded Info at the shared import seam; root-anchored, never a bare binary name (closes #1460)
...
- **IOC-provenance panels wait for a running import** — the two full-super-timeline reads are deferred while an import job runs and fire once when it finishes; concurrent requests for the same case share one scan (closes #1447)
- **Jobs chip survives a big import** — the live socket carries the job list in every `job_changed` push and on connect, so the chip and its Cancel work while every HTTP lane is stuck behind the import; the jobs loader now runs first in the connect fan-out so the #1447 deferral has data (closes #1453)
- **IOC provenance is indexed at write time** — every timeline row's terms go into a full-text index as it is written, so the IOC-provenance panels look up matches instead of rescanning the record: 1.4 s instead of 75 s on a 900k-event case, identical answers; existing cases build the index once on their first read (closes #1452)
...
[Unreleased]: https://github.com/hasamba/DFIR-Companion/compare/v0.37.0...HEAD
