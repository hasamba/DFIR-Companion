...
### Added
- **Startup pre-flight diagnostics** — non-blocking self-test on server start probes AI provider, configured enrichment providers (via `probe()`), and Velociraptor reachability; logs OK/WARN/CRITICAL to the console; red banner on the dashboard for AI failures; `GET /diagnostics/preflight` (cached 30 s) + `POST` (force re-run) in Settings → Diagnostics (closes #179).
