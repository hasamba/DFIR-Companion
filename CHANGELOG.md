...
### Added
- **Import from DFIR-IRIS** — pull an existing IRIS case's assets/IOCs/timeline into a Companion case (the reverse of the push); deterministic `importIris` / `irisImport.ts` maps timeline events (severity from colour, MITRE/asset/hash from tags+content), IOCs (type from the IRIS ioc-type or value shape), and assets → evidence events; dashboard "Import from IRIS" picker + `npm run iris:import` (closes #88).
