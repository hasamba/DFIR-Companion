...
### Added
- **Every integration in Settings has a connection test** — Timesketch, Notion, ClickUp, Velociraptor, MISP, Jira and ServiceNow join DFIR-IRIS and Presidio. Each says which of the three states it is in — not configured, connected (naming the account it authenticated as), or configured but unreachable with the remote's own error. The five the dashboard can configure save the fields first and apply them live, so a corrected token needs no restart; Jira and ServiceNow stay read-only and test the running connection. MISP's control sits on the Enrichment tab beside its fields, and rebuilds the IOC provider as well as the push client.
