...
### Fixed
- **A big MFT/USN file import flooded the forensic timeline with Info telemetry and marked every super-timeline row "promoted"** — a file/drop Velociraptor import merged ALL events (including hundreds of thousands of Info-level MFT rows) into the forensic timeline, relying on a post-import "demote" pass to move Info back out to the super-timeline. At a raised `DFIR_MAX_EVENTS` that produced a ~350 MB forensic timeline and a demote step that could stall, leaving 500k+ Info events in case state — so every super-timeline row showed the green "Promoted" badge. The forensic severity gate now runs AT MERGE TIME on the file/drop path: the super-timeline gets every event (it's the complete record) while only graded signal (rank ≥ the per-case forensic gate, default Low) enters the forensic timeline. Info telemetry never floods case state, and the demote pass is no longer load-bearing. Live hunt/external/bundle imports are unchanged.
