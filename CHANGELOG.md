...
### Added
- **Adversary emulation — likely next techniques** — from the matched ATT&CK groups, surfaces the techniques they're known to use that the case *hasn't* observed yet (base-level), ranked by how many matched groups use each + worst-tactic tie-break; predictive hunt priorities, still hypothesis fuel not attribution; offline/deterministic, `DFIR_ADVERSARY_NEXT_MAX` (default 10), in the dashboard *Adversary Hints* panel + report §4.6.1 (closes #121).
