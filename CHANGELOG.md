...
- **The Anonymization panel says which masking needs Presidio** — a read-only **Real names (people)** row, greyed as *needs Presidio* until `DFIR_PRESIDIO_URL` is set, plus a note naming the three kinds of PII that go undetected without the layer (real names, national IDs other than Israeli, IBANs) and making clear that cards / phones / emails still run on the built-in patterns either way. `GET/POST /cases/:id/anon-control` now report `presidioConfigured`.
...
### Added
- **Signed custody manifest in the redacted package** (#362) — a redacted case package now carries a `custody-manifest.json` built over its *redacted* records and signed, so the sender can later prove what they sent and the recipient gets a machine-readable index of the chain. Verification still needs the instance secret, which a recipient does not have.
- **Firefox extension zip attached to every release** (#366) — a `v*` tag now publishes `dfir-capture-extension-firefox-<tag>.zip` alongside the Chrome zip, so installing on Firefox is download-and-unzip rather than clone-and-build. Zipped from inside `dist-firefox/`, so `manifest.json` sits at the archive root where Firefox wants it, and the version comes from `extension/manifest.json` like every other target — there is no second version to bump. Still unsigned, so it loads as a temporary add-on; the AMO listing is separate work. CI builds both targets on every PR so a Firefox-only break is caught before the tag.
- **Firefox build for the capture extension** (#270) — `npm run build:firefox` inside `extension/` emits `extension/dist-firefox/`, loaded in **Firefox 128+** via `about:debugging` → *Load Temporary Add-on…* → `dist-firefox/manifest.json` (the manifest file, not the folder Chrome asks for; and Firefox drops temporary add-ons on restart). Same bundles as the Chrome target — only the manifest differs, and it is generated from `manifest.json` at build time so the two cannot drift. No Mozilla Add-ons listing yet.
- **Custody hashes survive a redacted export** (#362) — the redacted case package's Chain of Custody appendix is now redacted field by field instead of wholesale, so artifact SHA-256s (and the chain's `prevHash` links) stay intact and a recipient can check the chain against the evidence they hold. Paths, source hosts and collector names are still tokenized, and a field added to a custody record later is redacted by default.
- **Chain of custody** (#231) — a court-ready record per artifact: every screenshot and import is recorded automatically with who collected it, when, from where and its SHA-256, and every export appends an `exported` event. Records form a hash chain, so editing or deleting an entry is detectable; a `custody-manifest.json` signed with the instance secret ships in the report and the encrypted archive and pins the chain's length, which the log alone cannot vouch for. Evidence is re-verified in the background when you open a case (all-cases sweep available via `DFIR_CUSTODY_VERIFY_INTERVAL_MS`, off by default), surfaced in Diagnostics, in a Chain of Custody dashboard panel, and as a report appendix.
...
### Added
- **Attack-sequence matching against known playbooks** (#230) — asks whether the case's ATT&CK techniques happened in the ORDER a published ransomware playbook describes, not just whether they overlap. Eight chains distilled from CISA advisories (Conti, LockBit, BlackCat, Akira, Scattered Spider, Black Basta, BlackSuit, Play), matched as a fuzzy subsequence so a noisy, incomplete timeline still scores; each playbook is matched both case-wide and per host, keeping its best. The dashboard's *Playbook Match* panel and report §4.6.2 show a matched / out-of-order / not-observed breakdown per step, every matched step clicks through to the event that evidences it, and a step the chain implies but nothing recorded becomes an Evidence Gaps item with its collection directive. Offline, no AI — and it says "matches the playbook", never "is Conti".
...
- **Attacker session / story reconstruction** (#229) — the forensic timeline re-threaded as per-host chapters instead of one flat run of rows. A session is a contiguous run of activity on one host; it ends on a long gap (default 5 min, `DFIR_SESSION_GAP_S`), or on a successful logon under a different account. Events sharing a concrete indicator (hash, path, IP, decoded-payload IOC) survive a gap up to 3× longer before splitting (`DFIR_SESSION_IOC_GRACE`). Each session reports its host, account, dominant ATT&CK tactic, time span, size and severity range. Pure and deterministic — no AI.
  - **Story view** — a new Attacker Sessions panel above the Forensic Timeline. Click a card to filter the timeline to exactly that session; collapse all for a high-level read of the intrusion.
  - **Per-session AI summary** — "✨ Summarize session" runs one focused call over just that session's events, far cheaper than full synthesis and more coherent than a flat pass over the whole timeline.
  - **Report section** — a new toggleable "Attacker Sessions" section (on in Standard and Technical Detail, off in the Executive Brief). It segments the already scope/false-positive-filtered report state, so it can never cite an event the rest of the report excludes.
  - Events whose host was never recorded are bucketed under an explicit "(host not recorded)" — never dropped, and never dressed up as a machine name, since that bucket can span several hosts.
