...
### Fixed
- **OneDrive's update traffic is not a C2 channel** — a first-party Windows updater (OneDrive, Edge update, Defender) connecting on 80/443 to a vendor range nobody can rent grades Info with the reason in the row, so it stays in the super-timeline instead of being fused into a C2 finding; addresses inside a published Microsoft/Akamai service range are recorded with a `vendor:` note instead of as candidate-C2 `ip` indicators (a Cloudflare edge is named but still an indicator); loopback written the long way (`0:0:0:0:0:0:0:1`) and a module version (`1.0.0.0`) no longer become indicators; a network row now carries its image and destination as prompt tags the 240-character cut cannot reach (closes #1530)
