...
### Fixed
- **Duplicate detection wrongly dropped genuinely-different screenshots (text/log pages).** The perceptual de-duplicator — which skips AI analysis of a screenshot that's the same frame as the previous one — used an **8×8 average-hash (64-bit)**. That's far too coarse for text-heavy evidence: a page of a log/table viewer downscales to a handful of cells that land identically no matter *what* text they contain, so paging through different events (same toolbar/columns, different rows) was flagged as one big duplicate and most of the evidence never reached the model. Replaced with a **512-bit dual difference-hash (dHash)** — 16×16 horizontal **and** vertical — which keys on local brightness gradients in both axes (where text starts/ends in a row, and which rows carry content), so two pages that share UI chrome but differ in content are correctly told apart while a true re-capture still hashes identically (regression-tested with a "same chrome, different rows" case). Dedup is now also **configurable**: `DFIR_DEDUP=off` disables it entirely (every screenshot analyzed), and `DFIR_DEDUP_THRESHOLD` (default 10; lower = stricter) tunes the match distance — both exposed in **Settings → General**. **Recovering an affected case:** the original screenshots were always saved as evidence, so re-run `npm run reanalyze -- <caseId> --all --reset` to analyze every capture (the `--all` flag ignores the stale duplicate flags).
