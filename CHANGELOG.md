...
- **The scope presets say what they measure** — the `1h` / `24h` / `7d` / `30d` buttons are labelled *of activity* and measure back from the newest event in the case, not from the current time. Type into the from/to fields to scope by wall-clock time instead. (closes #738)
