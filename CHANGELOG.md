...
- **The command palette reaches every panel again** — a dashboard view profile writes its layout into the same section-visibility store the palette filtered on, so from the default Now cockpit the palette offered two jumps and the super-timeline was unreachable. A panel the active profile hides is now listed, and picking it reveals that one panel without disturbing the rest of the layout.
