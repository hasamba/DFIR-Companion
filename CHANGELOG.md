...
### Added
- **Response Playbook — turn AI next steps into a trackable checklist (issue #36, Phase 1).** A new **Playbook** section (below _Findings_) converts the synthesized **recommended next steps** and every non-dismissed **Critical/High finding** into actionable tasks the analyst works through: each task has a **status** (To do → In progress → Done / Skipped), **priority**, optional **assignee** + **due date**, and links back to the finding that generated it. A completion badge + progress bar show "_N/M done · X%_", with an **Open only** filter and **▲▼** reordering. Auto-tasks are **re-derived idempotently** on every synthesis and panel load — re-derivation refreshes the wording but **always preserves your status, assignee, due date, notes, and ordering** (and your **custom** tasks). Persisted per case in `state/playbook.json` (`PlaybookStore`), so it **survives synthesis** like comments/tags/notebook, broadcasts changes over the existing WebSocket hub, and is rendered into the report (Markdown + HTML) as a **Response Playbook** section. Pure derivation/merge/stats (`analysis/playbook.ts`) + the store are unit-tested, with route-level tests for the GET-auto-sync, add/patch/sync/reorder/delete flow. _(Phase 2 severity templates + drag-and-drop and Phase 3 ClickUp/Jira/TheHive export remain open on #36.)_
