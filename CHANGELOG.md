...
- **An MCP scp identity file must be an absolute path** — `-i` accepted any string, so a relative or `..` path resolved against whatever the server's working directory was. (closes #850)
...
- **An oversized VQL body is refused with a 400** — `POST /velociraptor/run`, `/hunt` and `/collect-host` took a query of any size up to the 256 MB body limit, which could only ever fail deep inside the CLI spawn; they now refuse past 100,000 characters, above any compiled hunt. (closes #825, #828)
