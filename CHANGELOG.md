...
### Fixed
- **Bash-history uploads now also tag T1567.002**, not just T1041 — was mislabeling plain HTTPS uploads to cloud storage as C2 exfil.
- **Threat-intel verdicts on the case's own infrastructure no longer drive false findings** — flagged as a conflict instead of letting a stale/wrong verdict anchor synthesis on internal assets.
- **`logAggregate` truncation now keeps rare/unique lines instead of the most frequent** — was silently dropping one-off attack lines from large, mostly-benign logs.
...
### Added
- **Cisco ASA firewall syslog importer** (`analysis/ciscoAsaImport.ts`) — deterministic Built/Teardown/Deny parsing, replacing AI log-triage for ASA logs.
- **Apache/Nginx/Squid combined access-log importer** (`analysis/combinedLogImport.ts`) — deterministic parsing for web-server and forward-proxy access logs, replacing AI log-triage for those formats.
