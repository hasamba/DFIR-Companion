...
- **The Geographic Map opens fullscreen** — a ⤢ Fullscreen button on the map fills the screen with it and Esc returns it to the panel, so a world of markers is readable instead of crammed into a 440px strip.
- **Every AI job in the jobs popover names the model running it** — a synthesis, Deep Pass or CSV/log import row shows the text model beside its label, so three rows that all read "synthesis — timed out" can be told apart. The model is pinned when the job is queued, so a finished run keeps naming the model that produced it after Settings points synthesis elsewhere. Jobs no model runs (enrichment, a deterministic import, an MCP run with no `DFIR_MCP_MODEL`) name none rather than guessing.
- **USB device history reaches the forensic timeline** — `Windows.Mounted.Mass.Storage` joins the Best Practice bundle, and a new tagger rule grades a removable-media mount as Low (T1091/T1052.001) so it is no longer left at Info, where the AI could never see it. It also matches USB registry evidence (USBSTOR, MountedDevices, MountPoints2) from any artifact. **Applies to collections imported from here on**: a tagger rule promotes an event during the import that collected it, so evidence already imported stays in the super-timeline — a re-collect is what surfaces it to the AI.
- **`Custom.Windows.EventLogs.Network.Share.Access` added to the Super-Timeline Triage bundle** — share access is searchable per event without flooding the forensic record, which per-file access auditing on a file server would otherwise do.
- **A bundle that will not launch now names the tool that stopped it** — the run pre-flight reports the third-party tools your Velociraptor server has no file for yet, and a hunt that refuses to start names those tools, the artifacts that need them and where to upload them, instead of "no bundle hunt id" and three generic guesses.
- **`Windows.EventLogs.CondensedAccountUsage` added to the built-in Best Practice bundle** — a condensed per-account summary of Security.evtx logon activity (logon types, source hosts, counts and first/last seen), so a quick-wins sweep answers who logged in where without importing every raw logon event.
- **Every integration in Settings has a connection test** — Timesketch, Notion, ClickUp, Velociraptor, MISP, YETI, OpenCTI, Jira and ServiceNow join DFIR-IRIS and Presidio. Each says which of the three states it is in — not configured, connected (naming the account it authenticated as), or configured but unreachable with the remote's own error. The seven the dashboard can configure save the fields first and apply them live, so a corrected token needs no restart; Jira and ServiceNow stay read-only and test the running connection. The three self-hosted threat-intel sources — MISP, YETI and OpenCTI — have their controls on the Enrichment tab beside their fields; each probes with a call that sends no indicator, and the verdict is recorded in the reachability gate, so the provider's ● dot and the enrichment engine agree with what the button just said.
- **Every "Skip TLS verify" flag is configurable from Essential settings** — a self-hosted MISP, YETI, OpenCTI, DFIR-IRIS, Timesketch, Notion, ClickUp or notification webhook behind a self-signed certificate is dead until `DFIR_<NAME>_INSECURE` is set, so all eight now show in the Essential view rather than only under All, alongside a new global "Allow insecure TLS to external hosts" field (`DFIR_TLS_ALLOW_INSECURE_EXTERNAL`, default false) that the non-loopback MITM guard already required but no UI offered. Jira and ServiceNow keep theirs read-only by design. The new global key saves and reloads live, so a connection test or reconnect picks it up without a restart.
- **The setup wizard can configure self-signed DFIR-IRIS and Timesketch** — both steps now carry Skip TLS verify and the global insecure-external opt-in, so the wizard's own connection test can be made to pass instead of dead-ending on a certificate error.
...
- **Ransomware note and renamed-file detection (T1486)** — a ransom-note filename or a file renamed with a known ransomware family's extension is graded above Info, aggregated per host and guarded against system-directory false positives, so it survives the most-severe-first event cap instead of being buried under MFT/USN noise. Extend the family list with `DFIR_RANSOM_EXTS`. (part of #700)
- **Contextual YARA severity** — a YARA hit is graded by where and what it matched instead of a flat High: a self-scan reads as Info, a page-file or memory-dump string as Low (aggregated per host), a broad heuristic as Medium (Low on a signed OS binary), and only a named malware family on a real path stays High. Across four evaluated intrusion scenarios this cut High+Critical findings by 74-86% with zero new false positives in the Critical tier. (closes #700)
- **Lateral movement, drive-by download and cloud-exfil detection** — RDP lateral movement (T1021.001) is graded from `Custom.DFIR.RDPLateralMovementDetection`'s explicit-credential logons, firing only on a genuine remote target so boot noise stays Info; an internet-zone runnable download now also carries T1189 alongside T1204.002; and `rclone`/`restic`/`megasync`/`megacmd` execution recorded in Prefetch grades Medium with T1567.002 from the process name alone. (part of #701)
- **IOCs are classified as threat indicators or observations** — a value that earned a signal (a Medium+ risk tier, or a reputation verdict) is now marked an indicator; a bare file path or hash scraped from evidence with no signal is an observation. Both stay in the case; the dashboard IOC panel and reports break the count out separately instead of presenting thousands of untriaged file paths as equally significant. (part of #701)

### Fixed
- **Velociraptor's own detection content no longer reads as an intrusion** — Sigma rule files and attack-sample event logs unpacked by Velociraptor to run a hunt, and the PowerShell modules Windows generates from cdxml, were graded High and Medium as if an attacker had put them there. Both are demoted to Info; no row is dropped. On two live collections this took High file-system findings from 58 to 11 and Medium event-log findings from 123 to 25. A demotion can never lower a High or Critical verdict.
- **The same attacker action on two machines is now two events, not one** — a Windows event was grouped by *what* happened but never by *where*, so one service installed on twelve servers, one account authenticating to a DC and a workstation, or the security log cleared on three hosts each collapsed into a single row naming exactly one machine. The rest left the case: absent from the timeline, unknown to the asset graph, and every indicator scraped off the merged-away records stamped with the surviving host. `count: 12` was the only trace left, and it cannot say whether twelve means repeats on one machine or one repeat on twelve — which is the difference between a compromised host and a compromised estate. The host now groups the event, as it already did for THOR, Hayabusa, EDR/`ecar` exports and this importer's own non-Windows fallback, so lateral movement reads as lateral movement. Genuine repeats on one host still merge into a counted row, `SRV-A` and `srv-a` are still one host, and an export naming no host at all groups as before. The cost is deliberate: an estate where 500 workstations log the same benign logon now yields 500 rows rather than 1 — bounded, as ever, by the severity floor and the event cap. **Applies to imports from here on**: evidence already imported keeps the grouping it was imported with, so a re-import is what separates it. (closes #659)
- **A PowerShell script block yields its IOCs however it was collected** — a script block that arrived as a *natively parsed* EVTX row produced no indicators at all: not the URL, not the address, not the domain, not a hash. Only DetectRaptor's flattened row shape extracted anything, so whether a case saw its C2 depended on which artifact happened to collect EID 4104 — and the shapes that failed included the two that matter most: `Windows.EventLogs.PowershellScriptblock`, Velociraptor's ordinary way to hunt script blocks, and a Sigma detection wrapping a 4104, the highest-signal row the importer handles. Nothing looked wrong on the way through: the event carried the right host, the right time and a readable description holding the script in full, beside an empty IOC list. The script block is now read as evidence wherever the event is parsed, so the same script yields the same indicators through all of it — Velociraptor eventlog, Sigma and DetectRaptor rows, raw EVTX XML exports, Chainsaw reports, generic SIEM records and Hayabusa timelines alike — and a parsed script block is labelled *Script block logged* with the script itself named in the event, instead of "Event 4104" or the message's boilerplate first line. Two script blocks that differ now stay two events: they previously shared one aggregation key, so all of both scripts' indicators were stamped with whichever event survived the merge and a pivot led to a script that never named them; genuine repeats of one script still merge into a single event. Script-block logging stays **Info** telemetry — it is on by policy on a hardened estate, so the event type is not a verdict and does not by itself enter the forensic timeline or the AI's prompt; a block that is suspicious is graded, as before, by whatever adjudicated it. **Applies to collections imported from here on**: evidence already imported keeps the IOCs it was imported with, so a re-import is what surfaces them. (closes #652)
- **A long script block no longer hides its C2 table** — an imported event message is cut at 4,000 characters so one quoted script cannot fill case state, and the cut said nothing about what it removed. On a real Velociraptor collection 181 events each stored exactly 4,001 characters of PowerShell setup code while the script's indicator table — 17 C2 domains, 10 C2 addresses and an exfil endpoint — sat past the cut, so no readable event in the case named a single C2 host. The cut is unchanged; it now ends with one bounded line saying how many characters it removed and naming the indicators that were in them, so a truncated event is searchable for its own evidence. A cut that removed no indicators says that too, instead of staying silent.
- **A Velociraptor collection now yields the C2 domains named in its script blocks** — the free-text IOC scraper read URLs, IP addresses and hashes out of a collected PowerShell script block, command line or YARA hit string, but had no pattern for a bare domain. On a real case that meant 566 IOCs and not one of type `domain`: seventeen C2 domains sat in the script text while the ten C2 addresses beside them came through, and the AI summary concluded no command-and-control infrastructure was confirmed. Velociraptor and the SIEM importers now share one domain scraper, so the same name is recognised wherever it arrives. It stays quiet on the dotted tokens Windows text is full of — file names, registry keys, version strings, `C:\Windows\Microsoft.NET\…` path components and terminal .NET namespaces such as `System.Net` and `[System.IO]` — while a UNC host (`\\host.example.com\share`) and a real `microsoft.com` still come through. **Two limits worth knowing.** It applies to collections imported from here on: evidence already imported keeps the IOCs it was imported with, so a re-import is what surfaces the domains. And it reaches the row shapes that already extracted URLs and IP addresses — a script block collected as a native parsed-EVTX row yielded no IOCs of any type, which was a separate gap, fixed below (#652).
- **IOC provenance no longer points an indicator at an event that never carried it** — the timeline collapses repeats of the same shape into one event and keeps the first record's text, and the grouping key ignored every digit, so two Velociraptor Sigma network connections to different destinations merged into one row. Every address scraped from the merged-away records was then linked to that one event, and the chain panel showed an external IP beside a line naming a different destination. Network addresses — IPv4 and every IPv6 form, including the compressed `::` ones — now keep events distinct, while repeats that differ only in a port, PID or other volatile id still merge, whatever identifier text sits beside them in any script. Where merging is right, the chain panel says so — each extraction row reports "1 of N merged records" and its time span, and warns when the indicator appears nowhere in the record as stored, which also covers a value sitting past the storage cut-off in a long script block. (closes #640, #643, #646, #649)
- **The Geographic Map draws its world again** — markers sat on an empty grey square because the dashboard's content-security policy allows images only from the companion itself, while the map asked OpenStreetMap for every tile; the browser refused all of them, silently. The companion now fetches tiles itself and serves them from its own origin, so the policy is unchanged and your browser never contacts the tile server. `DFIR_GEOMAP_TILE_URL` still points at an internal tile server for air-gapped work, and a tile server the companion cannot reach is reported over the markers instead of leaving a blank basemap.
- **The dashboard stops running yesterday's code after an update** — its scripts and stylesheets were cached for 24 hours under filenames that never change, so a shipped fix could not appear until the day rolled over and every reload confirmed the old behaviour. They now revalidate on each load and answer unchanged with a 304.
- **Enrichment no longer stops at 100 IOCs and says nothing** — a case with more indicators than `DFIR_ENRICH_MAX` enriched exactly that many and finished. Nothing scheduled the rest, and nothing reported them: the count the cap deferred was added to `skipped`, which already meant *already enriched*, so "done" and "abandoned 150 IOCs" printed the same number. The only way to find out was to re-open the enrichment panel and press OK, which worked by accident — the already-checked marker had moved the cursor on. The cap now bounds a **batch**, not the case: a capped run saves, then starts the next batch where it stopped, up to `DFIR_ENRICH_MAX_BATCHES` (default 20, so 2000 IOCs per Enrich). Every call is still throttled and every batch is still cancellable, so the rate-limit exposure per unit time is unchanged. Whatever the budget still leaves is named in the status line with the two settings that decide it, and recorded as a warning on the run's ledger entry. A batch that queried nothing — every provider probed down — does not chain; the reachability poller resumes the case on recovery, as before.
- **The Anonymization panel no longer reports a dead Presidio analyzer as on** — the **Real names (people)** row was drawn from `DFIR_PRESIDIO_URL` being non-empty, which says the analyzer is *configured*, not that it *answers*. A container that exited days ago still read as *via Presidio*, and the analyst only found out when an AI call failed closed. Opening the panel now probes the analyzer: a dead one turns the row red — *analyzer unreachable*, with a retry — and the note names the URL, the error, and the two ways out. The probe writes nothing, so an outage cannot silently change what the case has saved.
- **A Velociraptor collection can no longer start without a case** — the dashboard pre-fills the case picker from your last session without connecting, and every Velociraptor guard read that field rather than the connected case, so Run launched a hunt on live endpoints for a case nobody had opened — or, for a typed-in id, one that never existed. The server refused nothing either: the hunt went out, the job record then failed to write, and the hunt was left running in Velociraptor with no job card and no way to collect it. Every `/cases/:id/velociraptor/*` route now 404s an unknown case before anything is launched, and Run stays disabled until a case is connected.
- **Essential settings no longer strands a lone field at a third of the line** — a row that contributes one field to Essential drew it at its column share (49%, or 33% in a three-column row) with the rest of the line blank, because hiding its row-mates does not collapse a fixed grid's tracks. 22 rows were in that position — every enrichment and exposure API key among them — so two different widths landed a few rows apart and read as a bug. A lone field now spans its row; rows that show two or more still share it as before.
- **A rejected Timesketch certificate now names the settings that fix it** — the connection test reported Node's own `self-signed certificate ... try running Node.js with --use-system-ca (DEPTH_ZERO_SELF_SIGNED_CERT)`, which names nothing this app has. It now also points at `DFIR_TIMESKETCH_CA`, "Skip TLS verify" (`DFIR_TIMESKETCH_INSECURE`) and the global opt-in a non-loopback host needs — and stays silent on failures that are not certificate failures, so a refused connection is never answered with advice to disable verification.
...
- **The collector's own detection content and sample corpora no longer read as host findings on three more importers** — the noise-suppression check for a tool/sample location matched any path component, which an attacker could forge, instead of only the collector's own root path; it now also covers THOR, ThorZIP and native-Hayabusa, alongside the YARA and Chainsaw paths it already applied to. A bundled EVTX-ATTACK-SAMPLES corpus that made up to 85% of one file's rows no longer reads as evidence from the case host. (part of #700)
- **Bulk artifact imports no longer starve on the AI request budget** — deterministic imports (Velociraptor, THOR, Chainsaw, Hayabusa) now draw from their own rate limiter (default 300/min, `DFIR_IMPORT_RATE_MAX`) instead of the 20/min AI budget, so a multi-file collection no longer stalls behind it; CSV/log imports that call an LLM still meter against the AI budget as before. (part of #700)
- **IOC extraction and "confirmed exfiltration" are more precise** — IPv4 addresses are octet-validated, version fields and code tokens (`aes.iv`, `proc.id`) no longer read as IOCs, and "confirmed exfiltration" now requires an attributable outbound transfer rather than a T1041 tag on a neighboring event in the staging window. (part of #700)
- **A capped import now says how much evidence it dropped** — a 275k-row MFT or 333k-row USN import previously reported only "N events from M rows"; the Velociraptor, Chainsaw and Hayabusa import notes now also say how many rows were omitted at the event cap. (part of #701)
...
- **AI requests now wait 15 minutes before giving up** (was 3) — a synthesis over a real case takes minutes, and the CLI-backed providers (Claude Code, Codex) are local agent processes rather than completion APIs, so every such run died at exactly the old 180000 ms limit and read as "the CLI is broken" when it was not. `DFIR_AI_TIMEOUT_MS` still overrides the default. `.env.example` also declared that key twice, and the stale 180000 copy silently overrode the documented value for anyone who copied the file.
