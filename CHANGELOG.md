...
### Added
- **Cross-tool command-line correlation (#68)** — process-creation events that describe the *same* creation but come from different tools (Sysmon and an EDR, say) with different pids and no shared file hash now merge into one timeline row when they share a normalized command line + parent process + host within a window (default 60s, `cmdlineWindowSeconds`). Each process-creation event now carries a time-independent `chainSignature` (`shortHost|processName|parentName|normalizedCommandLine`); the SIEM/EVTX importer populates the structured `commandLine`, and correlate.ts scrapes it from the description as a fallback. A same-tool corroboration guard keeps distinct commands from one tool separate, so kill-chain steps are never collapsed. Deterministic, no AI.
- **SSH brute-force-success detection (ATT&CK T1110.001)** — the syslog importer now correlates sshd auth lines and flags a successful login (`Accepted password/publickey`) that follows a burst of failures (default ≥5 within 60 min, `DFIR_SSH_BRUTEFORCE_MIN_FAILS`/`_WINDOW_MIN`) from the same source IP → Medium, with the failure count and IP in the description. Deterministic, no AI. More actionable than Timesketch's `ssh_sessionizer` (which only groups sessions).
- **Windows logon-type risk grading** — successful-logon (4624) events now decode the logon-type code into a readable name in the timeline (e.g. "RemoteInteractive/RDP from 203.0.113.9") and grade the risky shapes: external RDP (type 10 from a public IP) and internet-facing network logons (type 3) → Medium with T1021.001/T1078, plus NetworkCleartext (8) and NewCredentials/`runas /netonly` (9) → Medium (T1078/T1550.002). Internal interactive logons stay Low. Applies across the SIEM/EVTX, Chainsaw, and Velociraptor paths. Inspired by Timesketch's login analyzer (which only tags).
- **Lookalike / typosquat domain detection** — a new offline "Lookalike Domain" enrichment provider flags domain IOCs that imitate a bundled list of commonly-impersonated brands (Microsoft, Google, Okta, PayPal, banks, crypto…) via homoglyph-skeleton (incl. IDN/punycode + Cyrillic/Greek confusables), edit distance, and brand-token impersonation → `suspicious` verdict (T1566/T1583.001). Runs entirely on-box (local scope, nothing sent anywhere), so it's on by default; add your own domains via `DFIR_LOOKALIKE_EXTRA_DOMAINS`. Adapted from Timesketch's `phishy_domains` analyzer.
- **NTFS timestomp detection (ATT&CK T1070.006)** — MFT imports (Windows.NTFS.MFT via Velociraptor, MFTECmd via KAPE) now compare a file's $STANDARD_INFORMATION ($SI) and $FILE_NAME ($FN) creation times on the same row and flag likely timestomping as Medium: when $SI is backdated more than the threshold before $FN (default 10 min, `DFIR_TIMESTOMP_THRESHOLD_MINUTES`), or $SI's sub-second precision is zeroed while $FN's isn't. Deterministic, no AI; the tag shows on the event in the Forensic/Super Timeline.
...
- **Live progress for Velociraptor imports** — a large import (e.g. a multi-hundred-thousand-row MFT/USN) now streams `importing X/Y` progress to the import job's progress bar and the status line as it parses, instead of showing nothing until it finishes. The parse runs in chunks and yields to the event loop between them, so the server stays responsive during a big import rather than freezing on one synchronous pass. Output is byte-for-byte identical to the previous parse.
- **Structured, deployable collection directives (investigation-guidance #8)** — "collect X from host Y" next-steps and unknown/partial key questions now carry a machine-actionable `collect{host,artifact,logSource}` target instead of free prose: a one-click "Collect on `<host>`" button launches the matching Velociraptor artifact when the host is a known case asset, and a later import that satisfies the request is detected automatically so the recommendation stops re-appearing and the served question is re-evaluated against the new evidence.
- **Evidence Gaps dashboard panel + report section (investigation-guidance #9)** — uncovered kill-chain phases now render as structured items with a deterministic host+artifact collection directive (one-click deploy where possible), in a new "Evidence Gaps" panel and report §4.6.2, instead of only a prompt-only text list the model saw but the analyst didn't.
- **Zero-yield import warnings (investigation-guidance #10)** — a large file routed through AI log/CSV triage that produces zero events (the failure mode that can silently drop an incident's recon/exfil front half) now surfaces a red warning on the import banner and an Evidence Gaps "blind spot" row, naming the file and suggesting a re-run or manual grep.
- **Second-look loop — bounded raw re-query (investigation-guidance #11)** — after synthesis, open hypotheses/questions plus a new model-issued `evidenceRequests` list are resolved against the *complete* super-timeline (not just the sampled window), promoting matching not-yet-analyzed events and triggering exactly one bounded re-synthesis, reaching raw rows the sampler never showed the model.
- **Immediate false-positive cascade (investigation-guidance #12)** — marking a finding/IOC/event false positive now synchronously re-evaluates every key question, next-step, and hypothesis that depended on it (badged "stale — re-synthesis queued" / "needs review") instead of waiting for the next async synthesis run.
- **Rabbit-hole detection (investigation-guidance #13)** — findings are now scored connected / disconnected / undetermined against the main corroborated evidence-graph component, so a disconnected finding (a planted red herring, an unrelated benign event) is demoted and badged "possible rabbit hole" instead of ranking alongside real leads.
- **ACH-style hypotheses (investigation-guidance #14)** — hypotheses now track contradicting evidence, a discriminating host+artifact, and an "exhausted" flag (set once enough linked hunts come back empty), ranked fewest-contradictions-first — the classic Analysis-of-Competing-Hypotheses fix for red herrings winning unopposed.
- **Per-case prevalence baseline + proactive FP-pattern propagation (investigation-guidance #15)** — the tool now tracks how often each normalized activity pattern occurs across a case's timeline (rare events earn a selection seat over common noise) and, after each import, flags new events that reproduce an already-dismissed FP pattern for one-click bulk dismissal.
- **Content-based event tagger (Timesketch-style `tags.yaml`)** — a YAML rule engine (`contains`/`equals`/`regex`/`exists` over any real event field) tags matching events, raises severity, and unions MITRE techniques, running automatically after import or on demand from Super-Timeline → Content tagger; the AI-assisted rule authoring above builds on this engine.
- **Composite IOC risk score (#63)** — every indicator now gets one `critical`/`high`/`medium`/`low`/`benign` tier from a transparent weighted rubric (verdict, corroborating source count, KEV, an own-infrastructure guard, NSRL/whitelist), shown as a colour-keyed badge, an IOC risk filter lens, and a Risk column in the CSV/markdown reports.
- **KEV / tool-confirmed / unconfirmed-lead confidence badges on findings (#61)** — findings now show whether they're independently corroborated by a CISA-KEV actively-exploited CVE, confirmed by a tool-graded detection ("tool-confirmed"), or supported only by raw Info telemetry with no verdict or intel ("unconfirmed lead"); confidence-lowering signals only, never a boost.
- **Synthesis coverage audit (#62)** — the synth-meta card now shows how many in-window events a run actually considered vs. omitted (by prompt-size budget, FP filter, or scope window), plus how many budget-omitted events were Critical/High — visibility into what a large-timeline synthesis run left out.
- **Per-source noise/trust scores (#66)** — every event source now carries a trust weight (CrowdStrike/Defender detections > Sigma-engine hits > raw Velociraptor artifacts > generic logs), used to pick the canonical wording when correlating duplicate detections and to cap confidence on findings supported only by low-trust sources; analysts can override a source's trust per case.
- **Learn from dismissed findings (#65)** — repeated reasoned dismissals of the same activity pattern now accumulate into a per-case ledger; new activity resembling a repeatedly-dismissed pattern is surfaced with lowered (not zero) confidence unless independently corroborated.
- **On-demand hypothesis falsification review (#71)** — a "Review" button runs a focused for/against pass over open hypotheses (plain-English case for and against each, plus an advisory recommended status) without re-running full synthesis; never mutates a hypothesis until the analyst clicks Apply.
- **`DFIR_MAX_EVENTS` — configurable event ingestion cap (#110)** — the previously hardcoded 2000-event-per-import safety cap is now overridable via `DFIR_MAX_EVENTS` (guarded against 0/negative/NaN silently reinstating the default), for cases that need a full MFT/USN import.
- **Case Statistics panel in Diagnostics (#36)** — a new panel shows per-case totals, per-source event breakdown, and import velocity.
...
### Changed
- **CSV and log AI extraction now use the text model (`DFIR_AI_SYNTH_MODEL`), not the vision model** — the split is now vision vs text: `DFIR_AI_MODEL` reads screenshots only, while `DFIR_AI_SYNTH_MODEL` handles CSV extraction, log triage, and synthesis (unset → falls back to `DFIR_AI_MODEL`, so single-model setups are unaffected). CSV/log triage is text reasoning, not OCR, and a cheap vision-tier model fails it **silently** — returning no events rather than wrong ones. Measured with `npm run eval:real`: `gpt-4o-mini` found zero of six large CONNECT-to-mega.nz transfers in a proxy log on every run, while `gemini-2.5-pro` found them every time on identical input. If you run a two-tier setup, your log/CSV imports will now use (and be billed at) the stronger model.
...
- **Second-opinion deltas now keyed by semantic content, not exact title (#69)** — a `semanticKey` (dominant ATT&CK technique + order-independent noun phrase) replaces raw-title matching between primary and second-opinion synthesis runs, so a reworded-but-equivalent finding keeps its delta id and the analyst's accept/reject decision instead of showing up as a spurious new delta. Live-verified on the demo case: agreements 16→28, total deltas 31→7.

### Fixed
- **Work-log filter could drop real detections that merely named a tool** — the analyst-work-log filter that keeps tool-usage narration ("Velociraptor notebook accessed") out of the forensic timeline only inspected the description text, so an event whose model-written wording read like tool chrome was dropped even when the importer had already attached hard evidence to it. It now also honours a structured-field gate (#67): any populated `sha256`/`md5`/`path`/`asset`/`processName`/`pid` or MITRE technique on the event overrides the drop, so a genuine detection is kept even when its description mentions the tool it was seen in. Pure UI-navigation rows (no evidence fields) still drop as before.
- **Large deterministic imports (big MFT/USN) could peg the CPU for an hour and freeze the server** — merging an import's events into case state deduplicated the forensic timeline with a linear `find()` per incoming event, an O(n²) scan that melts down once the timeline holds hundreds of thousands of events (e.g. a full MFT with `DFIR_MAX_EVENTS` raised). One import could burn 75+ minutes of blocked, single-threaded CPU. The dedup now uses an id→event index (O(n)); a 120k-event merge drops from minutes to ~0.6s.
- **Large imports could fail with `EPERM ... rename ...tmp -> investigation.json`** — a big state write (e.g. a 90 MB USN-journal import) held the state file long enough for antivirus / the search indexer to outlast the atomic-save retry budget, failing the import even though nothing was wrong. The rename now retries ~4× longer (20 attempts, ~8.4s) and is operator-tunable via `DFIR_ATOMIC_WRITE_RETRIES`; a save that only succeeds after retrying now logs a warning naming the likely cause (exclude the cases root from real-time scanning).
...
- **`caseId` wasn't validated on lock-status/unlock routes** — those two routes skipped the `isValidCaseId` guard every other case-password route already had, letting a crafted caseId reach the case-meta lookup before an existence check; closed with the same validation + a regression test.
- **Renaming a manually-added asset was lost** in the asset-graph view.
- **The enrich endpoint falsely reported acceptance when no enrichment providers were enabled.**
- **A partial dwell-window update could fail validation instead of merging with the existing window.**
...
- **Dashboard UI cleanups** — fixed synthesis job/status races and a drag-order regression on reorderable panels.
