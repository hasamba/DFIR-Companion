...
### Added
- **Run hunts in Velociraptor (API)** — with a Velociraptor `api_client` config set (`DFIR_VELOCIRAPTOR_API_CONFIG`), the hunt-pivot modal's Velociraptor card becomes an editable VQL box with a **▶ Run in Velociraptor** button that executes the query against the server and renders the result rows inline. New `VelociraptorClient` (`integrations/velociraptor/velociraptorApi.ts`) drives it by shelling out to the `velociraptor` binary with `--api_config` (the binary handles gRPC + mTLS) via an injectable runner — no new dependency, no network/process in tests. Route `POST /velociraptor/run`; `/health` now reports `velociraptorEnabled` (gates the button). Off by default (opt-in); localhost + analyst-driven; spawned without a shell (VQL is a single argv — no command injection), with per-query timeout + row/output caps. 12 unit tests.
