...
### Added
- **D3FEND defensive countermeasures** — for each identified ATT&CK technique, the MITRE D3FEND hardening/detection/isolation countermeasures grouped by defensive tactic, offline + AI-free from the bundled `data/d3fend-map.json` (`npm run data:update-d3fend`); `GET /cases/:id/d3fend-countermeasures`, dashboard *Defensive Countermeasures* panel + toggleable report section (closes #178).
