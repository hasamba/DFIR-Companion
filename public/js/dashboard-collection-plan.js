...
(function () {
  ...
  function fetchCollectionResults(rb, target) {
    const clientId = rb.dataset.cid, flowId = rb.dataset.fid, artifact = rb.dataset.art;
    const sources = (rb.dataset.src || "").split(",").filter(Boolean);
    target.innerHTML = "<span data-safe-style='color:var(--text-muted)'>fetching results…</span>";
    return fetch("/velociraptor/collect-results", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ clientId, flowId, artifact, sources }) })
      .then((r) => r.json().then((j) => ({ ok: r.ok, j })))
      .then(({ ok, j }) => {
        if (!ok || j.error) { target.innerHTML = `<div data-safe-style="color:var(--sev-high)">error: ${esc(j.error || "failed")}</div>`; return 1; }
        ...
        if (n) { target.innerHTML = renderVqlRows(j); return n; }
        if ((j.flowState || "").toUpperCase() === "FINISHED") {
          target.innerHTML = "<span data-safe-style='color:var(--text-muted)'>collection finished — 0 rows returned.</span>";
          return 1;   // terminal — stop auto-polling
        }
        target.innerHTML = "<span data-safe-style='color:var(--text-muted)'>no results yet — the endpoint reports on its next poll. Click ↻ to refresh.</span>";
        return 0;     // still pending — keep polling
      })
      .catch((e) => { target.innerHTML = `<div data-safe-style="color:var(--sev-high)">error: ${esc(e.message)}</div>`; return 1; });
  }
