...
(function () {
  ...
  // Gap leads from the analyst's OWN attribution assertions (#1405): for an assertion whose label
  // is a known group's name/alias, the techniques that group is documented to use that this
  // case's graded evidence has not shown. Derived server-side (GET /cases/:id/attribution-gap-leads),
  // fetched beside the hints, rendered under them. Absent when there is no matching assertion.
  let gapLeadsData = null;
  function loadAdversaryHints(caseId) {
    fetch(`/cases/${caseId}/adversary-hints`)
      ...
      .catch(() => {});
    fetch(`/cases/${caseId}/attribution-gap-leads`)
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        gapLeadsData = d && Array.isArray(d.leads) ? d : null;
        renderAdversaryHints();
      })
      .catch(() => {});
  }
  function techniqueRow(n, support) {
    ...
    const nm = n.name ? `<span class="adv-next-name">${esc(n.name)}</span>` : "";
    const ds =
    ...
    return `<div class="adv-next-row">${tech}${nm}<span class="adv-next-tactic">${esc(n.tactic)}</span>${support || ""}${huntBtn}</div>${ds}`;
  }
  function gapLeadsHtml() {
    const g = gapLeadsData;
    if (!g || !g.leads.length) return "";
    const cards = g.leads
      .map((l) => {
        const rows = l.techniques.map((t) => techniqueRow(t, "")).join("");
        const more = l.total > l.techniques.length ? `<div class="adv-next-sub">… and ${esc(l.total - l.techniques.length)} more not shown</div>` : "";
        const head = `<div class="adv-card-head"><span class="adv-name">${esc(l.tier)} · ${esc(l.label)}</span><span class="adv-aliases">→ <a href="${escAttr(l.group.url)}" target="_blank" rel="noopener">${esc(l.group.id)} ${esc(l.group.name)}</a></span><span class="adv-score" title="${escAttr(l.basis)}">${esc(l.observedCount)} of ${esc(l.observedCount + l.total)} of the group's techniques shown in this case</span></div>`;
        const body = l.techniques.length
          ? `<div class="adv-next-list">${rows}</div>${more}`
          : `<div class="adv-next-sub">Every technique this group is documented to use is already shown in this case's graded evidence.</div>`;
        return `<div class="adv-card">${head}${body}<div class="adv-next-sub">${esc(l.caveat)}.</div></div>`;
      })
      .join("");
    return (
      `<div class="adv-next">` +
      `<div class="adv-next-h">⌖ Gap leads from your attribution assertions</div>` +
      `<div class="adv-next-sub">For each attribution assertion you recorded whose label is a known ATT&CK group, the techniques that group is documented to use that this case's graded evidence has <strong>not shown</strong> — not observed ≠ did not happen. Hunt leads from your own claim, never evidence of absence, never attribution.${g.unmatchedAssertions ? ` ${esc(g.unmatchedAssertions)} assertion(s) name no known group and are not listed.` : ""}</div>` +
      cards +
      `</div>`
    );
  }
  ...
  function renderAdversaryHints() {
    ...
    const next =
      d.nextTechniques && d.nextTechniques.length
        ...
        : "";
    el.innerHTML =
      caveat + meta + `<div class="adv-list">${cards}</div>` + next + gapLeadsHtml();
  }
  ...
  function huntForTechnique(techId, btn) {
    ...
    const info =
      ((adversaryHintsData && adversaryHintsData.nextTechniques) || []).find((n) => n.id === techId) ||
      ((gapLeadsData && gapLeadsData.leads) || []).flatMap((l) => l.techniques).find((n) => n.id === techId);
