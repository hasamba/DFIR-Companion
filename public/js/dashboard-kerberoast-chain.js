...
(function () {
  ...
  function useRow(u) {
    const same = u.sameObservedAddress
      ? `${esc(u.sameObservedAddress.useAddress)} = request <code>${esc(u.sameObservedAddress.requestEventId)}</code> (seen by ${esc(u.sameObservedAddress.requestObserver)}) <span data-safe-style="color:var(--text-muted)">a shared address may be NAT / VPN / VDI</span>`
      : "—";
    return `<tr><td><code>${esc(u.eventId)}</code></td><td>${esc(String(u.at).slice(0, 19))}</td><td>${esc(u.host)}</td><td>${esc(u.kind)}${u.logonType !== undefined ? ` (type ${esc(String(u.logonType))})` : ""}</td><td>${esc(u.account)}${u.realmState === "not-established" ? " <span data-safe-style='color:var(--text-muted)'>(candidate: realm not established)</span>" : ""}${u.initiator ? ` <span data-safe-style='color:var(--text-muted)'>by ${esc(u.initiator)}</span>` : ""}</td><td>${esc(u.placement)}</td><td>${esc(u.hostBaseline)}</td><td>${u.firstSeenHost ? "yes" : "no"}</td><td>${same}</td><td>${esc(u.detail || "")}</td></tr>`;
  }
  ...
  function renderKerberoastChain() {
    ...
    const blocks = chain.accounts.map((a) => {
      ...
      return `<details open data-safe-style="margin-bottom:8px;border:1px solid var(--border);border-radius:6px;padding:4px 8px">
        <summary data-safe-style="cursor:pointer"><b>${esc(a.service)}</b> <span data-safe-style="color:var(--text-muted);font-size:12px">realm ${a.realm ? esc(a.realm) : "not established"}</span> — <span data-safe-style="color:${stageColor}">${esc(a.stage)}</span> <span data-safe-style="color:var(--text-muted);font-size:12px">${esc(a.stageReason)}</span></summary>
        <div data-safe-style="padding:6px 0;font-size:12px">
          <div>${esc(a.requestsTotal)} request(s): ${esc(String(a.rc4Count))} RC4, ${esc(String(a.aesCount))} other, ${esc(String(a.refusedCount))} refused${a.read.ticketRowsUnread ? `; ${esc(String(a.read.ticketRowsUnread))} not read` : ""}. ${esc(a.rc4Words)}.</div>
          <div data-safe-style="margin-top:2px">Earliest RC4 request: ${a.t0 ? esc(String(a.t0).slice(0, 19)) : "—"}${a.t0Anchors ? "" : " <span data-safe-style='color:var(--text-muted)'>(anchors no before / after split)</span>"}. Baseline before it: ${baseline}.</div>
          ...
          ${a.toolEvidence.length ? `<div data-safe-style="margin-top:4px">Roasting-tool rows by this account's requester: ${a.toolEvidence.map((id) => `<code>${esc(id)}</code>`).join(" ")}</div>` : ""}
