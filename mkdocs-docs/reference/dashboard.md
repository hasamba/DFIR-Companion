...
The preview also reports what the **last synthesis** read. On a small case synthesis already reads every graded event, so a deep pass would read the same events or fewer: the panel says so and **Run** stays off unless you tick **Run anyway** (a legitimate re-read after changing the model). If the timeline changed since that synthesis, the panel says the audit is stale and asks for a fresh synthesis first; if the size limit cut events from the synthesis prompt, it names how many a deep pass at Low+ would recover. Gated on the **synthesis** provider, not vision — see [Advanced → Synthesis Grouping & Budget](advanced.md#synthesis-grouping--budget) and [Settings → AI](settings.md#ai) for the tunables (`DFIR_DEEP_PASS_MAX_BATCHES`, `DFIR_AI_OBSERVE_PROMPT_FILE`).
