...
A free pre-flight preview reports the events/rows/batches/tokens each severity floor would cost **on this case** before anything is spent. **Run** shows live batch progress with **Cancel**; **Re-synthesize**/**2nd opinion** are locked meanwhile, since a deep pass ends in its own synthesis call. The result card (**What did my last deep pass do?**) names the floor, events, batches and observations, flags **partial coverage** in red if any batch failed, survives a page reload, and says when a later synthesis has replaced its conclusions; refusals (over the batch ceiling, or a closed/archived case) render as guidance naming a floor that would fit.

The preview also gives a verdict from what the **last synthesis** read. **Deep pass will not help** — synthesis already read every graded event, so a deep pass would read the same events or fewer; **Run** stays off unless you tick **Run anyway** (a legitimate re-read after changing the model). **Deep pass: benefit unknown** — the timeline changed since that synthesis; run synthesis again first. **Deep pass will help** — the size limit cut events from the synthesis prompt, and the panel names how many a deep pass at Low+ would recover. Gated on the **synthesis** provider, not vision — see [Advanced → Synthesis Grouping & Budget](advanced.md#synthesis-grouping--budget) and [Settings → AI](settings.md#ai) for the tunables (`DFIR_DEEP_PASS_MAX_BATCHES`, `DFIR_AI_OBSERVE_PROMPT_FILE`).
...
**Intel origins, not providers** — the verdict factor counts the *origins* the intel records name, not the providers that answered. VirusTotal and AbuseIPDB sum other parties' answers and count as one origin each; Hunting.ch's four abuse.ch back-ends are one; MISP, OpenCTI and YETI *relay* records other parties created, so a MISP event created by abuse.ch beside a ThreatFox hit reads as "2 hits, 1 origin", not two. A relayed record that names no creator reads "lineage not recorded" and counts as no origin — it never becomes a second confirmation (re-check with force to record the creator). Two named origins read "independence not established": only a Medium+ event in this case carrying the value earns "corroborated". The badge on a relayed hit says "created by …" or "origin not recorded".
