...
**Hayabusa Full** and Best Practice's Hayabusa take this window through `Windows.Hayabusa.Rules`' own `DateAfter` and `DateBefore` parameters. A custom range sets both; a relative preset sets `DateAfter` only. To keep the same dates on every run instead, put them in the bundle's **Advanced parameters** (Settings → Velociraptor → *Edit*), for example `{"Windows.Hayabusa.Rules": {"DateAfter": "2026-09-01T00:00:00Z"}}`. A date set there wins over the run form's window.
