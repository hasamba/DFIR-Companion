...
- **A row that does not show its whole record says so.** When anything shown was clipped or
  ...
  rebuilt past 600 characters — the description ends in an identity mark (`#` and 32 hex characters of a
  digest of the row's full key). Two records that read alike then stay two rows through import; the
